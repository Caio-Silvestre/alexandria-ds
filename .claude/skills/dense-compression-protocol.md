# Pharos Translation Protocol

Apply this protocol to create a dense translation for an Pharos component `.doc.mjs` file. The dense translation lives in the same file as a `docsDense` export and is the source the CLI reads when called with `--lang dense`.
