...
Every component directory has a `{Name}.doc.mjs` file with structured documentation. The Pharos CLI reads these files to generate agent-friendly docs, skill files, and reference material.
...
```bash
pharos component Button                       # Full docs (default)
pharos --detail compact component Button       # Token-optimized format
pharos --detail brief component Button         # Minimal one-line summary
pharos --lang zh component Button              # Chinese prose, same structure
pharos --lang dense component Button           # Compressed prose, same structure
pharos --detail compact --lang dense component Button  # Compact + compressed
```
...
```bash
pharos docs                          # list topics
pharos docs tokens                   # full output
pharos docs tokens spacing           # single section
pharos --detail compact docs tokens  # agent-friendly
pharos --lang dense docs tokens      # compressed prose
```
