# Copilot instructions for Pharos

Pharos is a React design system built with StyleX and shipped as a set of
`@pharos-ds/*` packages from this monorepo. When reviewing a pull request or
...
- **`CLAUDE.md`** — AI guidance: package manager, the Pharos CLI bootstrap,
...
- **Contributing wiki** — the review protocol lives here:
  ...
  and [Contributing with AI Assistants](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Contributing-with-AI-Assistants).
...
- **Removing a themeable surface.** Replacing a token, a `themeProps` target, or
  ...
  Cascade Model in [Theming Infrastructure](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Theming-Infrastructure))
...
- **Raw CSS or hand-rolled JS style workarounds** for anything StyleX supports
  ...
  where an Pharos primitive exists.
...
- **Hardcoded user-facing strings.** All UI text goes through `useTranslator()`
  / the i18n key system (`@pharos-ds/no-hardcoded-i18n-string`), and new keys must
  match the required format (`@pharos-ds/i18n-key-format`).
- **Public API-convention violations** (see [API Conventions](https://github.com/Caio-Silvestre/alexandria-ds/wiki/API-Conventions)) —
