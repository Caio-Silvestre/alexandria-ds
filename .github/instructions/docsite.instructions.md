...
The docsite (`apps/docsite/`) is a Next.js + StyleX app that documents Pharos.
...
- Importing a built theme directly in a page (e.g.
  `import {fooTheme} from '@pharos-ds/theme-foo/built'`). Use `themeObjects`
  from the generated `themeRegistry` instead.
- Hand-maintained arrays of component names. Use `componentRegistry`.
- Package-name switches like `if (pkg === '@pharos-ds/core')`. Let the
...
## Idiomatic Pharos (nudge, don't block)

The docsite is Pharos's own showcase — it should be built _with_ Pharos, not
around it. When a change reaches for raw HTML/CSS where an Pharos primitive
...
- Compose from Pharos components/primitives over raw elements — `VStack`/
...
reserve firm flags for raw CSS/hardcoded tokens that have a clear Pharos
equivalent.
...
data-from-pipeline rule is the primary gate, plus the idiomatic-Pharos and
