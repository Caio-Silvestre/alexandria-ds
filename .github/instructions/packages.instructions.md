...
These paths ship as published `@pharos-ds/*` packages, so review them
against Pharos's API guidance and component review protocol.
...
- **Is it a breaking change?** Scan for: a removed/renamed public export, prop,
  ...
  removed/renamed/changed public API, a **codemod** under `pharos upgrade`.
...
- **New package** — a net-new `@pharos-ds/*` package — see Lifecycle &
...
Many Pharos contributions come from designers building with an AI assistant.
...
- **[API Conventions](https://github.com/Caio-Silvestre/alexandria-ds/wiki/API-Conventions)** —
  ...
  `PharosButton`), prop patterns, and composition rules. Key principles to
  enforce:
...
- **[Component Specification Protocol](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Specification-Protocol)** —
  the process new components must follow.
- **[API Arbitration](https://github.com/Caio-Silvestre/alexandria-ds/wiki/API-Arbitration)** —
...
**[Design Conventions](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Design-Conventions)** —
...
Pharos components and templates move through a staging lifecycle
([Component Lifecycle](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Lifecycle),
[Component Hardening Protocol](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Hardening-Protocol)).
...
`@pharos-ds/lab` is the canary-only staging area (`private: true` +
`pharos.canaryOnly`) where new components develop and harden;
`@pharos-ds/core` ships to stable consumers. The expected path is
...
with an `@pharos-ds/*` name, new `exports`, its own build/release wiring).
...
- **Publish posture is intentional** — `private: true` + `pharos.canaryOnly` for
...
[Contributing Templates](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Contributing-Templates)
...
- **Avoid unnecessary wrapper elements — prefer props and hooks.** Pharos favors
...
- **Reuse the existing accessibility primitives — don't hand-roll.** Pharos
