...
These are Pharos **templates** — page templates (`templates/pages/<name>/page.tsx`
...
**[Template Grading Rubric](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Contributing-Templates#template-grading-rubric)**
in [Contributing Templates](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Contributing-Templates).
...
1. **Pharos component purity (30)** — count JSX tags; every raw lowercase HTML
   tag (`div`, `span`, `button`, `nav`, `ul`, `p`, `h1`–`h6`, `form`, `input`,
   `table`, `img`, …) is a flag unless it has no Pharos equivalent. Use
   ...
   `<img>` with an pharos CDN src, `<form>` wrapping `FormLayout`,
...
3. **Custom CSS (15)** — style through Pharos props (`gap`, `padding`,
   `variant`, `size`, `color`, `level`, `minChildWidth`, `height`), not custom
   CSS. Count each declaration in `stylex.create`/`style={{}}`, and each
   `className`/`stylex.props()` usage. Token-valued Pharos props do **not**
   count.
...
5. **Doc metadata (10)** — read the `.doc.mjs`. Pages need
   ...
   `componentsUsed` listing **every** Pharos component actually used. Flag
   ...
   `3`).
6. **Image handling (5)** — images only from the pharos CDN via the permanent
   lookaside URL (`https://pharos-ds.vercel.app/assets/<asset>.png`)
...
7. **Code quality (10)** — `'use client';` first line; `export default`;
   self-contained imports (`@pharos-ds/core/*`, `@heroicons/react/*`, or
...
[Component Lifecycle](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Lifecycle#promotion-gates).
