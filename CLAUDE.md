...
Tests how well AGENTS.md helps LLMs generate correct Pharos component code.
...
1. Run `pnpm -F @pharos-ds/vibe-tests interactive --sample <count>` to set up iteration
2. Spawn parallel subagents (one per test prompt) to:
   - Read the task file from `results/<iteration>/tasks/{promptId}.json`
   - Generate code for the prompt using Pharos components (AGENTS.md auto-injected)
...
4. Run `pnpm -F @pharos-ds/vibe-tests aggregate --iteration <id>` to see results
...
For architectural context, decisions, and research, see the **[GitHub Wiki](https://github.com/Caio-Silvestre/alexandria-ds/wiki)**:
...
Pharos CLI|Run from repo root. Load agent docs before any component work.
PHAROS="node packages/cli/bin/pharos.mjs"
BOOTSTRAP (run every branch, <500ms):
$PHAROS help # discover all commands and options
$PHAROS docs # list available doc topics
$PHAROS docs principles --dense # design rules, anti-patterns, xstyle, tokens
$PHAROS docs tokens --dense # spacing, color, radius, typography, shadow
$PHAROS docs theme --dense # theme provider, light/dark, overrides
$PHAROS component --list # all components grouped by category
$PHAROS template --list # available page templates
ON DEMAND:
$PHAROS component <Name> --dense # props, variants, usage, anatomy for one component
$PHAROS template <name> # emit full page source
$PHAROS template <name> --skeleton # layout skeleton with spatial annotations
$PHAROS swizzle <Name> # eject component source for deep customization
$PHAROS upgrade --apply # run version migration codemods
...
RULE: always run $PHAROS component <Name> --dense before modifying a component
RULE: after @pharos-ds/core bump, always run $PHAROS upgrade --apply
