...
The OSS documentation site for Pharos. Built with Next.js and StyleX.
...
| `themeRegistry.ts`     | Installed `@pharos-ds/theme-*` packages | Built theme objects, keyed by package name                    |
...
- No `import {fooTheme} from '@pharos-ds/theme-foo/built'` in page files; use `themeObjects` from the generated `themeRegistry`
- No hand-maintained arrays of component names; use `componentRegistry`
- No `if (pkg === '@pharos-ds/core')` switches; let the pipeline classify packages
...
| -------- | ------------ | ---------------------------------- | -------------------------------------- |
| `latest` | `@latest`    | the last **published** npm release | production (`pharos-ds.vercel.app`)    |
...
always resolve `@pharos-ds/core` from the bundled workspace version.

> **⚠️ Contributor gotcha: library changes don't show on the production site
> until they're released.** Production (`pharos-ds.vercel.app`) documents the last
...
2. Add `"@pharos-ds/theme-<name>": "*"` to `apps/docsite/package.json` dependencies
3. Add `@import "@pharos-ds/theme-<name>/theme.css"` to `src/app/globals.css`
...
2. Add `"@pharos-ds/<name>": "*"` to `apps/docsite/package.json` dependencies
3. Run `pnpm generate`
...
│   ├── app/
│   │   ├── globals.css       # CSS imports (reset, pharos base, theme stylesheets)
