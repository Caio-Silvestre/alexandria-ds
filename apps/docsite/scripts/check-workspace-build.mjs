...
/**
 * Preflight for the docsite: fail fast when @pharos-ds/core isn't built.
 *
 * The docsite's `generate` step runs `pharos theme build`, which imports the
 * compiled @pharos-ds/core theme entry (dist/theme/index.js). In a fresh
...
// core's built theme module. `build:theme` (pharos theme build) imports the
// compiled @pharos-ds/core/theme entry, so if this file is missing, the
...
// docsite-test job runs — it builds only @pharos-ds/core). The theme CSS
...
if (missing.length > 0) {
  console.error(
    [
      '',
      '⚠  Pharos workspace packages aren\'t built yet.',
      '   The docsite consumes the built output of @pharos-ds/core.',
