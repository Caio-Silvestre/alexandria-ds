 ...
 *
 * Build-time data extraction for the Pharos docsite.
 ...
 * find a matching subpath export (e.g. `@pharos-ds/core/Chat`).
 *
...
function generatePackageRegistry() {
  ...
  const packages = packageDirs
    .map(dir => {
      ...
      return {
        name: raw.name,
        displayName: raw.displayName || raw.name.replace('@pharos-ds/', '').replace('theme-', 'Theme: ').replace(/^\w/, c => c.toUpperCase()),
        version: raw.version,
...
export interface ComponentEntry {
  ...
  moduleName: string;
  directory: string;
  /** Resolved import path, e.g. \`@pharos-ds/core/Chat\`. Derived from package.json exports. */
...
async function generateThemeRegistry(packages) {
  ...
  const themePackages = packages.filter(p => p.name.startsWith('@pharos-ds/theme-'));
...
`);
  ...
  const imports = themePackages.map(p => {
    const slug = p.name.replace('@pharos-ds/theme-', '');
