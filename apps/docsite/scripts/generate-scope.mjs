 ...
 *
 * Maps every Pharos module available in the playground preview iframe.
...
const SCOPE_THEMES = [
  {pkg: '@pharos-ds/theme-neutral', name: 'neutralTheme', subpath: '/built'},
  {pkg: '@pharos-ds/theme-matcha', name: 'matchaTheme', subpath: '/built'},
];
...
// compiled stylex.props() understands, so `xstyle` passed into @pharos-ds/core components
...
for (const name of components) {
  lines.push(`import * as ${name} from '@pharos-ds/core/${name}';`);
}
lines.push('');
...
// ── Theme + tokens ──────────────────────────────────────────────────
lines.push("import {Theme} from '@pharos-ds/core/theme';");
lines.push("import type {DefinedTheme} from '@pharos-ds/core/theme';");
lines.push("import {createElement, type ComponentProps} from 'react';");
lines.push("import * as pharosTokens from '@pharos-ds/core/theme/tokens.stylex';");
lines.push('');

// ── Hooks ──────────────────────────────────────────────────────────────
// camelCase, so the PascalCase component scan skips it — add it explicitly so
// templates using useMediaQuery etc. render.
lines.push("import * as Hooks from '@pharos-ds/core/hooks';");
lines.push('');
...
lines.push("  '@pharos-ds/core/theme': {Theme: ControlledTheme},");

// tokens.stylex
lines.push("  '@pharos-ds/core/theme/tokens.stylex': pharosTokens,");

// hooks (useMediaQuery, etc.)
lines.push("  '@pharos-ds/core/hooks': Hooks,");

// Per-component subpath entries
for (const name of components) {
  lines.push(`  '@pharos-ds/core/${name}': ${name},`);
}

// Barrel export — all components spread
const spreads = components.map((n) => `    ...${n},`).join('\n');
lines.push(`  '@pharos-ds/core': {
${spreads}
  },`);
