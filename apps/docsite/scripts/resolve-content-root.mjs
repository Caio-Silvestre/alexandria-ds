 ...
 * *which version of the @pharos-ds/* packages* supplies that documentation:
 *
 ...
 * (`npm view @pharos-ds/core version`) so it always tracks the current
 * published release with nothing to hand-maintain. Each docsite @pharos-ds/*
...
const VERSION_SOURCE_PKG = '@pharos-ds/core';
...
/**
 * The docsite's @pharos-ds/* dependencies that are documented on the
 ...
 *   - `pharos.canaryOnly` packages (e.g. @pharos-ds/charts, @pharos-ds/lab)
...
function latestPublishablePackages() {
  ...
  return Object.keys(deps)
    .filter(name => name.startsWith('@pharos-ds/'))
    .map(name => {
      const short = name.slice('@pharos-ds/'.length);
    ...
    .filter(({dir}) => {
      ...
      return manifest.private !== true && manifest.pharos?.canaryOnly !== true;
    });
}
 ...
 *   <cache>/packages/core/…              — @pharos-ds/core tarball
 *   <cache>/packages/themes/<name>/…     — @pharos-ds/theme-<name> tarballs
 */
function materializeFromNpm(version, packages) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-pin-'));
  try {
...
export function resolveContentRoot() {
  ...
  return {
    ...
    // @pharos-ds/core from node_modules — i.e. the version the docsite
