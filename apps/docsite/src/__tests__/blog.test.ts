 ...
 *
 * Run: yarn workspace @pharos-ds/docsite test
 */
...
describe('validatePostFrontmatter', () => {
  ...
  it('accepts an optional article dek', () => {
    const meta = validatePostFrontmatter('slug', {
      ...VALID_FM,
      dek: '[Launch](/blog/introducing-pharos). One month in.',
    });
    expect(meta.dek).toBe('[Launch](/blog/introducing-pharos). One month in.');
  });
...
describe('discoverPosts (temp fixtures)', () => {
  ...
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-blog-'));
  });
...
describe('blog content', () => {
  ...
  it('ships the Pharos launch announcement', () => {
    const posts = discoverPosts(POSTS_DIR);
    const post = posts.find(p => p.slug === 'introducing-pharos');
