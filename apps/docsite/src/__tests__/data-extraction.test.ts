 ...
 * Run: pnpm -F @pharos-ds/docsite test
 */
...
describe('packageRegistry', () => {
  it('discovers installed packages (private and uninstalled excluded)', () => {
    ...
    expect(names).toContain('@pharos-ds/core');
    expect(names).toContain('@pharos-ds/cli');
    expect(names).toContain('@pharos-ds/theme-neutral');
    expect(names).toContain('@pharos-ds/theme-gothic');
    expect(names).toContain('@pharos-ds/theme-stone');
    expect(names).not.toContain('@pharos-ds/theme-default');
    expect(names).not.toContain('@pharos-ds/theme-brutalist');
    expect(names).not.toContain('@pharos-ds/theme-chocolate');
    expect(names).not.toContain('@pharos-ds/theme-daily');
    expect(names).not.toContain('@pharos-ds/lab');
    expect(names).not.toContain('@pharos-ds/build');
  ...
  it('CLI package has a substantial README', () => {
    const cli = packages.find(p => p.name === '@pharos-ds/cli');
...
describe('componentRegistry', () => {
  it('discovers components in @pharos-ds/core', () => {
    expect(components['@pharos-ds/core']).toBeDefined();
    expect(components['@pharos-ds/core'].length).toBeGreaterThan(100);
  });
  ...
  it('components have all required fields', () => {
    for (const [pkgName, comps] of Object.entries(components)) {
      expect(pkgName).toMatch(/^@pharos-ds\//);
  ...
  it('expands compound components into sub-component entries', () => {
    const core = components['@pharos-ds/core'];
  ...
  it('Citation satisfies its required source prop via playground defaults', () => {
    ...
    expect(citation!.playground?.defaults).toMatchObject({
      source: {title: 'Pharos Design', url: 'https://example.com'},
      number: 1,
    });
  });
  ...
  it('keeps core component docs complete for generated component pages', () => {
    ...
    const incompleteDocs = components['@pharos-ds/core'].flatMap(comp => {
  ...
  it('extracts theming data', () => {
    ...
    expect(button!.theming!.targets[0].className).toBe('pharos-button');
...
describe('themeRegistry', () => {
  ...
  it('has an import and entry for every theme package', () => {
    const themePackages = packages.filter(p =>
      p.name.startsWith('@pharos-ds/theme-'),
    );
    expect(themePackages.length).toBeGreaterThan(0);
    for (const pkg of themePackages) {
      const slug = pkg.name.replace('@pharos-ds/theme-', '');
  ...
  it('has no entries for non-theme packages', () => {
    const nonTheme = packages.filter(
      p => !p.name.startsWith('@pharos-ds/theme-'),
    );
...
describe('LinkProvider utility page', () => {
  const linkProvider = components['@pharos-ds/core'].find(
