...
describe('MCP branding', () => {
  it('route file uses Pharos (XDS) in tool descriptions', async () => {
    const fs = await import('node:fs');
    const routePath = new URL('../app/mcp/route.ts', import.meta.url);
    const source = fs.readFileSync(routePath, 'utf-8');
    expect(source).toContain('Search Pharos (XDS) design system');
    expect(source).toContain('Pharos (XDS) component');
    expect(source).toContain("name: 'pharos'");
