 ...
 *   ReferenceDocView).
 * - package: non-theme @pharos-ds packages (e.g. @pharos-ds/core, @pharos-ds/cli), rendered as
...
function getInstallSteps(pkgName: string): InstallStep[] {
  if (pkgName.endsWith('/cli')) {
    return [
      ...
      {label: 'Run a command', code: `npx @pharos-ds/cli component --list`},
    ];
  }
  return [
...
/** Sections to remove from the @pharos-ds/core README on the package page. */
const CORE_STRIP_SECTIONS = ['Quick Start', 'Resources', 'Pharos CLI'];
...
export function generateStaticParams() {
  return [
    ...
    ...packages
      ...
      .map(p => ({topic: p.name.replace('@pharos-ds/', '')})),
  ];
}
...
export async function generateMetadata({
  ...
  if (topic) {
    return pageMetadata({
      ...
      description:
        topic.description ||
        `${topic.title} documentation for the Pharos design system.`,
      path: `/docs/${slug}`,
      type: 'article',
    });
  }
  ...
  if (pkg) {
    return pageMetadata({
      ...
      description:
        pkg.description ||
        `The ${pkg.name} package for the Pharos design system.`,
      path: `/docs/${slug}`,
      type: 'article',
    });
  }
...
export default async function DocPage({
  ...
  const isComponentPkg = pkg.name === '@pharos-ds/core';
