...
import {spacingVars} from '@pharos-ds/core/theme/tokens.stylex';
...
// Count of public @pharos-ds/core components (excluding hooks and hidden entries).
// Sourced from the generated registry so the number stays accurate as the
// library grows.
const CORE_COMPONENT_COUNT = (components['@pharos-ds/core'] ?? []).filter(
...
const styles = stylex.create({
  ...
  // (0,5,0)-specificity selector (`.pharos-card-XXX:not(#\#):not(#\#)
  ...
  // from a marketing-only theme token (defined in pharosTheme.ts as
  // `--pharos-marketing-feature-card-bg`). Sourcing the color from the
...
const features: Record<string, Feature> = {
  components: {
    ...
    href: '/components',
    // Live, theme-aware sampler of real Pharos components instead of a
        ...
        {hasPreview ? (
          ...
          feature.image && (
            ...
            <HStack
              ...
              {/* Decorative image — kept as a raw <img> because @pharos-ds/core
