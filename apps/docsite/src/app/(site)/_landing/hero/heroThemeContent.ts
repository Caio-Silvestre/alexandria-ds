 ...
 * @input the docsite theme package registry (generated) + the local Pharos theme
...
import type {DefinedTheme} from '@pharos-ds/core/theme';
...
import {pharosTheme} from '@/themes/pharos';
import {BRAND_BLUE} from '@/constants';

// Sentinel for the docsite's local brand theme (not an @pharos-ds/theme-* package).
const PHAROS = 'pharos';

// Shared Pharos asset CDN. The per-theme reel cards pull the same product photos
// the /themes showcase uses (see themeShowcaseContent.ts) so the hero and the
// gallery stay in sync.
/** Stable demo images (not Meta CDN). */
const demoImg = (id: string) =>
  `https://picsum.photos/seed/pharos-${encodeURIComponent(id)}/640/640`;
...
export interface HeroThemeSlide {
  /** Theme package name, e.g. '@pharos-ds/theme-matcha'. */
  name: string;
...
const REEL_THEMES: ReadonlyArray<string> = [
  PHAROS,
  '@pharos-ds/theme-matcha',
  '@pharos-ds/theme-butter',
  '@pharos-ds/theme-gothic',
  '@pharos-ds/theme-y2k',
];

// Per-theme card content, keyed by theme name (or the PHAROS sentinel).
const CONTENT_BY_THEME: Record<string, HeroThemeContent> = {
  ...
  '@pharos-ds/theme-butter': {
    product: {
      image: demoImg('Butter-Croissant'),
      title: 'Butter croissant',
      description: 'Flaky, laminated layers baked golden each morning.',
      price: '$6',
    },
    feature: {
      image: demoImg('Butter-Waffle'),
      title: 'Belgian waffle',
      price: '$8',
    },
    mini: {
      image: demoImg('Butter-Pancake'),
      title: 'Pancakes',
  ...
  '@pharos-ds/theme-matcha': {
    product: {
      image: demoImg('matcha-product-1'),
      title: 'Matcha',
      description: 'Stone-ground ceremonial matcha over cold milk.',
      price: '$6',
    },
    feature: {
      image: demoImg('matcha-product-2'),
      title: 'Strawberry matcha',
      price: '$7',
    },
    mini: {
      image: demoImg('matcha-product-4'),
      title: 'Ube matcha',
  ...
  '@pharos-ds/theme-gothic': {
    product: {
      image: demoImg('Gothic-1'),
      title: 'Sea holly',
      description: 'A single preserved thistle stem with a steely bloom.',
      price: '$24',
    },
    feature: {
      image: demoImg('Gothic-2'),
      title: 'Garden rose',
      price: '$18',
    },
    mini: {
      image: demoImg('Gothic-3'),
      title: 'Ranunculus',
  ...
  '@pharos-ds/theme-y2k': {
    product: {
      image: demoImg('Y2K-Phone'),
      title: 'Phone',
      description: 'Iridescent clamshell with a rainbow screen.',
      price: '$18',
    },
    feature: {
      image: demoImg('Y2K-Star'),
      title: 'Glow star set',
      price: '$12',
    },
    mini: {
      image: demoImg('Y2K-Butterfly'),
      title: 'Butterfly',
...
function fallbackContent(name: string): HeroThemeContent {
  const slug = name.replace('@pharos-ds/theme-', '');
...
const LABEL_BY_THEME: Record<string, string> = {
  [PHAROS]: 'Pharos',
  '@pharos-ds/theme-neutral': 'Neutral',
  '@pharos-ds/theme-butter': 'Butter',
  '@pharos-ds/theme-matcha': 'Matcha',
  '@pharos-ds/theme-gothic': 'Gothic',
  '@pharos-ds/theme-y2k': 'Y2K',
};
...
function labelFor(name: string): string {
  ...
  const raw = pkg?.displayName ?? name.replace('@pharos-ds/theme-', '');
  return raw.replace(/^Theme:\s*/, '').replace(/\s*Theme$/, '');
}

// Resolve a slide's theme object (Pharos is local; others from the registry).
...
// Per-theme wordmark overrides. Pharos is special: its theme repoints every
...
// uses the brand blue directly so the Pharos logo stays blue while the rest of
