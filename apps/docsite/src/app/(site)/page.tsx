...
// runtime style injection. Importing the source pharosTheme.ts re-triggers it.
...
const styles = stylex.create({
  ...
  showcaseOverlay: {
    ...
    paddingBlockStart: 'var(--pharos-marketing-section-gap)',
    // Smaller than the section gap — the footer adds its own top spacing.
    paddingBlockEnd: spacingVars['--spacing-12'],
    paddingInline: spacingVars['--spacing-6'],
    gap: 'var(--pharos-marketing-section-gap)',
  },
...
function DarkScope({isDark, children}: {isDark: boolean; children: ReactNode}) {
  return (
    <Theme theme={pharosTheme} mode={isDark ? 'dark' : 'light'}>
      {children}
    </Theme>
  );
}
  ...
  return (
    <div ref={heroScopeRef} {...stylex.props(styles.heroScope)}>
      {/* HeroReelProvider holds the shared cycling state for the cards, wordmark,
          and dots. The headline/CTAs stay in stable Pharos brand style. */}
      <HeroReelProvider>
