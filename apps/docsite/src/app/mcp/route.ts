...
const handler = createMcpHandler(
  server => {
    ...
    server.tool(
      'search',
      `Search Pharos (XDS) design system: finds components, documentation topics, and page templates.\n\n` +
        ...
        const results = top.map(entry => {
          if (entry.type === 'component') {
            ...
            return {
              ...
              import: `import {${comp.moduleName}} from '@pharos-ds/core/${comp.name}';`,
              keyProps: comp.props
                .filter(
                  p =>
                    p.required ||
    ...
    server.tool(
      'get',
      `Get full documentation for a specific Pharos (XDS) component, doc topic, or template by name.\n\n` +
            ...
            ...(groupMembers.length > 0
              ? {
                  groupMembers: groupMembers.slice(0, 6).map(m => ({
                    ...
                    import: `import {${m.moduleName}} from '@pharos-ds/core/${m.name}';`,
          ...
          keywordResolved.length > 0
            ? visibleComponents
                ...
                .map(c => ({
                  ...
                  import: `import {${c.moduleName}} from '@pharos-ds/core/${c.name}';`,
