 ...
 * @output Self-hosted loader config + TypeScript-service setup with Pharos typedefs
 ...
 *
 * Configures Monaco's TypeScript service with real Pharos type definitions loaded
 * from a pre-built JSON bundle (generated at build time), so the editor offers
 * accurate autocomplete and diagnostics for @pharos-ds/core, React, StyleX, and icons.
 *
...
/**
 * Configure Monaco's TypeScript service with real Pharos type definitions.
...
export function configureMonaco(
  ...
  fetch('/playground-types.json')
    ...
    .then((packages: Record<string, Record<string, string>>) => {
      ...
      const coreFiles = packages['@pharos-ds/core'] ?? {};
      const submoduleReexports: string[] = [];

      for (const [relPath, content] of Object.entries(coreFiles)) {
        ts.addExtraLib(
          content,
          `file:///node_modules/@pharos-ds/core/dist/${relPath}`,
        );
        ...
        if (relPath.endsWith('/index.d.ts')) {
          ...
          ts.addExtraLib(
            content,
            `file:///node_modules/@pharos-ds/core/${moduleName}/index.d.ts`,
          );
      ...
      const barrelContent = submoduleReexports
        .map(m => `export * from '@pharos-ds/core/${m}';`)
        .join('\n');
      ts.addExtraLib(
        `declare module '@pharos-ds/core' {\n${barrelContent}\n}`,
        'file:///node_modules/@pharos-ds/core/index.d.ts',
      );
