...
import {Code} from '@pharos-ds/core/Code';
import {
  VStack,
  HStack,
  Layout,
...
function TemplatePreviewHeader({
  ...
  const copyButton = (
    <HStack gap={2} vAlign="center">
      <Code>{`npx @pharos-ds/cli template ${item.slug}`}</Code>
      <Button
        variant="ghost"
        isIconOnly
        size="lg"
...
export function TemplatePreviewDialog({
  ...
  const useCommand = `npx @pharos-ds/cli template ${current.slug} ./src/app/${current.slug}`;
