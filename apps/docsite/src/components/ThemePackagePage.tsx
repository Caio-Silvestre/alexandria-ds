...
const DEFAULT_THEME_PACKAGE = '@pharos-ds/theme-pharos';

// The CLI command that copies a theme into the consumer's project as
// editable source (see `pharos theme add`). The destination defaults to
// `src/themes/<slug>/`. We invoke the scoped package (`@pharos-ds/cli`)
// rather than the bare `pharos` bin so the copy-paste command works even when
// the CLI isn't installed yet — bare `npx pharos` would resolve to an
// unrelated package on the npm registry.
function themeScaffoldCommand(slug: string): string {
  return `npx @pharos-ds/cli theme add ${slug}`;
}
...
// Strip the `@pharos-ds/theme-` prefix so the slug matches the URL form
...
function packageNameToSlug(packageName: string): string {
  return packageName.replace(/^@pharos-ds\/theme-/, '');
}
...
const styles = stylex.create({
  ...
  // miniature brand preview rather than a docsite-Pharos card with
  ...
  // theme-<slug>-picker.png banner hosted on the Pharos asset CDN
  ...
  surfaceButter: {
    ...
    backgroundImage: `url(${demoImg('theme-butter-picker')})`,
  ...
  surfaceGothic: {
    backgroundColor: 'transparent',
    backgroundImage: `url(${demoImg('theme-gothic-picker')})`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat',
  },
  surfaceY2k: {
    backgroundColor: 'transparent',
    backgroundImage: `url(${demoImg('theme-y2k-picker')})`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat',
  },
  surfaceStone: {
    backgroundColor: 'transparent',
    backgroundImage: `url(${demoImg('theme-stone-picker')})`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat',
  },
  surfaceNeutral: {
    backgroundColor: 'transparent',
    backgroundImage: `url(${demoImg('theme-neutral-picker')})`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat',
  },
  surfaceMatcha: {
    backgroundColor: 'transparent',
    backgroundImage: `url(${demoImg('theme-matcha-picker')})`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundRepeat: 'no-repeat',
  },
  ...
  themeCardLabel: {
    ...
    // .pharos-text.display-3 selector in each theme's @scope'd CSS (legacy class
    ...
    <VStack gap={2} hAlign={isCentered ? 'center' : undefined}>
      ...
      <VStack gap={1} hAlign={isCentered ? 'center' : undefined}>
        <Text type="body" color="secondary" justify={align}>
          Pharos comes with a default theme built in. To make it your own, copy
...
export function ThemePackagePage({packageName, theme}: ThemePackagePageProps) {
  ...
  const themePackages = useMemo(() => {
    return packages
      .filter(p => p.name.startsWith('@pharos-ds/theme-'))
      .sort((a, b) => {
  ...
  // full @pharos-ds/theme-<slug> package name (matches state), label is
              ...
              <LinkProvider component={PreviewAnchor}>
                ...
                <ThemeShowcaseStore
                  {...getThemeShowcaseContent(
                    selectedPkgName.replace('@pharos-ds/theme-', ''),
                  )}
                />
              </LinkProvider>
            </Theme>
