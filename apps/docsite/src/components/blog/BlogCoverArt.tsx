 ...
 *
 * Renders a muted field of tiled Pharos marks over a soft brand-hued wash, with
...
// Pharos icon mark geometry, mirrored from PharosIcon in components/logos.tsx
...
export function BlogCoverArt({seed, feature = false}: BlogCoverArtProps) {
  ...
  // home hero's Pharos aurora), each nudged within a corner-biased range so
  ...
  const markId = `pharos-cover-mark-${uid}`;
  const yellowId = `pharos-cover-yellow-${uid}`;
  const pinkId = `pharos-cover-pink-${uid}`;
