...
The Pharos docsite blog lives at `/blog` (index) and `/blog/<slug>` (post detail).
...
`introducing-pharos.md` → `/blog/introducing-pharos`.
...
description: 'How Pharos structures component APIs and docs so humans and agents can build with confidence.'
date: '2026-06-17'
type: 'guide'
authors:
  - 'your-handle'
...
| `releasePackage` | string           | Package label on the generated release cover (default `@pharos-ds/core`). Only used for release posts. |
...
version (e.g. "Pharos v0.1.3: …"), the version is rendered on a solid
Pharos-blue field with the Pharos mark as the separator between segments and
the package name beneath. Set `releasePackage` to change the package label
(defaults to `@pharos-ds/core`); set an explicit `coverImage` to opt out.
...
**[Blog Review Rubric](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Blog-Review-Rubric)**.
