...
tags:
  ...
  - 'AI'
  - 'Composition'
coverImage: '/blog/pharos-cli-build-command/cover.png'
coverAlt: 'Many copies of the Pharos astronaut cat mascot arranged in rows on a light blue background'
relatedDocs:
  - title: 'The best CLI is one you never run'
    href: '/blog/the-pharos-cli'
  - title: 'CLI'
    href: '/docs/cli'
  - title: 'Components'
    href: '/components'
...
This is the second post in the series on the Pharos CLI. The first was about how [the CLI is the docs](/blog/the-pharos-cli).

## You have probably noticed AI is very good at copying

Arguably what an LLM does best is copy. We all know it is much worse at coming up with original ideas. And we all know it has major struggles with frontend code. This is not news to anyone who has spent time with any of the major models out there. And we ran straight into it building Pharos.

Early on our templates were not good. There was too much plain StyleX. Too many divs and spans. Not enough actual components. We would ask an agent to build a page and watch it barely touch Pharos or hand back something that just looked broken. That confused us for a while. The agent had the docs. It had every prop and every example through the CLI. It had everything it could possibly need. Why was it ignoring all of it?

The answer was almost too simple. It was copying our bad examples. Garbage in, garbage out. The old saying still holds. That is why we wrote a [grading rubric for templates](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Contributing-Templates). It scores every template the same way every time. Nothing sloppy slips through and a room full of designers can hold one bar. We fixed what the agent was copying and the output followed.
...
Then it picks a single starting move. A strong page match means scaffold this one and a weaker match means treat it as a layout reference and compose the rest. All of this is fast and deterministic so the same prompt returns the same result every time. There is no network call and no model behind it. You can read [the whole thing](https://github.com/Caio-Silvestre/alexandria-ds/blob/main/packages/cli/src/api/search.mjs) yourself.

Finally every `pharos build` run ends with a short set of rules. No divs for layout and no inline styles and tokens for everything else. It is the rubric again but this time pointed at your agent instead of our designers. The agent still gets to improvise and the fences just keep it on the system while it does.

## The whole point

A big part of what we are building toward with Pharos is this. An agent should be able to produce a genuinely good frontend without much effort from you at all. You should be able to spin something up and trust it without thinking hard about how it got there.
