 ...
 * Pharos Theme
 *
 * The Pharos brand uses its blue ONLY for the logo/wordmark. Everything else
 ...
 *   2. BRAND_BLUE — the Pharos brand blue, reserved for the logo only. Not
 ...
 * categorical gray) is intentionally left at the Pharos defaults — Pharos is a
 * thin brand layer of primary + accent + body on top of the design system.
 */

import {defineTheme, type TokenValue} from '@pharos-ds/core/theme';

// Relative import (not the `@/` alias) because this theme file is also loaded
// in isolation by the `pharos theme build` CLI via jiti, which does not resolve
...
export const pharosTheme = defineTheme({
  ...
  // other token at the Pharos default.
  tokens: {
    ...
    // near-black in dark. Without this it stays the Pharos default white and
    ...
    // warm Pharos cream; dark falls back to the Pharos default body color
    ...
    // categorical gray) are intentionally left at the Pharos defaults — only the
    // primary ink is a brand decision.
    '--color-text-primary': PRIMARY,
    '--color-icon-primary': PRIMARY,

    // Pharos display headings render semibold (Pharos default is normal weight).
