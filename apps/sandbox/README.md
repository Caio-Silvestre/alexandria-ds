...
Pharos component sandbox for designer exploration and vibe testing. Deployed alongside Storybook on every PR.
...
This automatically generates `AGENTS.md` with the Pharos component index via `pharos init --features agents`. **Read `AGENTS.md` for all Pharos component documentation**: it contains CLI commands to browse components, tokens, themes, and design rules.

If `AGENTS.md` is missing, regenerate it:

```bash
npx @pharos-ds/cli init --features agents
```
...
```
https://pharos-ds.vercel.app/{commit}/sandbox/
```
...
import {VStack, Heading, Text} from '@pharos-ds/core';
...
Builds `@pharos-ds/core` once, then starts the sandbox. Edits to `packages/core/src/` require a manual rebuild (`pnpm build`). Good for working on sandbox pages themselves without touching core components.
...
Resolves `@pharos-ds/core` from TypeScript source directly via the `"source"` exports condition. Edits to component source hot-reload instantly (~200ms). **Theming and CSS layers don't work** in this mode because the CSS `@layer` wrapping only exists in the built dist output. Use for layout and behavior iteration only.
