...
const GITHUB_REPO = 'facebook/pharos';

const CHANGELOG_SOURCES = [
  {pkg: '@pharos-ds/core', file: 'packages/core/CHANGELOG.md'},
  {pkg: '@pharos-ds/cli', file: 'packages/cli/CHANGELOG.md'},
  {pkg: '@pharos-ds/theme-neutral', file: 'packages/themes/neutral/CHANGELOG.md'},
  {pkg: '@pharos-ds/theme-matcha', file: 'packages/themes/matcha/CHANGELOG.md'},
];

const DEPRECATED_SCAN_DIRS = [
  {pkg: '@pharos-ds/core', dir: 'packages/core/src'},
];

const PEER_DEP_SOURCES = [
  {pkg: '@pharos-ds/core', file: 'packages/core/package.json'},
];
...
function parseChangelog(markdown, pkg) {
  ...
  for (const rawLine of markdown.split('\n')) {
      ...
      if (inCodeBlock) {
        if (currentVersion && currentSection === 'upgrade') {
          const applyMatch = codeBlockContent.match(
            /npx pharos upgrade --apply --to [\d.]+/,
          );
          const dryMatch = codeBlockContent.match(
            /npx pharos upgrade --to [\d.]+/,
          );
    ...
    if (currentSection === 'upgrade') {
      const dryMatch = line.match(/`(npx pharos upgrade --to [\d.]+)`/);
    ...
    if (
      ...
      if (text.match(/^\s*-?\s*@pharos-ds\/\w/)) continue;
