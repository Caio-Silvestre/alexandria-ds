...
export function PreviewShell({children}: {children: React.ReactNode}) {
  ...
  if (isEmbed) {
    ...
    // ([data-pharos-theme]) → template root. Rendered inline (not in the layout
    // <head>) so it applies reliably in the embed context.
    return (
      <>
        <style>{`html,body{height:100%}body>[data-pharos-theme]{height:100%}`}</style>
        {children}
      </>
    );
  }
  ...
  useEffect(() => {
    ...
    iframe.contentWindow.postMessage(
      {type: 'pharos-theme-sync', theme: themeName, mode},
      '*',
    );
  }, [themeName, mode]);
