...
// Pharos components — used in the editor popover, export dialog, and the
// row trigger so the audit drawer's interactive surfaces stay
// consistent with the rest of Pharos instead of a hand-rolled clone of
// each one.
...
const S = {
  ...
  //
  // When the token has no Pharos default (theme-only addition), the caller
...
export function ThemeAuditDrawer({
  ...
  // of every color token the audit knows about (theme-defined + Pharos
...
function ColorRow({
  ...
  // Resolve the Pharos-default value for this token in the active mode.
    ...
    <div style={S.row}>
      ...
      <div style={S.editorCell}>
        <div
          ...
          title={
            defaultHexWithAlpha
              ? `Original (Pharos default): ${formatHexWithAlpha(defaultHexBase ?? '#000000', defaultAlpha)}`
              : 'No Pharos default — token added by theme'
          }
          aria-label={
            defaultHexWithAlpha
