...
import type {TreeListItemData} from '@pharos-ds/core/TreeList';
 ...
 *   control only (via the `pharos-tree-list-chevron` target), instead of
 ...
 *   text only (via the `pharos-tree-list-item-label` target), instead of a
 ...
 * `pharos-tree-list-guide` theme target — `display: 'none'` — when the
 * surrounding layout already conveys nesting, or when a theme supplies its own
 * connectors. No prop is needed: the theme rule lands in `@layer pharos-theme`,
...
/**
 * Recolor the guides via the `pharos-tree-list-guide` theme target instead of
 ...
 * `pharos-tree-list-guide` target regardless of variant.
 */
