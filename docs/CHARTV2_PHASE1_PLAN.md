...
```bash
pnpm -F @pharos-ds/build build      # prereq
pnpm -F @pharos-ds/core build       # prereq (lab typecheck needs core's dist)
pnpm storybook                         # http://localhost:6006  (Lab / ChartV2*)
pnpm -F @pharos-ds/lab typecheck    # ChartV2 currently typechecks clean
...
- **Continuous marks (line/area/dot) glue to plot edges + overshoot escapes.**
  ...
  There is **no clipPath** (v1 has `pharos-chart-plot`) so overshoot renders into
...
- **Approach:** add `yBaseline?: 'auto'|'zero'|'data'`, `yDomain?`, `xDomain?` to
  ...
  `clipPath` (like v1 `pharos-chart-plot`) around the marks layer. Add small domain
...
- Add colocated vitest for: `layout` (domain/baseline/empty/stacking), `_uid`
  ...
  `pnpm test`, `pnpm -F @pharos-ds/lab typecheck`, `lint`, and re-screenshot all
  forms light+dark.
