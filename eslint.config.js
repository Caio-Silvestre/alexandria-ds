...
/**
 * Pharos ESLint Configuration
 *
 * Two-tier linting philosophy:
 * - CI/Agents: Strict mode (errors) - Set PHAROS_STRICT_LINT=1 or CI=true
 * - Humans: Recommended mode (warnings) - Default for local development
 *
 * Usage:
 *   pnpm lint                    # Human mode (warnings)
 *   PHAROS_STRICT_LINT=1 pnpm lint  # Strict mode (errors)
...
const pharosEslintPlugin = /** @type {import('eslint').ESLint.Plugin} */ (
  /** @type {unknown} */ (pharosPlugin)
);
...
export default defineConfig(
  ...
  // Pharos design token enforcement - applies to core package (excluding theme files)
  {
    files: ["packages/core/src/**/*.{ts,tsx}"],
  ...
  // circular. Turn off @pharos-ds/no-hardcoded-i18n-string just for this dir.
  {
    files: ["packages/core/src/i18n/**/*.{ts,tsx}"],
    rules: {
      '@pharos-ds/no-hardcoded-i18n-string': 'off',
    },
  },
  // React bug-prevention rules - applies to core package
  // Uses @eslint-react for bugs that TypeScript alone cannot catch.
  // Children.*/cloneElement are already covered by @pharos-ds/no-react-introspection.
  {
    files: ["packages/core/src/**/*.{ts,tsx}"],
    plugins: {
  ...
  // contract (#2467) at author time via @pharos-ds/no-raw-console-cli.
  {
    files: [
      "packages/cli/api/**/*.mjs",
      "packages/cli/cli/**/*.mjs",
    ...
    rules: {
      ...
      "@pharos-ds/no-raw-console-cli": "error",
    },
  },
  ...
  // keep intentionally-unused fixtures). Must come after the CLI block above.
  {
    ...
    rules: {
      "@pharos-ds/no-raw-console-cli": "off",
