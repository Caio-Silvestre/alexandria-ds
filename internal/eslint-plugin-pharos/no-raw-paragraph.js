 ...
 * @description Disallow Pharos components from rendering a `<p>` by default.
 *
 ...
 *
 * Pharos components accept arbitrary `ReactNode` content into their slots, so
 * any component that emits a `<p>` can be handed block content and break. The
 * enforceable fix on our side: Pharos's own components never emit a `<p>` by
 ...
 * consequences.
 *
 * This rule flags two patterns in Pharos source:
 ...
 * Pharos defaulting to `<p>` on its own.
 *
 * Bad:
...
const rule = {
  meta: {
    ...
    docs: {
      description:
        'Disallow Pharos components from rendering a <p> by default — render <div> so any content composes without hydration mismatches',
      category: 'Best Practices',
      recommended: true,
    },
    messages: {
      rawParagraph:
        'Pharos components must not render a <p> by default. <p> cannot contain ' +
      ...
      defaultParagraph:
        'Element-type prop defaults to "p". Pharos components must not default to ' +
