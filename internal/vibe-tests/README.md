...
These are intentional and documented; they slightly favor baseline, making Pharos wins more credible:

- **Efficiency:** Tailwind's single-line `className` gets a lower styling-ratio than Pharos's multi-line `stylex.create` blocks, despite encoding more decisions
...
- **Pharos+Tailwind scoring:** The hybrid target counts styling decisions from both Pharos props and Tailwind classes. This may inflate its decision count relative to pure Pharos, but accurately reflects the code's actual styling surface area
