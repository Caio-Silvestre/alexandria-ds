...
> **When an agent lands in a project that just ran `npm install @pharos-ds/core` — with no one having run `pharos init` — does it ever discover the CLI on its own? And which "foolproofing" channel makes that happen _consistently_?**

This is the **chicken-and-egg problem**. The thing that reliably makes agents use the CLI is the `<!-- PHAROS:START -->` cheat sheet that `pharos init` injects into `AGENTS.md`/`CLAUDE.md`/`.cursorrules`. But that cheat sheet is installed _by the CLI_. A user who only installs the package never runs `init`, so the agent never learns the CLI exists. We want to know which passive channels close that gap.

**Note:** the current `environments/project-pharos/README.md` already contains "Use the CLI to look up component props." That makes it unfit as a control for _this_ experiment — it pre-seeds the answer. Here, the README hint is one of the **conditions under test**, not a constant.
...
| H2  | README top callout           | Moderate lift; depends on whether the agent opens `node_modules/@pharos-ds/core/README.md`          | Sometimes       |
...
| H5  | CLI first-run nudge          | Converts _awareness → init_, but only fires once the agent already runs `pharos`                       | Conditional     |
...
2. **Invocation** — did it actually run any `npx pharos …`? (**ground truth**, see §6.2)
3. **Init** — did it run `pharos init`?
4. **Doc retrieval** — did it run `pharos component` / `build` / `search` / `template`?
...
| `c1-readme`       | Core `README.md` gains a top "Using an AI agent? run `npx pharos init`" callout | Candidate           |
| `c2-types`        | Generated `@see npx pharos …` banner injected into each component's `.d.ts`     | Candidate           |
| `c3-postinstall`  | Simulated install-time banner (see §9 realism caveat)                           | Candidate (probe)   |
| `c4-firstrun`     | CLI present; any `pharos` call prints the "run init" nudge                      | Candidate           |
| `c5-agents-md`    | Fully initialized: `AGENTS.md` with the real `<!-- PHAROS:START -->` block      | **Ceiling**         |
...
- `patch:agents-md` — run the real `npx pharos init --features agents --agent-docs-path AGENTS.md` (reuses `installAgentsDocs()` from `interactive.ts`).
- `patch:first-run-nudge` — swap the `pharos` bin shim for one that also prints the nudge.
...
The existing symlink `node_modules/.bin/pharos → packages/cli/bin/pharos.mjs` becomes a **logging shim**: a tiny wrapper that appends `{ts, argv}` to `projects/<promptId>/.pharos-invocations.log`, then execs the real bin. This gives an **objective** record of _every_ CLI call (invocation, init, which subcommands) independent of what the agent claims. This is the single most important measurement — discovery is defined by this log, not by the transcript.
...
```json
{
  ...
  "discovery": {
    ...
    "ranInit": false,
    "cliCommandsRun": ["pharos component Button", "pharos build \"...\""]
  }
}
```
...
# Evaluate
pnpm -F @pharos-ds/vibe-tests aggregate --iteration <id>        # reuse: quality dims
...
- **Discovery rate** = P(ran any `npx pharos`) with a Wilson 95% CI over all runs.
...
- `setup-discovery.mjs` — builds one isolated project per `condition × prompt × rep` from a bare "just installed `@pharos-ds/core`" base (core + theme symlinked, **CLI not listed**), wires the ground-truth `pharos` logging shim into `node_modules/.bin`, and writes task files + `discovery-config-<expId>.json`. Pilot wires `c0-bare` (floor) and `c5-agents-md` (ceiling); other conditions are guarded until added to `PATCHES`.
...
Verified: `c0` has no `AGENTS.md`; `c5` receives the real `<!-- PHAROS:START -->` cheat sheet; the shim records `{ts, argv}` on every `pharos` call; the task prompt carries **no** `expectedComponents` (no answer leak, §3).
...
Known note: inside this monorepo the generated `AGENTS.md` prints `pnpm exec pharos` (run-prefix detection sees pnpm); a standalone consumer sees `npx`. The shim resolves both, so it doesn't affect measurement.
...
Self-reports contradict it: both `c0` agents ran the CLI heavily (**9** and **35** `pharos` commands) with `howDiscovered: "nudge"`/`"readme"`. They invoked the **repo-root** CLI (`node packages/cli/bin/pharos.mjs`, per this repo's own always-applied `CLAUDE.md`), which **bypasses the project shim** — so `c0`'s ground-truth log stayed empty despite heavy use.

**Two contamination vectors, both from running the experiment _inside the repo_:**

1. **Injected workspace rules** — the repo's always-applied `CLAUDE.md` (the PHAROS-CLI bootstrap block) teaches the CLI to every in-session sub-agent. `c0` fwc-1 literally reported `howDiscovered: "nudge"` = injected rules.
2. **Repo-root binary + source proximity** — agents reach the workspace `pharos` and the full design-system source by walking up out of the project, bypassing the isolated sandbox.
...
- **The shim must be the _only_ reachable `pharos`** for the number to mean anything.
...
- Spawn a **headless agent with NO pharos workspace rules** (Cursor SDK / `cursor-agent`), not in-session Task sub-agents.
- Ensure **no repo-root `pharos` on `PATH`** — only the project shim (or `npx` from the registry) resolves, so every call is captured and attributable to a project channel.
...
Funnel (ground truth = pharos shim log):

| condition                | invoked pharos | ran init | core docs.mjs* |
| ------------------------ | -------------- | -------- | -------------- |
...
\* self-reported use of core's built-in `docs.mjs` (README channel); the pharos shim can't see it.

**Headline findings:**

1. **Isolation validated** — `c0` floor = 0 pharos calls (vs 9–35 when contaminated in-repo). The §14 fix holds.
...
3. **Nudge-alone can't bootstrap** — `c4-nudge` = 0/2, identical to floor. The first-run nudge only fires once the agent already runs `pharos`; it converts, it can't discover. Chicken-and-egg confirmed (H5).
...
| condition                | invoked pharos (n=5) | ran init | completed code |
| ------------------------ | -------------------- | -------- | -------------- |
...
**Attribution — considered and removed.** A `--via` tag + local `.pharos/attribution.jsonl` was prototyped, but it's local-only (no passive way to aggregate across the ecosystem), so it was dropped. The npm download share (~63% of core installs already pull the CLI) is the adoption signal we keep.
...
**Isolated runner** — `run-isolated.mjs` fixes the §14 contamination. Each task runs in a `/tmp` sandbox with packages **copied out of the repo** (walking up lands in `/tmp`, not the design-system source), driven by a headless `cursor-agent` whose **cwd is the sandbox** (so this repo's always-applied `CLAUDE.md` never applies), with only the shim `pharos` reachable. Verified without agents: `c0` bare, `c5` tagged `AGENTS.md`, `core`→`/tmp`, shim logs, and `cursor-agent -p` headless responds.
...
Rebuilt as isolated `/tmp` sandboxes driven by headless `cursor-agent` (cwd=`/tmp` → the repo `CLAUDE.md` never loads; **verified** a `/tmp` agent reports `NONE` for rules and `pharos`/`npx pharos` both not found). Clean run — but no separation:
...
**Why they tied:** the agent reported `howDiscovered: "readme"` — the **`@pharos-ds/core` README already ships a CLI section** (`## XDS CLI`, `## Page Layouts`, `npx pharos …`). So a "bare" install already advertises the CLI, and a capable agent finds + uses it unprompted (fwc-1: 3 calls; ps-1: 21 calls). The `.bin` shim was a second, artifactual hint.
...
The PATH proxy (run 2) was flaky (a README agent's `npx pharos` hit the real registry → 401, a false negative). Switched to a reliable `node_modules/.bin` shim (npx resolves local bin first). Result (n=2/cell):
...
`c0-strip/ps-1` (stripped README, no AGENTS.md) still used the CLI 24× — transcript: _"exploring the project revealed an `pharos` CLI in `node_modules/.bin`."_ The reliable shim is discoverable, so a thorough agent finds it on hard tasks.
...
**Interpretation of the original question:** for a _capable_ agent, CLI discovery is largely **not the bottleneck** — it already finds the CLI via the README, `node_modules`, or by guessing `npx pharos`. The shippable channels (types, first-run nudge, README callout) should help most for **weaker agents/harnesses** that don't explore or read docs — which this pilot did not test.

**Caveats:** n=1–2/cell (directional only); the `.bin` shim is discoverable (mild floor inflation vs a real bare install, though a real agent guessing `npx pharos` would succeed via the public registry); only one agent and one theme; **no code-quality scoring yet.**
