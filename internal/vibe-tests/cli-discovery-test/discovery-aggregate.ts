 ...
 *
 * Funnel is defined by GROUND TRUTH — the .pharos-invocations.log the bin shim
 ...
 * discovery path that ships in `@pharos-ds/core` today (PLAN.md §14/§15).
 *
  ...
  if (iso) {
    for (const t of config.tasks as Array<{
      ...
      arr.push({
        ...
        category: '',
        projectDir: t.dir,
        invocationLog: path.join(t.dir, '.pharos-invocations.log'),
      });
...
function main() {
  ...
  console.log(`Funnel (ground truth: pharos shim log), Wilson 95% CI:\n`);
  console.log(
    `${'condition'.padEnd(15)} ${'invoked pharos'.padEnd(20)} ${'ran init'.padEnd(18)} ${'doc retrieval'.padEnd(20)} ${'core docs.mjs*'.padEnd(18)} completed`,
  );
  console.log('─'.repeat(100));

  for (const [conditionId, tasks] of conditionTasks) {
    const runs = tasks.map(evaluateRun);
    const invoked = rate(runs, r => r.invokedPharos);
  ...
  console.log(
    `\n* core docs.mjs = self-reported use of @pharos-ds/core's built-in docs (README channel); the pharos shim can't observe it.`,
  );
  const outPath = path.join(RESULTS_DIR, `discovery-summary-${id}.json`);
  fs.writeFileSync(outPath, JSON.stringify(summary, null, 2));
  console.log(`📄 Summary: ${outPath}`);
  console.log(
    `\nBar: a channel "works consistently" when invoked-pharos lower bound clears the control's upper bound (PLAN.md §8).\n`,
  );
}
