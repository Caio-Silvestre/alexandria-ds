 ...
 * NONE of this repo's rules apply and the only reachable `pharos` is the shim.
 *
...
const TMP_ROOT = path.join(os.tmpdir(), 'pharos-disco');
...
function ensureSharedPkgs(expId) {
  ...
  const core = path.join(dir, '@pharos-ds', 'core');
  if (!fs.existsSync(core)) {
    ...
    fs.cpSync(
      ...
      path.join(dir, '@pharos-ds', 'theme-neutral'),
...
/** Executable `pharos` shim: logs argv to the sandbox, then execs the real CLI. */
function installLoggingShim(sandboxDir, {nudge = false} = {}) {
  const binDir = path.join(sandboxDir, 'node_modules', '.bin');
  ensureDir(binDir);
  const logPath = path.join(sandboxDir, '.pharos-invocations.log');
  const nudgeBlock = nudge
    ? `try { if (!existsSync(LOG)) process.stderr.write('\\n▶ Pharos: run \`npx pharos init\` to set up your project and AI agent docs.\\n\\n'); } catch {}`
    : '';
...
function createSandbox(sandboxDir, sharedPkgs, {nudge = false} = {}) {
    ...
    JSON.stringify(
      {
        name: 'my-app',
        private: true,
        dependencies: {'@pharos-ds/core': '*', '@pharos-ds/theme-neutral': '*', react: '^19', 'react-dom': '^19'},
      },
      null,
      2,
    ),
  );
  const nm = path.join(sandboxDir, 'node_modules', '@pharos-ds');
  ensureDir(nm);
  ...
  fs.symlinkSync(path.join(sharedPkgs, '@pharos-ds', 'core'), path.join(nm, 'core'), 'dir');
  fs.symlinkSync(path.join(sharedPkgs, '@pharos-ds', 'theme-neutral'), path.join(nm, 'theme-neutral'), 'dir');
...
const PATCHES = {
  ...
  'c1-readme': sandboxDir => {
    fs.writeFileSync(
      ...
      `# my-app\n\n> **Using an AI agent?** Run \`npx pharos init\` first — it installs the component index your agent needs.\n\nThis app uses the Pharos design system (\`@pharos-ds/core\`).\n`,
    );
  },
  'c4-nudge': () => {
  ...
  for (const prompt of prompts) {
    for (let k = 1; k <= reps; k++) {
      ...
      const task = {
        ...
        invocationLog: path.join(sandboxDir, '.pharos-invocations.log'),
