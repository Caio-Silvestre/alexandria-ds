 ...
 *
 * Builds isolated, realistic "just installed @pharos-ds/core" projects and
 * measures whether an agent discovers + uses the CLI with NO one having run
 * `pharos init`. Each CONDITION = the bare base project + zero or more channel
 * patches (see conditions.json). Only the channel varies (Checker Protocol §2).
 *
 * Ground truth: every project gets an `pharos` bin SHIM at node_modules/.bin
 * that logs argv to <project>/.pharos-invocations.log before exec-ing the real
...
/**
 * Create the realistic "npm install @pharos-ds/core" base: package.json with
 * core + theme (NOT the CLI — that's the whole point), symlinked packages, and
 * the logging `pharos` shim so `npx pharos` resolves AND is recorded.
 */
function createBaseProject(projectDir) {
  ensureDir(projectDir);

  // A freshly-installed consumer: core + theme, no @pharos-ds/cli listed.
  const pkg = {
    name: 'my-app',
    private: true,
    dependencies: {
...
/**
 * Write an executable `pharos` shim into node_modules/.bin that appends each
 * invocation to <project>/.pharos-invocations.log, then execs the real CLI.
...
Metadata JSON shape:
{
  ...
  "discovery": {
    ...
    "ranInit": <true|false>,
    "cliCommandsRun": [<exact commands, e.g. "pharos component Button">]
  }
}
