...
This is the per-tick playbook for **automatically evolving the pharos-init prompt** until
...
- Path: `"$TMPDIR/pharos-purity/_loop/iterations-log.json"` (outside the repo, like all
...
- Based on the last run's per-run detail in `purity-summary.json` (which markers survived
  ...
  - mandating `pharos component`/`search` verification vs not
...
- Read `"$TMPDIR/pharos-purity/$EXP/purity-summary.json"`.
...
- All runs stay under `"$TMPDIR/pharos-purity"` (never the repo), so parallel Cursor
