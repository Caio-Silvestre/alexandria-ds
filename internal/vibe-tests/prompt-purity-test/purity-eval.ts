 ...
 *
 * This is the ONE place that defines what counts as "veering off Pharos". Tune here.
 *
 * HARD markers (drive the binary "veered" flag): unambiguous non-Pharos styling —
 *   className=, inline style={{...}}, imported .css / @apply / @import, Tailwind
 *   utility classes. In the Pharos (stylex) path these should never appear; styling
 *   is Pharos components + the `xstyle` prop + tokens.
 ...
 *   (createTheme/defineVars) is legitimate Pharos, and we don't want to punish that.
 */
...
export interface Classification {
  ...
  caught: boolean;
  /** everVeered && !finalClean — non-Pharos styling still present at the end (bad). */
...
/** 0-100 purity of a single code string (100 = pure Pharos, 0 = drowning in raw CSS/TW). */
...
export async function gradedQuality(code: string): Promise<number | null> {
  try {
    ...
    const score = mod.evaluate(code, 'pharos');
