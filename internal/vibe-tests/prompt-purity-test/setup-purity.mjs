 ...
 *
 * Builds isolated, realistic "installed @pharos-ds/core" projects whose ONLY
 * guidance is a VARIANT of the pharos-init prompt (the independent variable). Each
 * CONDITION = a fresh `pharos init` AGENTS.md + one variant self-check block spliced
 * inside the <!-- PHAROS:START/END --> markers (see conditions.json + variants/*.md).
 * Only the injected prompt varies (Checker Protocol §2).
 *
 * ISOLATION: everything is created under an OUT-OF-WORKSPACE sandbox root
 * (os.tmpdir()/pharos-purity by default, --out to override) so it never touches your
 * open repo, editor file-watchers, other Cursor agents, or `git status`. Repo
 * packages are only symlinked (read-only).
 *
 * GROUND TRUTH: every project gets an `pharos` bin SHIM at node_modules/.bin that
 * logs argv to <project>/.pharos-invocations.log before exec-ing the real CLI, so we
...
const REAL_CLI_BIN = path.join(REPO_ROOT, 'packages', 'cli', 'bin', 'pharos.mjs');
...
/** Default sandbox base — OUTSIDE the repo. Overridable with --out or $PHAROS_PURITY_OUT. */
export function sandboxBase(out) {
  return out || process.env.PHAROS_PURITY_OUT || path.join(os.tmpdir(), 'pharos-purity');
}
...
/**
 * Realistic "installed @pharos-ds/core" base. Crucially lists a StyleX *compiler*
 * plugin in devDependencies so the CLI's detectStylingSystem() returns 'stylex' and
 * the injected prompt recommends the Pharos xstyle/StyleX path (what we're testing) —
...
function createBaseProject(projectDir) {
  ...
  symlink(path.join(REPO_ROOT, 'packages', 'core'), path.join(nm, '@pharos-ds', 'core'), 'dir');
  symlink(path.join(REPO_ROOT, 'packages', 'themes', 'neutral'), path.join(nm, '@pharos-ds', 'theme-neutral'), 'dir');
  // CLI is symlinked (so `npx pharos` resolves) but NOT listed in package.json.
  symlink(path.join(REPO_ROOT, 'packages', 'cli'), path.join(nm, '@pharos-ds', 'cli'), 'dir');

  installLoggingShim(projectDir);
}

/**
 * Executable `pharos` shim in node_modules/.bin that appends each invocation to
 * <project>/.pharos-invocations.log, then execs the real CLI. Absolute paths baked in.
 */
function installLoggingShim(projectDir) {
  const binDir = path.join(projectDir, 'node_modules', '.bin');
  ensureDir(binDir);
  const logPath = path.join(projectDir, '.pharos-invocations.log');
...
`;
  const shimPath = path.join(binDir, 'pharos');
...
/**
 * Splice a variant's self-check text inside the injected PHAROS block of AGENTS.md.
...
function applyVariant(projectDir, variantId) {
  ...
  if (!fs.existsSync(agentsPath)) {
    throw new Error(`AGENTS.md missing at ${agentsPath} — did \`pharos init\` run?`);
  }
...
/** Run the real `pharos init` (direct bin, NOT the shim) to write AGENTS.md. */
...
You are building a React + TypeScript (.tsx) UI using the Pharos design system. Your
  ...
  for (const prompt of prompts) {
    for (let k = 1; k <= reps; k++) {
      ...
      const task = {
        ...
        invocationLog: path.join(projectDir, '.pharos-invocations.log'),
