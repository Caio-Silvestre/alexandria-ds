...
const PURE = `import {Card, VStack, Heading, Text, Button} from '@pharos-ds/core';
...
check('detectMarkers finds nothing hard in pure Pharos code', () => {
...
check(
  ...
  () => {
    const c = classifyTimeline([PURE], {
      transcriptText:
        'I considered using Tailwind here but Pharos has a component for it.',
    });
    assert.ok(
...
console.log(
  '\nB. setup-purity.mjs wiring (real pharos init into a temp sandbox)',
);
...
check(
  'injected prompt recommends the Pharos xstyle/StyleX path (stylex detected)',
  () => {
    assert.ok(setupOk, 'setup did not complete');
...
check('logging pharos shim is installed', () => {
  assert.ok(setupOk, 'setup did not complete');
  const shim = path.join(proj('A-control'), 'node_modules', '.bin', 'pharos');
  assert.ok(fs.existsSync(shim), 'shim missing');
  assert.ok(
    fs.readFileSync(shim, 'utf-8').includes('.pharos-invocations.log'),
