 ...
 * that renders the component. Supports Pharos, baseline, and raw HTML targets.
 *
 * Usage:
 ...
 * polyfill variables. Pharos tokens use native light-dark() which is baseline 2024:
...
/**
 * Fix missing Pharos component imports in AI-generated .tsx files.
 * Scans for Pharos* identifiers used in JSX that aren't imported,
...
function fixMissingXDSImports(filePath: string): string[] {
  ...
  const importPattern =
    /import\s*\{([^}]+)\}\s*from\s*['"]@pharos-ds\/core[^'"]*['"]/g;
  ...
  const importLine = `import {${missing.sort().join(', ')}} from '@pharos-ds/core';\n`;
  fs.writeFileSync(filePath, importLine + code);
  return missing.sort();
}

/**
 * Validate a built preview HTML for unresolved Pharos component references.
...
function createEntryFile(
  ...
  // For Pharos and Pharos+Tailwind targets, wrap in theme provider and import reset
  if (target === 'pharos' || target === 'pharos-tailwind') {
    const tailwindImport =
      target === 'pharos-tailwind'
...
import '@pharos-ds/core/reset.css';${tailwindImport}
...
function createIndexHtml(
  ...
  // Baseline and pharos-tailwind previews need Tailwind CSS for styling
  const tailwindCdn =
    target === 'baseline' || target === 'pharos-tailwind'
...
function createViteConfig(tmpDir: string, target: string): string {
  ...
  // For Pharos and Pharos+Tailwind previews, the generated component may use
  ...
  const plugins =
    target === 'pharos' || target === 'pharos-tailwind'
      ? `
    {
    ...
    stylex.vite({
      ...
      aliases: {
        '@pharos-ds/core/theme/tokens.stylex': '${path
...
function getTsconfigForTarget(target: string): string {
  switch (target) {
    ...
    case 'pharos':
      return path.join(VIBE_DIR, 'tsconfig.pharos.json');
    case 'baseline':
...
async function main() {
  ...
  for (const iterationId of iterations) {
    ...
    const target = iterManifest.config?.target ?? 'pharos';
    ...
    for (const file of files) {
      ...
      if (ok) {
        // Post-build validation: ensure no unresolved Pharos references
        if (target === 'pharos' || target === 'pharos-tailwind') {
          const unresolved = validatePreviewHtml(previewPath);
          if (unresolved.length > 0) {
            console.error(
              `  ⚠ Unresolved Pharos components in ${previewFile}: ${unresolved.join(', ')}`,
            );
          }
        }
  ...
  if (Object.keys(importFixes).length > 0) {
    ...
    console.log(
      `\n⚠ ${Object.keys(importFixes).length} prompt(s) had missing Pharos imports (auto-fixed):`,
    );
