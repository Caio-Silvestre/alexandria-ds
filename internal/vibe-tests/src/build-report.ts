 ...
 * - packages/core/dist/pharos.css (component styles)
...
function ensureCssDist(): void {
  const pharosCss = path.join(REPO_ROOT, 'packages/core/dist/pharos.css');
  ...
  if (!fs.existsSync(pharosCss)) {
    missing.push('@pharos-ds/core/dist/pharos.css');
  }
  if (!fs.existsSync(themeCss)) {
    missing.push('@pharos-ds/theme-neutral/dist/theme.css');
  }
  ...
  if (missing.length > 0) {
    ...
    // Build core package (includes pharos.css via postbuild)
    if (!fs.existsSync(pharosCss)) {
      execSync('pnpm -F @pharos-ds/core build', {
        cwd: REPO_ROOT,
        stdio: 'inherit',
      });
    }
    ...
    if (!fs.existsSync(themeCss)) {
      execSync('pnpm -F @pharos-ds/theme-neutral build', {
        cwd: REPO_ROOT,
        stdio: 'inherit',
      });
    }
...
function parseArgs(): {
  ...
  if (!iteration) {
    console.error(
      'Usage: tsx src/build-report.ts --iteration <id> [--baseline <id>] [--html <id>] [--pharos-tailwind <id>] [--with-screenshots] [--dev]',
    );
    process.exit(1);
  }

  return {iteration, baseline, html, pharosTailwind, withScreenshots, dev};
}
...
function ensureComparison(
  ...
  if (!fs.existsSync(comparisonPath)) {
    ...
    const twFlag = pharosTailwindId
      ? ` --pharos-tailwind ${pharosTailwindId}`
      : '';
    const vsLabel = [baselineId, htmlId, pharosTailwindId]
      .filter(Boolean)
      .join(' vs ');
    console.log(`Generating comparison for ${pharosId} vs ${vsLabel}...`);
    execSync(
      `tsx ${path.join(import.meta.dirname, 'universal-compare.ts')} --pharos ${pharosId} --baseline ${baselineId}${htmlFlag}${twFlag}`,
      {cwd: path.join(import.meta.dirname, '..'), stdio: 'inherit'},
    );
  }
...
function buildDataScript(opts: {
  ...
  const reportData = {
    ...
    pharosTailwindSourceCode: opts.pharosTailwindSourceCode,
...
async function main() {
  ...
  if (baseline) {
    ...
    if (pharosTailwind) {
      ensureUniversalJson(pharosTailwind);
    }
    comparison = ensureComparison(iteration, baseline, html, pharosTailwind);
  }
  ...
  let pharosTailwindSourceCode: Record<string, string> | undefined;
  if (pharosTailwind) {
    pharosTailwindSourceCode = {};
    const twCodeDir = path.join(resultsDir, pharosTailwind, 'results');
    if (fs.existsSync(twCodeDir)) {
      ...
      for (const file of fs
        ...
        .filter(f => f.endsWith('.tsx'))) {
        const promptId = path.basename(file, '.tsx');
        pharosTailwindSourceCode[promptId] = fs.readFileSync(
