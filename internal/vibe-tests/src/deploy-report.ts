...
function parseArgs(): {
  ...
  if (!iteration) {
    console.error(
      'Usage: tsx src/deploy-report.ts --iteration <id> [--baseline <id>] [--html <id>] [--pharos-tailwind <id>] [--dry-run] [--skip-previews]',
    );
    process.exit(1);
  }

  return {iteration, baseline, html, pharosTailwind, dryRun, skipPreviews};
}
...
async function main() {
  const {iteration, baseline, html, pharosTailwind, dryRun, skipPreviews} =
    parseArgs();
  ...
  if (!skipPreviews) {
    ...
    for (const id of [iteration, baseline, html, pharosTailwind].filter(
      Boolean,
    ) as string[]) {
  ...
  if (pharosTailwind) {
    buildArgs.push(`--pharos-tailwind ${pharosTailwind}`);
  }
...
<head>
  ...
  <title>Pharos Vibe Test Reports</title>
  <style>
    body {
...
<body>
  <h1>📊 Pharos Vibe Test Reports</h1>
  <p>Evaluation reports comparing Pharos, baseline, and raw HTML targets.</p>
  <ul>
