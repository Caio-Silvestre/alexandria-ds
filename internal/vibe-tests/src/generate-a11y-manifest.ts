 ...
 * @description Generates accessibility manifests from official sources for Pharos and baseline (Radix/shadcn)
 *
 * Usage:
 *   pnpm -F @pharos-ds/vibe-tests generate-a11y-manifest
 *
 * This script:
 * 1. Parses Pharos component README.md files for accessibility info
...
/**
 * Parse an Pharos component README for accessibility information
 */
function parsePharosReadme(readmePath: string): ComponentA11yInfo | null {
...
/**
 * Generate Pharos accessibility manifest from component README files
 */
function generatePharosManifest(): A11yManifest {
  ...
  console.log(`Found ${readmePaths.length} Pharos component README files`);

  for (const readmePath of readmePaths) {
    const info = parsePharosReadme(readmePath);
    if (info) {
...
async function main(): Promise<void> {
  ...
  // Generate Pharos manifest
  console.log('📦 Parsing Pharos component README files...');
  const pharosManifest = generatePharosManifest();
  const pharosJsonPath = path.join(MANIFESTS_DIR, 'pharos.json');
  const pharosMdPath = path.join(MANIFESTS_DIR, 'pharos.md');
  fs.writeFileSync(pharosJsonPath, JSON.stringify(pharosManifest, null, 2));
  fs.writeFileSync(
    pharosMdPath,
    formatManifestAsMarkdown(pharosManifest, 'Pharos'),
  );
  console.log(
    `  ✓ Found ${Object.keys(pharosManifest.components).length} components with a11y info`,
  );
  console.log(`  ✓ Written to ${pharosJsonPath}`);
  console.log(`  ✓ Markdown: ${pharosMdPath}`);
