 ...
 *
 * Usage:
 *   pnpm -F @pharos-ds/vibe-tests interactive
 *   pnpm -F @pharos-ds/vibe-tests interactive --sample 5
 */
...
1. Do NOT import any component library (no shadcn, no MUI, no Pharos, no Radix)
...
function installAgentsDocs(): void {
  ...
  // Generate fresh AGENTS.md from pharos init CLI
  try {
    execSync('npx pharos init --features agents --agent-docs-path AGENTS.md', {
      cwd: vibeTestsDir,
      stdio: 'pipe',
    });
    console.log('✓ Generated AGENTS.md from CLI (pharos init)');
  } catch (_error) {
    console.warn('⚠ Failed to generate AGENTS.md via pharos init');
  }
}
...
interface InteractiveConfig {
  ...
  target: 'pharos' | 'baseline' | 'html'; // Target design system
}
...
interface AgentTask {
  ...
  target: string; // Target design system (pharos, baseline)
}
...
Pharos documentation is auto-loaded via AGENTS.md.`;

  return `# Vibe Test Task

You are running a vibeability test for the Pharos component library.
...
1. **Generate a response** to the prompt as if you were an AI assistant helping a developer.
   Write working React/TSX code using Pharos components.
...
2. **Self-evaluate** your response against these criteria:
   ...
   - **CSS Variables**: Did you use valid Pharos design tokens?
...
function generateSubagentPrompt(
  ...
  const personaFraming: Record<string, Record<string, string>> = {
    pharos: {
      ...
      experienced: `Use Pharos components from @pharos-ds/core. `,
...
Run: pnpm -F @pharos-ds/vibe-tests aggregate --iteration ${iterationId}
...
async function main() {
  ...
  const target =
    targetIndex !== -1
      ? (args[targetIndex + 1] as 'pharos' | 'baseline' | 'html')
      : 'pharos';
  ...
  console.log(
    `Mode: ${target === 'pharos' ? 'AGENTS.md' : target === 'baseline' ? 'AGENTS.baseline.md' : 'AGENTS.html.md'} (retrieval-led)`,
  );
  console.log(
