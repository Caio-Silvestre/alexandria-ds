 ...
 *   const projectDir = createAgentProject('pharos', iterDir, 'fwc-1');
 */
 ...
 *
 * @param {'pharos' | 'pharos-tailwind' | 'baseline' | 'html'} target
...
export function createAgentProject(target, iterDir, promptId) {
  ...
  const templateMap = {
    ...
    'pharos-tailwind': 'project-pharos-tailwind',
  ...
  if (target === 'pharos' || target === 'pharos-tailwind') {
    // Symlink node_modules/@pharos-ds/core → packages/core
    const coreLink = path.join(projectDir, 'node_modules', '@pharos-ds', 'core');
    ensureDir(path.dirname(coreLink));
    fs.symlinkSync(path.join(REPO_ROOT, 'packages', 'core'), coreLink, 'dir');

    // Symlink node_modules/@pharos-ds/cli → packages/cli
    const cliLink = path.join(projectDir, 'node_modules', '@pharos-ds', 'cli');
    fs.symlinkSync(path.join(REPO_ROOT, 'packages', 'cli'), cliLink, 'dir');

    // Symlink node_modules/@pharos-ds/theme-neutral → packages/themes/neutral
    const themeNeutralLink = path.join(projectDir, 'node_modules', '@pharos-ds', 'theme-neutral');
    fs.symlinkSync(path.join(REPO_ROOT, 'packages', 'themes', 'neutral'), themeNeutralLink, 'dir');

    // Symlink node_modules/.bin/pharos → cli bin so npx pharos works
    const binDir = path.join(projectDir, 'node_modules', '.bin');
    ensureDir(binDir);
    fs.symlinkSync(path.join(REPO_ROOT, 'packages', 'cli', 'bin', 'pharos.mjs'), path.join(binDir, 'pharos'), 'file');
  }
