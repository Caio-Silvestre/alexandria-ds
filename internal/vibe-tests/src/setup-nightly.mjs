 ...
 * (pharos, pharos-tailwind, baseline, html) using the same prompts.
 * Pharos and Pharos+Tailwind tasks include CLI-retrieval instructions.
 *
 * Usage:
...
function generatePharosTaskPrompt(prompt, projectDir) {
  return `You are generating React/TSX code using the Pharos design system.
...
function generatePharosTailwindTaskPrompt(prompt, projectDir) {
  return `You are generating React/TSX code using the Pharos design system with Tailwind CSS.
Your project is at ${projectDir}. Explore it to find available components.
...
const TARGET_CONFIG = {
  pharos: {label: 'Pharos', generateFn: generatePharosTaskPrompt, cliRetrieval: true},
  'pharos-tailwind': {label: 'Pharos+TW', generateFn: generatePharosTailwindTaskPrompt, cliRetrieval: true},
...
const TARGETS = ['pharos', 'pharos-tailwind', 'baseline', 'html'];
...
console.log(`\n  # Compare (3-way: pharos vs baseline vs html)`);
console.log(`  npx tsx src/universal-compare.ts --pharos ${iterations.pharos} --baseline ${iterations.baseline} --html ${iterations.html}`);
console.log(`\n  # Compare (pharos vs pharos+tailwind)`);
console.log(`  npx tsx src/universal-compare.ts --pharos ${iterations.pharos} --baseline ${iterations['pharos-tailwind']}`);
console.log(`\n  # Deploy report (pharos vs baseline vs html + pharos-tailwind)`);
console.log(`  npx tsx src/deploy-report.ts --iteration ${iterations.pharos} --baseline ${iterations.baseline} --html ${iterations.html} --pharos-tailwind ${iterations['pharos-tailwind']}`);
