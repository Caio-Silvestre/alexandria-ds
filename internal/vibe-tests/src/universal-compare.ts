 ...
 *
 * Usage:
 *   tsx src/universal-compare.ts --pharos abc123 --baseline def456
 *   tsx src/universal-compare.ts --pharos abc123 --baseline def456 --html ghi789
 *   tsx src/universal-compare.ts --pharos abc123 --baseline def456 --html ghi789 --pharos-tailwind jkl012
 *   tsx src/universal-compare.ts --pharos abc123 --baseline def456 --json
 *   tsx src/universal-compare.ts --pharos abc123 --baseline def456 --html ghi789 --markdown
 */
...
function winner(
  ...
  if (pharosTailwindVal != null) {
    entries.push(['pharos-tailwind', pharosTailwindVal]);
  }
...
function parseArgs(): {
  ...
  let pharosTailwind: string | undefined;
  let json = false;
  let markdown = false;

  for (let i = 0; i < args.length; i++) {
    if ((args[i] === '--pharos' || args[i] === '-x') && args[i + 1]) {
      pharos = args[++i];
    } else if ((args[i] === '--baseline' || args[i] === '-b') && args[i + 1]) {
      baseline = args[++i];
    } else if (args[i] === '--html' && args[i + 1]) {
      html = args[++i];
    } else if (args[i] === '--pharos-tailwind' && args[i + 1]) {
      pharosTailwind = args[++i];
    } else if (args[i] === '--json') {
      json = true;
  ...
  if (!pharos || !baseline) {
    console.error(
      'Usage: tsx src/universal-compare.ts --pharos <id> --baseline <id> [--html <id>] [--pharos-tailwind <id>] [--json] [--markdown]',
    );
    process.exit(1);
  }

  return {pharos, baseline, html, pharosTailwind, json, markdown};
}
...
function toMarkdown(opts: {
  ...
  baselineId: string;
  htmlId?: string;
  pharosTailwindId?: string;
  byPrompt: UniversalComparison['byPrompt'];
}): string {
  const {comparison, pharosId, baselineId, htmlId, pharosTailwindId, byPrompt} =
    opts;
  const {
    pharos,
    baseline,
    html: htmlData,
    pharosTailwind: twData,
    winners,
  } = comparison;
  ...
  const pharosRow = dimOrder.map(d => pharos.averages[d]).join(' | ');
  lines.push(
    `| **Pharos** | \`${pharosId}\` | ${pharos.overall} | ${pharosRow} |`,
  );
  ...
  if (twData) {
    ...
    lines.push(
      `| **Pharos+TW** | \`${pharosTailwindId}\` | ${twData.overall} | ${twRow} |`,
    );
  }
  ...
  if (promptEntries.length > 0) {
    ...
    for (const [, data] of promptEntries) {
      if (data.winner === 'pharos') {
        xWins++;
      } else if (data.winner === 'baseline') {
        bWins++;
      } else if (data.winner === 'html') {
        hWins++;
      } else if (data.winner === 'pharos-tailwind') {
        twWins++;
      } else {
        ties++;
      }
    }
    const parts = [`Pharos ${xWins}`, `Baseline ${bWins}`];
    if (htmlData) {
      parts.push(`HTML ${hWins}`);
    }
    if (twData) {
      parts.push(`Pharos+TW ${twWins}`);
    }
    parts.push(`Tie ${ties}`);
    lines.push(
  ...
  if (twData) {
    dmParts.push(`Pharos+TW ${twData.darkModeRate}%`);
  }
...
async function main() {
  const {
    ...
    baseline: baselineId,
    html: htmlId,
    pharosTailwind: pharosTailwindId,
    json,
    markdown,
  } = parseArgs();

  const pharos = loadOrGenerate(pharosId);
  const baseline = loadOrGenerate(baselineId);
  const htmlData = htmlId ? loadOrGenerate(htmlId) : undefined;
  const twData = pharosTailwindId
    ? loadOrGenerate(pharosTailwindId)
    : undefined;
  ...
  for (const promptId of allPromptIds) {
    const pharosScore = pharos.byPrompt[promptId];
    ...
    if (pharosScore && baselineScore) {
      byPrompt[promptId] = {
        pharos: pharosScore,
        baseline: baselineScore,
        ...(htmlScore ? {html: htmlScore} : {}),
        ...(twScore ? {pharosTailwind: twScore} : {}),
        winner: winner(
          getAverageScore(pharosScore),
  ...
  const comparison: UniversalComparison = {
    ...
    ...(twData ? {pharosTailwind: twData} : {}),
    winners,
    byPrompt,
  };

  // Save — include all IDs in filename for uniqueness
  const idParts = [pharosId, baselineId];
  if (htmlId) {
    idParts.push(htmlId);
  }
  if (pharosTailwindId) {
    idParts.push(pharosTailwindId);
  }
  ...
  // --- Print report ---
  const targetNames = ['Pharos', 'Baseline'];
  if (isThreeWay) {
    targetNames.push('HTML');
  }
  if (isFourWay) {
    targetNames.push('Pharos+TW');
  }
  ...
  type TargetEntry = {label: string; data: typeof pharos};
  const targets: TargetEntry[] = [
    {label: 'Pharos', data: pharos},
  ...
  if (isFourWay && twData != null) {
    targets.push({label: 'Pharos+TW', data: twData});
  }
