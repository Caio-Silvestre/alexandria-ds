...
function analyzeAccessibility(code: string): DimensionScore {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Form inputs without labels (skip Pharos inputs with built-in labels)
...
function countStylingDecisions(code: string, target: string): number {
  ...
  if (target === 'baseline' || target === 'pharos-tailwind') {
 ...
 * @param target - The target system ('pharos' | 'pharos-tailwind' | 'baseline' | 'html')
