 ...
 *   the library atom prefix defaults to `pharos` and is configurable
...
const pharosBabelPlugin = require('./babel.js');
...
function transformLibraryFile(libraryPrefix) {
  const result = babel.transformSync(SOURCE, {
    ...
    filename: 'node_modules/@pharos-ds/core/src/Box/XDSBox.tsx',
    babelrc: false,
    configFile: false,
    plugins: [
      [
...
function atomicClasses(code) {
  // StyleX emits atoms like "xds1a2b3c" / "pharos1a2b3c" in the compiled output.
  return code.match(/\b(?:xds|pharos|lib)[a-z0-9]{4,}\b/g) ?? [];
}

describe('xds babel wrapper -- library StyleX prefix', () => {
  it('defaults library atoms to the `pharos` prefix', () => {
    const code = transformLibraryFile(undefined);
    const atoms = atomicClasses(code);
    expect(atoms.length).toBeGreaterThan(0);
    expect(atoms.every(c => c.startsWith('pharos'))).toBe(true);
    expect(
      atoms.some(c => c.startsWith('xds') && !c.startsWith('pharos')),
    ).toBe(false);
  });
