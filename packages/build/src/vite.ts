...
const LIBRARY_PATTERN = 'node_modules/@pharos-ds/';
 ...
 * Prefer the zero-config form: pharosStylex()
 */
export interface PharosVitePluginLegacyOptions {
  stylexOptions: Parameters<typeof stylex.vite>[0];
  libraryPattern?: string;
  /** StyleX atomic class-name prefix for Pharos library styles. @default 'pharos' */
  stylexPrefix?: string;
  layers?: {
    library?: string;
    product?: string;
...
export interface PharosVitePluginOptions {
  /**
  ...
  /**
   * Pattern to identify Pharos library files vs product files.
   * @default 'node_modules/@pharos-ds/'
   */
  libraryPattern?: string;
   ...
   */
  layers?: {
    /** Layer name for Pharos library styles @default 'pharos-base' */
    library?: string;
  ...
  /**
   * StyleX atomic class-name prefix for Pharos *library* styles. The product
...
/**
 * Pharos Vite plugin for source builds.
 *
 * Provides sensible defaults for StyleX compilation with Pharos.
 ...
 * - CSS layer ordering (reset < pharos-base < pharos-theme < product)
 * - resolve.alias for @pharos-ds/core source
 * - optimizeDeps.exclude to prevent Vite pre-bundling Pharos
 *
 * @param options — optional overrides
 */
export function pharosStylex(
  options: PharosVitePluginOptions | PharosVitePluginLegacyOptions = {},
): Plugin[] {
  // Detect legacy API: pharosStylex({stylexOptions: {...}})
  if ('stylexOptions' in options && options.stylexOptions) {
    return pharosStylexLegacy(options as PharosVitePluginLegacyOptions);
  }

  const opts = options as PharosVitePluginOptions;
  const {
  ...
  const libraryLayer = layers.library ?? 'pharos-base';
  ...
  const pharosBabelPlugin = path.resolve(__dirname, 'babel.js');
  ...
  const layerOrderPlugin: Plugin = {
    ...
    transformIndexHtml() {
      return [
        {
          tag: 'style',
          children: `@layer reset, ${libraryLayer}, pharos-theme, ${productLayer};`,
  ...
  const configPlugin: Plugin = {
    name: 'pharos-config',
    config(): UserConfig {
      // Discover all @pharos-ds/* packages to exclude from pre-bundling.
      // Pharos ships as source that must be compiled by StyleX — pre-bundling
      // strips stylex.create/defineVars calls and causes runtime errors.
      let xdsPackages: string[] = ['@pharos-ds/core'];
      try {
        const fs = require('node:fs');
        const xdsDir = path.resolve(rootDir, 'node_modules/@pharos-ds');
        if (fs.existsSync(xdsDir)) {
          xdsPackages = fs
            .readdirSync(xdsDir)
            .filter((name: string) => !name.startsWith('.'))
            .map((name: string) => `@pharos-ds/${name}`);
        }
      } catch {
        // Fallback to just @pharos-ds/core if discovery fails
      }
...
/**
 * Legacy implementation — handles the old pharosStylex({stylexOptions: {...}}) API.
 * Used by Storybook and other existing configs.
 */
function pharosStylexLegacy(options: PharosVitePluginLegacyOptions): Plugin[] {
  const {
    stylexOptions,
    libraryPattern = LIBRARY_PATTERN,
    stylexPrefix = 'pharos',
