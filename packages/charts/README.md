...
Pharos charts — a config-model data visualization library built on d3.

```tsx
import {Chart, bar, line} from '@pharos-ds/charts';
...
and grid line reads, so they can never disagree. It consumes `@pharos-ds/core`
theme tokens directly (StyleX build mirrors `@pharos-ds/lab`).
...
> `ChartV2`) that used to live in `@pharos-ds/lab`; that code has moved here and
...
```tsx
import {Chart, bar, line, ChartGrid, ChartAxis} from '@pharos-ds/charts';
import '@pharos-ds/core/pharos.css';
import '@pharos-ds/charts/charts.css';
```

### Trying charts in your own project (canary)

`@pharos-ds/charts` is published **only** under the `@canary` dist-tag, so you must request that tag explicitly. There is no `latest` version to install.

```bash
npm install @pharos-ds/charts@canary @pharos-ds/core@canary
```
