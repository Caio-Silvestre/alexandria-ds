...
/**
 * A back-compat external package discovered via `pkg.pharos.docs`.
...
/**
 * Locate `@pharos-ds/core`, throwing the stable not-found error when absent.
 * @param {string} cwd
 * @returns {string}
 */
export function requireCoreDir(cwd) {
  ...
  if (!coreDir) {
    throw new PharosError('Could not find @pharos-ds/core package', undefined, ERROR_CODES.ERR_CORE_NOT_FOUND);
  }
  return coreDir;
}
...
export function classifyScope(packageScope, {owners, loadedIntegrations, cwd, name}) {
  ...
  if (packageScope === CORE_PACKAGE) {
    ...
    if (!owner) {
      throw new PharosError(`No component "${name}" in package "${packageScope}"`, undefined, ERROR_CODES.ERR_UNKNOWN_COMPONENT);
    }
  ...
  // Legacy fallback: node_modules `pkg.pharos.docs` external package.
  const ext = resolveExternalPackage(packageScope, cwd);
  if (!ext) {
    throw new PharosError(`External package "${packageScope}" not found`, undefined, ERROR_CODES.ERR_UNKNOWN_PACKAGE);
  }
 ...
 * `pkg.pharos.docs` externals are intentionally NOT part of this ambiguity set.
...
export async function resolveUnscopedDoc(dirName, {coreDir, cwd, name}) {
  ...
  if (!readmePath) {
    ...
    if (results.length > 0) {
      ...
      throw new PharosError(`No component named "${name}"`, undefined, ERROR_CODES.ERR_UNKNOWN_COMPONENT);
    }
  }

  if (!readmePath || !readmePath.endsWith('.doc.mjs')) {
    throw new PharosError(`No .doc.mjs found for "${resolvedName}". The component needs a typed doc file.`, undefined, ERROR_CODES.ERR_NO_DOC);
  }
