...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-init-api-'));
});
...
describe('init() — errors', () => {
  it('throws PharosError(ERR_UNKNOWN_FEATURE) for an invalid feature', async () => {
  ...
  it('the thrown error is an PharosError with a helpful message', async () => {
    let caught;
    try {
    ...
    expect(caught).toBeInstanceOf(PharosError);
...
describe('init() — logger', () => {
  ...
  it('emits the install line + full next-steps through the injected logger', async () => {
    ...
    expect(text).toContain(getNextSteps('npx pharos')[2].slice(0, 20));
  });
});
