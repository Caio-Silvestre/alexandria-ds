...
/**
 * @file Programmatic API for `pharos validate-integration`.
 *
 * Validates exactly ONE integration package at a time and reports findings
 * using the PharosIntegrationIssue model
 ...
 *     package.json + sibling pharos.integration.{ts,mjs,js}).
...
/**
 * @typedef {import('../../types/integration').PharosIntegrationIssue} Issue
 */
...
async function validateAtPackageDir(packageDir, identity) {
  ...
  if (manifests.length === 0) {
    issues.push(
      error(
        'missing_manifest',
        `No pharos.integration.{ts,mjs,js} found next to package.json in ${packageDir}.`,
      ),
    );
    return result;
  }
 ...
 * + a single sibling pharos.integration.{ts,mjs,js}. A missing manifest yields
