...
/**
 * @file `pharos layout grammar` leaf — the agent cheatsheet, with the alias
...
/**
 * `pharos layout grammar` — the agent cheatsheet, with the alias table
...
WORKFLOW
  pharos layout check "<expr>"           validate; echoes canonical compact + outline forms
  pharos layout expand "<expr>" [path]   emit validated TSX (path optional; --name <Pascal>)
...
TEMPLATE REFERENCING  ({hint} pulls in real content — this is how XLE reaches past the @pharos-ds/core shell)
  C{card-callout}              splice a template block (pharos template --list --type block):
  ...
  app components               register local ones in pharos.config.mjs to import them by name:
