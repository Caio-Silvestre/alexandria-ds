...
async function gatherHooks(coreDir) {
  ...
  for (const hookName of names) {
    ...
    let keywords = [];
    let description = '';
    let importPath = '@pharos-ds/core/hooks';
    if (docPath) {
  ...
  switch (c.domain) {
    case 'component':
      return {
        ...base,
        import: c._import,
        command: `pharos component ${c.name}`,
      };
    case 'hook':
      return {
        ...base,
        import: c._import,
        command: `pharos hook ${c.name}`,
      };
    case 'doc':
      return {
        ...base,
        title: c._title,
        command: `pharos docs ${c.name}`,
      };
    case 'template':
      return {
        ...base,
        ...
        kind: c._kind,
        command: `pharos template ${c.name}`,
      };
    default:
      return base;
  }
...
export async function search(query, options = {}) {
  ...
  if (!query || !String(query).trim()) {
    throw new PharosError('A search query is required', [
      {name: 'pharos search button', reason: 'example'},
    ]);
  }
  ...
  if (!coreDir) {
    throw new PharosError('Could not find @pharos-ds/core package');
  }
