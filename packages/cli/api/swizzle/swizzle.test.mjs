...
describe('rewriteImports', () => {
  it('rewrites ../theme/tokens to @pharos-ds/core/theme', () => {
    const input = `import { tokens } from '../theme/tokens.stylex';`;
    const result = rewriteImports(input);
    expect(result).toBe(`import { tokens } from '@pharos-ds/core/theme';`);
  });

  it('rewrites ../utils/mergeProps to @pharos-ds/core/utils', () => {
    const input = `import { mergeProps } from '../utils/mergeProps';`;
    const result = rewriteImports(input);
    expect(result).toBe(`import { mergeProps } from '@pharos-ds/core/utils';`);
  });
  ...
  it('rewrites export from statements', () => {
    ...
    const result = rewriteImports(input);
    expect(result).toBe(`export { foo } from '@pharos-ds/core/hooks';`);
  });
  ...
  it('handles multiple imports in one file', () => {
    ...
    expect(result).toBe(
      [
        `import { tokens } from '@pharos-ds/core/theme';`,
        `import { mergeProps } from '@pharos-ds/core/utils';`,
        `import { helper } from './helper';`,
      ].join('\n'),
    );
  });
...
describe('swizzle() API', () => {
  ...
  it('unknown component → PharosError ERR_UNKNOWN_COMPONENT with suggestions', async () => {
