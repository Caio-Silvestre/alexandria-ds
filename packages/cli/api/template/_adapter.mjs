...
const CORE_PACKAGE = '@pharos-ds/core';

/**
 * Identity for a template in package-scoped views. Core (built-in) templates
 * have no `package` field; report them under @pharos-ds/core.
...
/**
 * Discover blocks from external packages that declare `pharos.blocks` in
...
async function discoverIntegrationTemplates(cwd = process.cwd()) {
  ...
  try {
    ...
    // A broken pharos.config (or an integration that fails to load) can't
    ...
    // discoverAll (the no-error variant that `pharos template` uses) and
