 ...
 *
 * These tests stand up a temp consumer project with an pharos.config and an
...
function makeConsumer() {
  const dir = fs.mkdtempSync(
    path.join(process.cwd(), '.pharos-template-it-'),
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(dir, 'pharos.config.mjs'),
...
function installWidgets(consumerDir) {
  ...
  fs.writeFileSync(
    path.join(pkgDir, 'pharos.integration.mjs'),
...
describe('integration template discovery', () => {
  ...
  it('always reports core templates under @pharos-ds/core', async () => {
    const result = await template(undefined, {list: true, cwd: tmpDir});
    const core = result.data.filter(t => t.package === '@pharos-ds/core');
  ...
  it('errors with candidates when an id is ambiguous across type/package', async () => {
    ...
    fs.writeFileSync(
      path.join(pkg2, 'pharos.integration.mjs'),
      `export default { templates: './templates' };\n`,
    );
    fs.writeFileSync(
