 ...
 * `pharos theme build`. It reads a theme file that uses defineTheme() and, via
 * @pharos-ds/core's shared generator (the SINGLE source of truth so the
 ...
 * the theme produced no CSS (nothing to build). Errors throw PharosError (with
...
// Import shared theme processing from core. `pharos theme build` MUST produce the
...
// action). A built, resolvable `@pharos-ds/core` is a hard requirement.
// A built, resolvable `@pharos-ds/core` is a hard requirement. These are
...
try {
  const coreTheme = await import('@pharos-ds/core/theme');
...
function generatedHeader(sourceFile, lang = 'js', command) {
  ...
  const body = [
    `@generated by \`pharos theme build\` — do not edit manually.`,
    `Source: ${sourceFile}`,
  ];
  if (command) {
...
/**
 * Resolve `@pharos-ds/core`'s package root relative to the CLI package. Core
 * and the CLI ship as siblings (`@pharos-ds/core`, `@pharos-ds/cli`), so
...
/**
 * Read the type declarations `@pharos-ds/core/<Component>` exposes so we can
...
/**
 * Determine whether `@pharos-ds/core/<Component>` exports an interface named
 ...
 *
 * An augmentation is only emitted when `@pharos-ds/core/<Component>` actually
...
async function generateVariantDeclarationsAsync(themeDef) {
  ...
  const sections = ['// Generated by pharos theme build', 'export {};', ''];
  ...
  for (const [component, props] of Object.entries(customValues)) {
    for (const [prop, values] of Object.entries(props)) {
      ...
      const modulePath = `@pharos-ds/core/${pascal}`;
...
// Theme @scope selector helpers. Keep the `pharos` literal in sync with
// packages/core/src/naming.ts (NAMESPACE) and generateThemeRules.ts.
// Theme scopes to data-pharos-theme; the static build path must match.
const themeScopeStart = (/** @type {string} */ name) => `[data-pharos-theme="${name}"]`;
const THEME_SCOPE_TO = `[data-pharos-theme]`;
...
`
    : '';
  return `${variantsRef}import type { DefinedTheme } from '@pharos-ds/core/theme';
...
/**
 * Known Pharos component names and their visual props.
 ...
 * PharosError (stable code) on failure. Progress is emitted through `logger`
 * (silent by default).
 *
...
export async function themeBuild(file, options = {}, {cwd = process.cwd(), logger = noopLogger} = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    throw new PharosError(`File not found: ${filePath}`, undefined, ERROR_CODES.ERR_FILE_NOT_FOUND);
  }
  ...
  try {
    ...
    const err = /** @type {Error} */ (e);
    throw new PharosError(err.message, undefined, ERROR_CODES.ERR_THEME_LOAD);
  }

  if (!themeDef.name) {
    throw new PharosError('Theme must have a name property.', undefined, ERROR_CODES.ERR_THEME_INVALID);
  }
  ...
  // `pharos theme build` and the `<Theme>` runtime MUST emit identical CSS, so
  // there is exactly one generation path: @pharos-ds/core/theme. If core could not
  ...
  if (!_defineTheme || !_generateThemeRulesSplit) {
    throw new PharosError(
      'Could not load @pharos-ds/core/theme — `pharos theme build` requires a ' +
        'built, resolvable @pharos-ds/core so it emits the same CSS as the ' +
        'runtime <Theme>. Build @pharos-ds/core first (e.g. `pnpm -F @pharos-ds/core ' +
        'build`)' +
  ...
  let resolvedTheme;
  {
    ...
    if (_generateOnMediaCSS) {
      ...
      if (onMediaCss) {
        cssParts.push(`@layer pharos-theme {\n${onMediaCss}\n}`);
      }
    }
  ...
  const buildCommand = `pharos theme build ${sourceRelative}${options.out ? ' --out ' + path.relative(cwd, path.resolve(cwd, options.out)) : ''}`;
  ...
  try {
    ...
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_WRITE_FAILED);
  }
