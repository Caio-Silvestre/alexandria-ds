...
export function detectInstalledTargetVersion(cwd) {
  for (const packageName of ['@pharos-ds/core', '@xds/core']) {
...
export function refreshAgentDocs({cwd, installedVersion, apply, logger = noopLogger}) {
  ...
  if (inspection.status === 'missing') {
    ...
    logger.warn(
      `No Pharos agent-docs block found — AI agents have no component index. Run \`${formatCliCommand('pharos init --features agents')}\` to install it.`,
    );
    return summary;
  }
  ...
  try {
    ...
    if (summary.refreshed) {
      ...
      logger.warn(
        `Agent docs look stale but couldn't be refreshed — the <!-- PHAROS:START -->/<!-- PHAROS:END --> markers may be malformed. Run \`${formatCliCommand('pharos init --features agents')}\` to reinstall the block.`,
      );
    }
  } catch {
    summary.action = 'error';
    logger.warn(
      `Could not refresh agent docs. Run \`${formatCliCommand('pharos init --features agents')}\` to update them manually.`,
    );
  }
  return summary;
}
