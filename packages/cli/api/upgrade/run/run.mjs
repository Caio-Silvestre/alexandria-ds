 ...
 * EXECUTION errors abort. Errors throw PharosError (stable code); human progress
 ...
 * terminal run receipt, or one of the status short-circuits; throws PharosError
...
export async function run(options = {}, {cwd = process.cwd(), logger = noopLogger} = {}) {
  ...
  if (!installed) {
    const msg = `Could not find installed @pharos-ds/core (or legacy @xds/core). Install the target version first, then rerun \`${getCliInvocation()} upgrade --from <old-version>\`.`;
    logger.error(msg);
    logger.outro('Aborted');
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_VERSION_DETECT);
  }
  ...
  if (!ready) {
    ...
    logger.outro('Aborted');
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_DEP_MISSING);
  }
  ...
  try {
    ...
    logger.outro('Aborted');
    throw new PharosError(configErr.message, undefined, ERROR_CODES.ERR_INVALID_ARGUMENT);
  }
  ...
  if (totalTransforms === 0 && totalOptional === 0) {
    ...
    logger.error(msg);
    logger.outro('Aborted');
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_UNKNOWN_CODEMOD);
  }
  ...
  if (postCodemodHooks.length > 0 && mergedFilesChanged > 0) {
    ...
    try {
      ...
      throw new PharosError(msg, undefined, ERROR_CODES.ERR_CODEMOD_FAILED);
    }
  }
