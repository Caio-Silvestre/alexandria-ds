...
/**
 * DRY-RUN only: the consumer's pharos.config fails strict validation, but a
...
export function statusConfigFixable({from, to, configError, configCodemods, agentDocs}, logger = noopLogger) {
  ...
  const suggestedCommand = `pharos upgrade --from ${from} ${codemodFlags} --apply`;
  const guidance =
    'Your pharos.config currently fails strict validation, but a pending ' +
