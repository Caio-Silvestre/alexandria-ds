 ...
 *
 * `upgrade(options)` runs codemods that migrate source from a previous Pharos
 ...
 * `_adapter.mjs` (the leaves project its results). Errors throw PharosError
 ...
 * list/status/run leaves. Returns the leaf's receipt; throws PharosError on
...
export async function upgrade(options = {}, {cwd = process.cwd(), logger = noopLogger} = {}) {
  ...
  if (!options.list && !options.from) {
    ...
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_INVALID_ARGUMENT);
  }
  ...
  if (!options.list && !isValidSemver(options.from)) {
    ...
    logger.error(msg);
    logger.outro('Aborted');
    throw new PharosError(msg, undefined, ERROR_CODES.ERR_INVALID_VERSION);
  }
