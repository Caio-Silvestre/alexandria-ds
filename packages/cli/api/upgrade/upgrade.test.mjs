...
/**
 * Direct API tests for `upgrade()` — the programmatic surface (`@pharos-ds/cli/api`).
 *
 ...
 * `upgrade()` in code: the typed receipt shape, thrown PharosError codes, that
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-upgrade-api-'));
...
function writeInstalledCore(dir, version) {
  const d = path.join(dir, 'node_modules', '@pharos-ds', 'core');
  fs.mkdirSync(d, {recursive: true});
  fs.writeFileSync(
    path.join(d, 'package.json'),
    JSON.stringify({name: '@pharos-ds/core', version}, null, 2),
  );
}
...
describe('upgrade() — errors throw PharosError', () => {
  ...
  it('no installed core → ERR_VERSION_DETECT (uses the cwd option to look)', async () => {
    // Empty cwd (no node_modules/@pharos-ds/core) — detection must fail here,
