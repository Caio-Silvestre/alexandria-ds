 ...
 * `pharos upgrade`, and otherwise return the definition unchanged. They do NOT
 ...
 *
 * @template {import('../types/codemod').PharosCodemodDef} T
 * @param {T} def
 * @returns {import('../types/codemod').PharosCodemod}
 */
export function createCodemod(def) {
  return /** @type {import('../types/codemod').PharosCodemod} */ ({
    ...def,
    type: 'code',
  });
}
...
/**
 * Define a codemod that targets the consumer's pharos.config.* file. Stamp-only:
 * returns the definition with `type: 'config'` injected. Validation happens at
 * the load boundary.
 *
 * @template {import('../types/codemod').PharosConfigCodemodDef} T
 * @param {T} def
 * @returns {import('../types/codemod').PharosConfigCodemod}
 */
export function createConfigCodemod(def) {
  return /** @type {import('../types/codemod').PharosConfigCodemod} */ ({
    ...def,
    type: 'config',
  });
}
