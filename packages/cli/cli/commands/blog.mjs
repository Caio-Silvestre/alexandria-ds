...
/**
 * @file blog command — read the Pharos blog from the published feed.
 *
 ...
 *
 *   pharos blog                    List posts from the feed
 *   pharos blog <slug>             Print a post as plaintext
 *   pharos blog --json             Structured list/detail envelope
 */
...
function formatList({feedUrl, posts}, run) {
  ...
  lines.push(`Read one: ${run} pharos blog <slug>`);
  return lines.join('\n');
}
...
export function registerBlog(program) {
  program
    .command('blog [slug]')
    .description('Read the Pharos blog from the published feed')
