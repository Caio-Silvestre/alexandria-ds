...
/**
 * @file Regression test for the `color-scheme` declaration `pharos theme build`
 * emits alongside `light-dark()` token output (see #3658).
 *
 * `pharos theme build` injects a `color-scheme` declaration into
 * `@layer pharos-theme` whenever the generated CSS contains `light-dark()`,
 ...
 * `@layer pharos-theme` being declared after `@layer reset` in layer order):
 *
 ...
 *
 * Building `pharos theme build` requires a compiled @pharos-ds/core (there is no in-CLI
...
// `pharos theme build` imports the compiled @pharos-ds/core/theme entry. Build core
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-build-theme-color-scheme-'));
});
afterEach(() => {
...
describe('theme build color-scheme output', () => {
  it('emits the mode-aware color-scheme rules in @layer pharos-theme for a light-dark() theme', async () => {
