...
/**
 * @file theme command — Commander wiring for `pharos theme` (build/list/add).
 *
 ...
 * PharosError → cliError. Watch mode (a human-interactive, long-running loop)
 * stays here because it re-invokes `theme build` as a child process.
 *
 * Usage:
 *   pharos theme build ./src/themes/ocean.ts
 *   pharos theme build ./src/themes/ocean.ts --out ./dist/ocean.css
 */
...
/**
 * Path to this CLI's real entry (bin/pharos.mjs), resolved from this module's
 * location (src/commands/build-theme.mjs → ../../bin/pharos.mjs). Used to
 * re-invoke `theme build` as a child process in watch mode.
 */
function resolveCliBin() {
  const commandsDir = path.dirname(fileURLToPath(import.meta.url));
  return path.resolve(commandsDir, '../../bin/pharos.mjs');
}
...
export function registerTheme(program) {
  const theme = program
    ...
    .action((/** @type {unknown} */ options, /** @type {import('commander').Command} */ cmd) => {
      ...
      // unknown subcommand (e.g. `pharos theme bogus`), Commander hands it to
      ...
      // Bare `pharos theme` — show the subcommand list. Exit 0 (help is success).
      theme.help();
    });
  ...
  theme
    ...
    .action(async (/** @type {string} */ file, /** @type {{out?: string, watch?: boolean}} */ options) => {
      ...
      try {
        ...
        const err = /** @type {import('../../api/error.mjs').PharosError} */ (e);
