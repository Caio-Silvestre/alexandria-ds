...
/**
 * @file Path-safety regression tests for `pharos theme build`.
 *
 * Covers:
...
function writeTheme(dir, name) {
  ...
  // without needing jiti / TS, and without depending on @pharos-ds/core being
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-build-theme-paths-'));
});
afterEach(() => {
