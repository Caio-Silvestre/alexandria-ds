...
/**
 * @file Regression test for prose output of `pharos theme build`.
 *
 * `pharos theme build` has a single CSS-generation path — @pharos-ds/core's generator,
 ...
 *     `@layer pharos-theme`, so component/Markdown StyleX always wins;
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-build-theme-prose-'));
});
afterEach(() => {
...
describe('theme build prose output', () => {
  it('always emits prose mappings in @layer reset', async () => {
    ...
    // NOT @layer pharos-theme, so Markdown/component StyleX always wins.
