 ...
 *
 * `pharos theme build` emits a component override as a `.pharos-<key>` rule,
 * where `<key>` is the theme's `components` key passed through verbatim
 * (generateThemeRules re-adds the `pharos-` prefix). The CLI's KNOWN_COMPONENTS
 ...
 * components render the hyphenated class (`pharos-text-input`, etc.). Authors
 * following the registry shipped dead `.pharos-textinput` rules.
 *
 ...
 * component docs, with the `pharos-` prefix stripped. This is the canonical
...
function realOverrideKeys() {
  ...
  for (const dir of fs.readdirSync(CORE_SRC, {withFileTypes: true})) {
    ...
    const re = /className:\s*'pharos-([a-z0-9-]+)'/g;
    let m;
...
describe('theme-build KNOWN_COMPONENTS registry (#4109)', () => {
  // `layer` is a layout/anchor concept with no rendered `pharos-layer`
...
describe('theme build emits a live TextInput selector (#4109)', () => {
  ...
  beforeAll(() => {
    ensureCoreBuilt();
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-4109-'));
  }, 200_000);

  it('emits .pharos-text-input (the rendered class), not the dead .pharos-textinput', async () => {
    ...
    expect(css).toContain('.pharos-text-input');
    expect(css).not.toContain('.pharos-textinput');
  });
});
