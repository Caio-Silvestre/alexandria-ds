 ...
 * `pharos theme build` (#3391 companion: #3371).
 *
 ...
 *
 * Building `pharos theme build` requires a compiled @pharos-ds/core, so this
...
let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-build-theme-variants-'));
});
afterEach(() => {
...
describe('theme build custom-variant augmentations', () => {
  it('targets the real (un-prefixed) core interface for a custom variant', async () => {
    ...
    expect(dts).toContain("declare module '@pharos-ds/core/Button'");
  ...
  it('skips props with no augmentation point (Button size, Heading type)', async () => {
    ...
    expect(dts).not.toContain("declare module '@pharos-ds/core/Heading'");
  });
