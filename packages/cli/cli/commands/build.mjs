 ...
 *
 *   pharos build                  → the PLAYBOOK (how to build a page)
 *   pharos build "<what>"         → a COMPOSITION KIT (closest page template,
 ...
 * @param {string} run - The CLI invocation prefix (e.g. `npx pharos`).
 */
function printPlaybook(run) {
  const lines = [
    '',
    'How to build a page with Pharos',
    '',
    ...
    '   - Wrap the app in <Theme theme={...}> and import core reset.css + pharos.css.',
    '',
...
export function registerBuild(program) {
  program
    ...
    .action(async (/** @type {string | undefined} */ query, /** @type {{type?: import('../../types/search').SearchDomain, limit?: string, detail?: boolean}} */ options) => {
      ...
      humanLog('');
      humanLog('SETUP — import "@pharos-ds/core/reset.css" + "pharos.css". No <div>/style for layout — use Stack/Grid + tokens.');
      humanLog('');
    });
}
