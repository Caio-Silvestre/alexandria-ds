...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-doctor-test-'));
  logCalls = [];
  exitCode = undefined;
...
afterEach(() => {
  ...
  vi.restoreAllMocks();
  delete process.env.PHAROS_THEME;
});

/** Make a minimal node_modules/@pharos-ds/core in tmpDir with the given version. */
function installCore(version = '0.0.14', peerDependencies) {
  const coreDir = path.join(tmpDir, 'node_modules', '@pharos-ds', 'core');
  fs.mkdirSync(coreDir, {recursive: true});
  const pkg = {name: '@pharos-ds/core', version};
...
describe('doctor — individual checks', () => {
  ...
  it('core-installed: FAIL when @pharos-ds/core is missing, PASS when present', () => {
  ...
  it('themes: WARN when theme installed but not wired', () => {
    installPkg('@pharos-ds/theme-neutral', '0.0.14');
    const res = checkThemes({cwd: tmpDir, configTheme: null});
    expect(res.status).toBe('warn');
    expect(res.message).toContain('@pharos-ds/theme-neutral');
  });
  ...
  it('themes: detects pnpm-style symlinked theme packages (#3530)', () => {
    installPkgPnpmStyle('@pharos-ds/theme-neutral', '0.1.2');
    const res = checkThemes({cwd: tmpDir, configTheme: 'default'});
    expect(res.status).toBe('pass');
    expect(res.message).toContain('@pharos-ds/theme-neutral');
  });

  it('config: INFO when no pharos.config.mjs', async () => {
    const res = await checkConfig({cwd: tmpDir, configPath: null});
    expect(res.status).toBe('info');
  });

  it('config: PASS when a valid pharos.config.mjs loads', async () => {
    ...
    const configPath = path.join(fixtureDir, 'pharos.config.mjs');
    try {
  ...
  it('config: FAIL when pharos.config.mjs throws on import', async () => {
    const fixtureDir = fs.mkdtempSync(
      path.join(path.dirname(fileURLToPath(import.meta.url)), '__doctor_cfg_'),
    );
    const configPath = path.join(fixtureDir, 'pharos.config.mjs');
  ...
  it('agent-docs: INFO when no docs present', () => {
    ...
    expect(res.status).toBe('info');
    expect(res.fix).toContain('pharos init');
  });
...
describe('doctor — command', () => {
  it('--json emits a doctor envelope', async () => {
    ...
    await program.parseAsync(['node', 'pharos', '--json', 'doctor']);
  ...
  it('exit code stays 0 when there are no failures', async () => {
    // Run against the monorepo (where @pharos-ds/core resolves) → no FAIL.
    const prevExit = process.exitCode;
    process.exitCode = undefined;
    const program = createProgram();
    await program.parseAsync(['node', 'pharos', 'doctor']);
    // In the repo, core resolves, so no failures → exitCode untouched.
    expect(process.exitCode).toBeUndefined();
    process.exitCode = prevExit;
  });

  it('sets exit code 1 when a check FAILs (bare dir, no @pharos-ds/core)', async () => {
