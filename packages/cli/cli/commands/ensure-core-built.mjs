...
/**
 * @file Shared test helper: build @pharos-ds/core once, race-free.
 *
 * `pharos theme build` imports the compiled @pharos-ds/core/theme entry
 ...
 * suites each ran `if (!exists) pnpm -F @pharos-ds/core build` in their own
...
const LOCK_DIR = path.join(os.tmpdir(), 'pharos-core-build.lock');
...
function buildCore() {
  execFileSync('pnpm', ['-F', '@pharos-ds/core', 'build'], {
    cwd: REPO_ROOT,
    stdio: 'pipe',
