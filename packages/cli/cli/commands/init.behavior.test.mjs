...
/**
 * @file Behavior coverage for `pharos init`.
 *
 ...
 *
 * These lock the non-interactive contract: `pharos init` never prompts, is
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-init-behavior-'));
});
...
describe('pharos init — default (no flags)', () => {
  ...
  it('is idempotent: re-running keeps a single Pharos block (no duplication)', async () => {
...
describe('pharos init --agent <preset>', () => {
...
describe('pharos init --remove-agents', () => {
  it('removes the Pharos section it previously installed', async () => {
