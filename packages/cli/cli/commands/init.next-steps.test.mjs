...
/**
 * @file Regression test for `pharos init` "Next steps" theme guidance.
 *
 ...
 * imports were never mentioned (facebook/pharos#3080).
 *
...
describe('init Next steps theme guidance', () => {
  const text = getNextSteps('npx pharos').join('\n');

  it('mentions the base CSS imports so the app is not left unstyled', () => {
    expect(text).toContain("'@pharos-ds/core/reset.css'");
    expect(text).toContain("'@pharos-ds/core/pharos.css'");
  });

  it('uses the pre-built theme path matching the runtime recommendation', () => {
    expect(text).toContain("'@pharos-ds/theme-neutral/built'");
    expect(text).toContain("'@pharos-ds/theme-neutral/theme.css'");
  });

  it('mentions building custom themes via `pharos theme build`', () => {
    expect(text).toContain('pharos theme build <file>');
  });
  ...
  it('does not steer users to the runtime style-injection import', () => {
    ...
    expect(text).not.toContain("from '@pharos-ds/theme-neutral'");
  });
});
