...
/**
 * @file Subprocess no-hang tests for `pharos init`.
 *
...
const CLI = path.resolve(__dirname, '..', '..', 'bin', 'pharos.mjs');
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-interactive-guard-'));
});
...
describe('init is non-interactive by default (no TTY needed)', () => {
  ...
  it('writes an agent-doc file containing the PHAROS cheat-sheet marker', () => {
    runCli(['init']);
    ...
    expect(fs.readFileSync(path.join(tmpDir, doc), 'utf8')).toMatch(/PHAROS:START/);
  });
