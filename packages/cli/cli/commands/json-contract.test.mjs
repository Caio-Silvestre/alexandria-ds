...
const CLI_BIN = path.resolve(__dirname, '../../bin/pharos.mjs');
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-json-contract-'));
});
...
describe('--json contract: rejects before side effects', () => {
  it('pharos init --json --features agents does not write agent docs', async () => {
  ...
  it('pharos init --json --all does not write any files', async () => {
  ...
  it('pharos theme --json (parent, no subcommand) rejects without printing help', async () => {
  ...
  it('pharos postinstall --json rejects (hidden command not on allowlist)', async () => {
...
describe('--json contract: supported commands emit valid envelopes', () => {
  it('pharos --version --json emits { type: "version", data: { version } }', () => {
  ...
  it('pharos --json (no subcommand) emits a help envelope', async () => {
  ...
  it('pharos upgrade --json --list returns the codemod list', async () => {
  ...
  it('pharos upgrade --json (already up to date) emits upgrade.status', async () => {
    // Force a no-op range: from > installed target.
    const coreDir = path.join(tmpDir, 'node_modules', '@pharos-ds', 'core');
    fs.mkdirSync(coreDir, {recursive: true});
    fs.writeFileSync(
      path.join(coreDir, 'package.json'),
      JSON.stringify({name: '@pharos-ds/core', version: '0.0.1'}, null, 2),
    );
  ...
  it('pharos discover --json (no config) includes meta.configured=false', async () => {
  ...
  it('pharos template --json emits a typed envelope', async () => {
  ...
  it('pharos docs --json emits a typed envelope', async () => {
  ...
  it('pharos doctor --json emits a doctor envelope with checks + summary', async () => {
    // Run in a bare tmp dir → @pharos-ds/core won't resolve → a FAIL → exit 1.
