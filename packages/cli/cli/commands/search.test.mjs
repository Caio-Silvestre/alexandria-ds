...
// Run against the monorepo root so @pharos-ds/core is discoverable.
...
describe('search() API — ranking', () => {
  it('ranks an exact component name match first', async () => {
    ...
    expect(top.command).toBe('pharos component Button');
  });
...
describe('search() API — cross-domain', () => {
  it('returns results from more than one domain for a broad query', async () => {
    ...
    // The doc topic should carry a follow-up `pharos docs` command.
    const docHit = data.results.find(r => r.domain === 'doc');
    expect(docHit.command.startsWith('pharos docs ')).toBe(true);
  });
...
describe('search CLI — exit codes + JSON contract', () => {
  ...
  it('shows the follow-up command hint in human output', async () => {
    const r = await runCli(['search', 'button'], REPO_ROOT);
    expect(r.stdout).toContain('pharos component Button');
