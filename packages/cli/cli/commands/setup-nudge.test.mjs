 ...
 *
 * Unit: isPharosInitialized() detects the Pharos marker across EVERY agent-doc
...
const NUDGE = /finish setup and install the Pharos agent prompt/;

let tmp;
beforeEach(() => {
  tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-setup-nudge-'));
});
afterEach(() => {
...
describe('isPharosInitialized — centralized setup check (one place)', () => {
  ...
  it('is false when a doc file exists WITHOUT the marker', () => {
    write('AGENTS.md', 'project notes, no pharos block');
    expect(isPharosInitialized(tmp)).toBe(false);
  });
});
