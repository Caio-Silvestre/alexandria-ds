...
/**
 * @file Path-traversal regression tests for `pharos swizzle --output`.
 *
...
// node_modules/@pharos-ds/core/src/Button so findCoreDir succeeds, and
...
function buildFakeRepo(tmpDir) {
  ...
  fs.writeFileSync(path.join(core, 'package.json'), '{"name":"@pharos-ds/core"}');
  return {project, outside};
}

let tmpDir;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-swizzle-paths-'));
});
afterEach(() => {
...
describe('swizzle path safety', () => {
  ...
  it('requires --overwrite in non-interactive mode when files already exist', async () => {
    const {project} = buildFakeRepo(tmpDir);
    const outDir = path.join(project, 'components', 'pharos', 'Button');
