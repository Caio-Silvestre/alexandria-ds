...
/**
 * @file Integration-routing tests for `pharos swizzle`.
 *
 ...
 * fixtures: a fake @pharos-ds/core under node_modules plus, for the
 * integration cases, a configured integration package (pharos.config.mjs +
 * pharos.integration.mjs + a `components` dir) — all under node_modules so the
 ...
 * escaping import, plus a colocated test file. Writes pharos.config.mjs at the
...
function buildIntegration(project, {issuesUrl, componentName = 'MetaAppShell'} = {}) {
  ...
  fs.writeFileSync(
    path.join(intDir, 'pharos.integration.mjs'),
  ...
  fs.writeFileSync(
    path.join(project, 'pharos.config.mjs'),
...
let project;
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-swizzle-routing-'));
...
describe('swizzle — core feedback routing via config', () => {
  it('routes core feedback to config.issuesUrl when set', async () => {
    ...
    expect(env.data.package).toBe('@pharos-ds/core');
    ...
    const out = fs.readFileSync(
      path.join(project, 'components', 'pharos', 'Button', 'Button.tsx'),
      'utf-8',
    );
    expect(out).toContain(`from '@pharos-ds/core/theme'`);
  ...
  it('falls back to the default issues URL when config has none', async () => {
    ...
    expect(env.data.feedback.issuesUrl).toBe(
      'https://github.com/Caio-Silvestre/alexandria-ds/issues/new',
    );
  });
});
...
describe('swizzle — integration-owned components', () => {
  it('copies the component dir (excluding test/doc), rewrites escaping imports to the owner package, routes feedback to the integration issuesUrl', async () => {
    ...
    const outDir = path.join(project, 'components', 'pharos', 'MetaAppShell');
...
describe('swizzle — ambiguous ownership', () => {
  it('errors when a name is owned by core + an integration and no --package is given', async () => {
    ...
    expect(pkgs).toContain('@pharos-ds/core');
  ...
  it('--package resolves an ambiguous name to core', async () => {
    ...
    const result = await runCli(
      ['--json', 'swizzle', 'Button', '--package', '@pharos-ds/core', '-f'],
      project,
    );
    expect(result.code).toBe(0);
    const env = JSON.parse(result.stdout);
...
/**
 * Build a fake @pharos-ds/core with a component that imports StyleX directly
...
describe('swizzle — StyleX build setup note (#3373)', () => {
  it('reports usesStyleX and prints a setup note for StyleX components', async () => {
    ...
    expect(humanResult.stdout).toMatch(/pharos docs styling/);
  });
