 ...
 * (package.json + pharos.config.mjs) under a REPO-LOCAL temp dir (so Vite
...
beforeEach(() => {
  ...
  // Repo-local temp dir: under cwd so Vite allows the pharos.config import.
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.pharos-upgrade-order-'));
...
function writeInstalledCore(version) {
  const dir = path.join(tmpDir, 'node_modules', '@pharos-ds', 'core');
  fs.mkdirSync(dir, {recursive: true});
  fs.writeFileSync(
    path.join(dir, 'package.json'),
    JSON.stringify({name: '@pharos-ds/core', version}),
  );
}

function writeConfig(body) {
  fs.writeFileSync(path.join(tmpDir, 'pharos.config.mjs'), body);
}
...
describe('upgrade — core codemods run before config load', () => {
  it('dry-run: legacy layout.components is reported as fixable, not an abort', async () => {
    ...
    expect(result.data.suggestedCommand).toBe(
      'pharos upgrade --from 0.1.2 --codemod migrate-layout-components-to-experimental --apply',
    );
