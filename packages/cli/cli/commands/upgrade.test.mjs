...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-upgrade-test-'));
...
function writeInstalledCore(version, packageName = '@pharos-ds/core') {
...
describe('upgrade gate (semver comparison)', () => {
  ...
  it('blocks when --from >= installed target by semver (e.g. 0.0.10 → 0.0.9)', async () => {
    ...
    const program = createProgram();
    await program.parseAsync(['node', 'pharos', 'upgrade', '--from', '0.0.10']);
...
describe('upgrade argument validation', () => {
  ...
  it('detects the installed target version from @pharos-ds/core', async () => {
    writePkg();
    writeInstalledCore('0.0.15');
    writeSourceFile();
...
describe('upgrade agent-docs refresh (#4168)', () => {
  ...
  it('refreshes a stale block with --apply, even when there are no codemods (up-to-date path)', async () => {
    ...
    expect(content).toMatch(/Pharos v0\.0\.15 ·/);
    expect(content).not.toMatch(/Pharos v0\.0\.1 ·/);
  });
  ...
  it('refreshes a stale block on the no-codemods path (--apply)', async () => {
    ...
    expect(result.data.agentDocs.refreshed).toBe(true);
    expect(fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8')).toMatch(/Pharos v0\.0\.11 ·/);
  });
  ...
  it('migrates a legacy XDS block to the current Pharos block (--apply)', async () => {
    writePkg();
    writeInstalledCore('0.0.15');
    fs.writeFileSync(
  ...
  it('surfaces agentDocs in the human output on the up-to-date path', async () => {
    ...
    const program = createProgram();
    await program.parseAsync(['node', 'pharos', 'upgrade', '--from', '0.0.15', '--apply']);
    const output = stdoutCalls.join('') + logCalls.join('\n');
