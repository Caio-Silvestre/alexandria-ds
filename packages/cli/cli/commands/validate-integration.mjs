...
/**
 * @file `pharos validate-integration` command — validate ONE integration
 * package's manifest and contributions and report findings using the
 * PharosIntegrationIssue model.
 *
 *   pharos validate-integration            validate the local package (cwd)
 *   pharos validate-integration <pkg>      validate an installed package
 *
...
const NO_MANIFEST_GUIDANCE =
  'No pharos.integration.* found next to package.json. ' +
  'To validate an installed integration: pharos validate-integration <package>';

/**
 * Register the `pharos validate-integration` command.
...
export function registerValidateIntegration(program) {
  program
    ...
    .description(
      'Validate an Pharos integration package (manifest + contributions)',
    )
    .addHelpText(
      'after',
