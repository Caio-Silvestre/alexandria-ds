...
/**
 * @file Command-level tests for `pharos validate-integration` — envelope shape
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.pharos-validate-cmd-'));
  logCalls = [];
...
describe('validate-integration — command', () => {
  it('--json emits an integration.validate envelope for a valid package', async () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'pharos.integration.mjs'),
      'export default {};\n',
    );
    process.chdir(tmpDir);

    const program = createProgram();
    await program.parseAsync(['node', 'pharos', '--json', 'validate-integration']);
  ...
  it('no-arg + no manifest prints guidance and stays exit 0', async () => {
    ...
    const program = createProgram();
    await program.parseAsync(['node', 'pharos', 'validate-integration']);

    const out = logCalls.join('\n');
    expect(out).toContain('No pharos.integration.* found next to package.json');
    expect(process.exitCode).toBeUndefined();
  });
