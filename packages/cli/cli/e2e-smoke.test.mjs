 ...
 * deterministic but deliberately bypasses `bin/pharos.mjs`: the Node-version
...
describe('e2e smoke: real binary boots + exits (happy paths)', () => {
  it('pharos --version boots and exits 0 with a semver', () => {
    const {status, stdout} = spawnCli(['--version']);
    expect(status).toBe(0);
    expect(stdout).toMatch(/\d+\.\d+\.\d+/);
  });

  it('pharos template --list runs a real command and prints to stdout (exit 0)', () => {
