...
/**
 * @file Pharos CLI — Commander program setup
 *
 ...
 * bin/pharos.mjs` per assertion. `bin/pharos.mjs` and legacy importers use the
...
import {isPharosInitialized} from '../lib/agent-docs/agent-docs.mjs';
...
/**
 * Build a fresh, fully-wired Pharos CLI program (root options, hooks, all
...
export async function createProgram() {
  ...
  program
    ...
    .action((options, cmd) => {
      ...
      if (program.opts().json) {
        ...
        // standalone via `pharos manifest --json` as `type: 'manifest'`).
        process.__xdsJsonHandled = true;
  ...
  /**
   * Enforcement layer 3 — setup nudge. If this project hasn't run `pharos init`
   * yet (no Pharos marker in any agent-doc file — see isPharosInitialized), remind
  ...
  program.hook('preAction', (thisCommand, actionCommand) => {
    try {
      ...
      if (isPharosInitialized(cwd)) return; // already set up — stay quiet
      // Same wording as the core/cli postinstall nudges. #4151's getCliInvocation()
      // renders the correct form for THIS project — scoped `npx @pharos-ds/cli`
      // one-off, or `<pm> pharos` when installed — never the bare `npx pharos` footgun.
      console.error(
        `\nNext step: run \`${getCliInvocation(cwd)} init\` to finish setup and install the Pharos agent prompt.`,
      );
    } catch {
  ...
  // scraping `--help`. `pharos manifest --json` is the dedicated surface; the bare
...
/**
 * Eager singleton program for `bin/pharos.mjs` and legacy importers. Tests
