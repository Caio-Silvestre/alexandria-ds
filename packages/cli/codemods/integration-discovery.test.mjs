...
/**
 * Build a consumer project with an pharos.config.mjs that loads a single
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.pharos-codemod-test-'));
...
describe('integration codemod discovery', () => {
  ...
  it('discovers and runs a config codemod against pharos.config.*', async () => {
    scaffold({
    ...
    expect(result.totalFilesChanged).toBe(1);
    expect(
      fs.readFileSync(path.join(tmpDir, 'pharos.config.mjs'), 'utf-8'),
