 ...
 * codemod). A config codemod runs against the consumer's pharos.config.* file
...
export async function runCodemods(
  ...
  // Config codemods target the consumer's pharos.config.* and never read
  ...
  for (const {version, transforms} of versionManifests) {
    ...
    for (const transformEntry of transforms) {
      ...
      // `(file, api)` contract and targets the consumer's pharos.config.*.
  ...
  if (skippedOptional.length > 0) {
    ...
    for (const {name, meta} of skippedOptional) {
      ...
      log.info(
        `    Run: pharos upgrade --codemod ${name} --path <dir> --apply`,
      );
    }
  }
