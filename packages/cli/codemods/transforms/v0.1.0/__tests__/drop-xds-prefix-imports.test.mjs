...
describe('drop-xds-prefix-imports', () => {
  ...
  it('aliases to Pharos<Name> when the bare name collides with a local export function', async () => {
    const input = [
    ...
    const output = await applyTransform(input);
    // Import aliased to PharosCodeBlock; local declaration untouched.
    expect(output).toContain('CodeBlock as PharosCodeBlock');
    expect(output).toContain('@xds/core/CodeBlock');
    expect(output).toContain('export function CodeBlock({code}');
    expect(output).toContain('<PharosCodeBlock code={code} />');
  ...
  it('aliases on collision with a local const/class binding', async () => {
    ...
    const output = await applyTransform(input);
    expect(output).toContain('import {Card as PharosCard}');
    expect(output).toContain('const Card = 42;');
    expect(output).toContain('<PharosCard />');
