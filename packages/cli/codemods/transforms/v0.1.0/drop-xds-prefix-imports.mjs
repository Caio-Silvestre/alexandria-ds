 ...
 * checking against @pharos-ds/core.
 *
 ...
 * then rewrites the module paths `@xds/*` -> `@pharos-ds/*`.
 *
...
export default function transformer(file, api) {
  ...
  // up front so we can alias to `Pharos<Name>` instead of producing a duplicate
  ...
  root.find(j.ImportDeclaration).forEach((/** @type {any} */ path) => {
    ...
    path.node.specifiers = (path.node.specifiers || []).map((/** @type {any} */ spec) => {
      ...
      if (bare !== importedName && existingBindings.has(bare)) {
        ...
        // declaration, so alias the import to `Pharos<Name>` and rename the
        // import's references to that alias, leaving the local binding intact.
        const alias = `Pharos${bare}`;
