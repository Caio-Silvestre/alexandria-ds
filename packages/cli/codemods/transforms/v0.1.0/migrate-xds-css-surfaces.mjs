...
/**
 * @file Codemod: migrate XDS CSS surfaces to their Pharos equivalents
 *
 * The v0.1.0 release renamed the DOM namespace emitted by @pharos-ds/core
 * from `xds-*` to `pharos-*` and dropped the dual-emit compatibility (core no
 ...
 *   `\@xds/core/xds.css` -> `\@pharos-ds/core/pharos.css` file rename and the
...
export const meta = {
  title: 'Migrate .xds-* / [data-xds-*] / @layer xds-* CSS surfaces to pharos',
  description:
    'Rewrites the documented XDS CSS surfaces to their Pharos equivalents: ' +
    ...
    '`@import` of @xds/* package stylesheets (scope rewrite, xds.css->pharos.css, ' +
    'theme-default/theme-daily->theme-neutral) all become their Pharos forms.',
  pr: '#3092',
...
const XDS_LAYER_NAMES = new Map([
  ['xds-theme', 'pharos-theme'],
  ['xds-base', 'pharos-base'],
]);
...
// from an @xds/* path to its @pharos-ds/* equivalent. Handles the two
// non-mechanical cases beyond the scope swap:
//   @xds/core/xds.css            -> @pharos-ds/core/pharos.css   (file rename)
//   @xds/theme-default/*         -> @pharos-ds/theme-neutral/*   (collapse)
//   @xds/theme-daily/*           -> @pharos-ds/theme-neutral/*   (collapse)
...
function rewriteImportSpecifier(/** @type {any} */ spec) {
  ...
  let next = spec
    .replace(/^@xds\/theme-default(\/|$)/, '@pharos-ds/theme-neutral$1')
    .replace(/^@xds\/theme-daily(\/|$)/, '@pharos-ds/theme-neutral$1');
  if (next === spec) {
    // Not a collapsed theme — plain scope swap.
    next = spec.replace(/^@xds\//, '@pharos-ds/');
  }
  // Core stylesheet file rename: xds.css -> pharos.css (only the core bundle;
  // reset.css and others keep their names).
  next = next.replace(
    /^@pharos-ds\/core\/xds\.css$/,
