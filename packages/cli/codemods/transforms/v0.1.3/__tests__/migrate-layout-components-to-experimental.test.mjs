 ...
 */
async function applyTransform(source, filePath = 'pharos.config.mjs') {
...
`;
  ...
  it('handles createConfig({ ... }) wrapper', async () => {
    const input = `import { createConfig } from '@pharos-ds/cli';
...
describe('migrate-layout-components-to-experimental — end-to-end via runConfigCodemod', () => {
  it('rewrites pharos.config.mjs on disk through the unified runner', async () => {
    ...
    const tmpDir = fs.mkdtempSync(
      path.join(process.cwd(), '.pharos-v013-codemod-test-'),
    );
    try {
