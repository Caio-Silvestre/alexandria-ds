...
/**
 * @file Codemod: migrate pharos.config `layout.components` to
 * `experimental.xle.components`.
 *
 * The published 0.1.2 CLI read XLE app-component registration from
 * `pharos.config.*` under `layout.components`. The next release relocates this
...
export const meta = {
  title: 'Migrate pharos.config layout.components to experimental.xle.components',
  description:
...
export default function transformer(file, api) {
  ...
  if (defaultExports.size() === 0) {
    throw new Error(
      `could not statically analyze pharos config (no default export); ${MANUAL}`,
    );
  }
  ...
  if (!configObject) {
    throw new Error(
      `could not statically analyze pharos config; ${MANUAL}`,
    );
  }
  ...
  if (!j.ObjectExpression.check(layoutProp.value)) {
    throw new Error(
      `pharos config \`layout\` is not an object literal; ${MANUAL}`,
    );
  }
  ...
  if (!j.ObjectExpression.check(componentsProp.value)) {
    throw new Error(
      `pharos config \`layout.components\` is not an object literal; ${MANUAL}`,
    );
  }
  ...
  if (otherLayoutKeys.length > 0) {
    throw new Error(
      `pharos config \`layout\` has keys other than \`components\`; ${MANUAL}`,
    );
  }
  ...
  if (experimentalProp) {
    if (!j.ObjectExpression.check(experimentalProp.value)) {
      throw new Error(
        `pharos config \`experimental\` is not an object literal; ${MANUAL}`,
      );
    }
    ...
    if (xleProp) {
      ...
      throw new Error(
        `pharos config already defines \`experimental.xle\`; ${MANUAL}`,
      );
    }
...
function buildMigratedComponents(/** @type {any} */ j, /** @type {any} */ oldComponents) {
  const newProps = oldComponents.properties.map((/** @type {any} */ prop) => {
    if (
      ...
      throw new Error(
        `pharos config \`layout.components\` has a non-literal entry; ${MANUAL}`,
      );
    }
    ...
    throw new Error(
      `pharos config \`layout.components\` entry is neither a string nor an ` +
