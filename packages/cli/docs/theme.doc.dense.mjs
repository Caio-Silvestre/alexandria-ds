...
export const docsDense = {
  ...
  sections: [
    ...
    { section: 'Available Themes', title: 'Themes', content: [null, null, { type: 'prose', text: 'published: neutral (start here), butter, chocolate, gothic (dark-only), matcha, stone, y2k. @pharos-ds/theme-{name} = source (runtime). @pharos-ds/theme-{name}/built = optimized (+ theme.css).' }] },
    ...
    { section: 'Component Style Overrides', title: 'Component Overrides', content: [{ type: 'prose', text: 'components field uses semantic component keys + style keys (base, variant:value, stateName), not raw selectors. for external CSS, prefer data-* selectors from `pharos docs styling`. write standard CSS (borderRadius, padding) — pipeline expands to internal vars. public vars (--button-press-scale etc) set directly. private vars (--_*) cannot be set — use CSS properties. run `pharos component <Name>` for details.' }, null, null, null, null] },
    { section: 'Custom Variants', title: 'Custom Variants', content: [{ type: 'prose', text: 'any unknown prop:value in components becomes a new variant. pharos theme build generates TS augmentations. works on any extensible prop axis (variant, status, etc).' }, null, null, null, null] },
    { section: 'Building Themes for Production', title: 'Build for Production', content: [{ type: 'prose', text: 'pharos theme build compiles defineTheme to static CSS. outputs .css + .js (__built:true) + .d.ts.' }, null, null, null, null] },
