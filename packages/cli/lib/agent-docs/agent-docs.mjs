...
const MARKER_START = '<!-- PHAROS:START -->';
const MARKER_END = '<!-- PHAROS:END -->';
 ...
 * write the Pharos block. SINGLE SOURCE OF TRUTH: discovery, removal, and the
 * `isPharosInitialized` predicate all derive from this list, so "where init
...
/**
 * Single source of truth for "is Pharos set up in this project?" — true when any
 * agent-doc file already carries the Pharos marker, i.e. `init` / `agent-docs`
 ...
 * @returns {boolean}
 */
export function isPharosInitialized(targetDir = process.cwd()) {
...
/**
 * Parse the Pharos version a managed block was generated for, from its header
 * line ("Pharos v1.2.3 · N components"). Returns null when the block predates
...
export function parseBlockVersion(content) {
  const match = /Pharos v(\d+\.\d+\.\d+[^\s·]*)/.exec(content ?? '');
  return match ? match[1] : null;
}

/**
 * Read-only staleness assessment of the managed agent-docs block(s) against the
 * installed core version. This is the detection half of the `pharos upgrade`
...
export function generateCompressedIndex(version, {coreDir, invocation = getCliInvocation(), stylingSystem = 'css'} = {}) {
  ...
  // Header — state the CLI prefix once; commands below are shown as `pharos <cmd>`.
  lines.push(`Pharos v${version} · ${componentCount} components`);
  lines.push(`CLI: run every command as \`${run} <cmd>\` (shown below as \`pharos ...\`).`);
  lines.push('');

  // Required setup — components ship precompiled CSS; without these imports
  // everything renders unstyled. Theme is optional (a default ships in pharos.css).
  lines.push('SETUP (once, in your app entry e.g. main.tsx) — without these, components render unstyled:');
  lines.push('  import "@pharos-ds/core/reset.css";');
  lines.push('  import "@pharos-ds/core/pharos.css";');
  lines.push('');

  // Workflow — `build` is the front door; discover before writing UI.
  lines.push("WORKFLOW — discover, don't guess. Before writing UI:");
  lines.push('1. `pharos build "<idea>"` — START HERE: returns a kit (closest [page] + [block]s + [component]s). No args = full playbook.');
  lines.push('2. `pharos template <name> [--skeleton]` — scaffold the [page]/[block]s it named, or study their layout. Templates are reference code.');
  lines.push('3. `pharos component <Name>` — props + examples for every component you use.');
  lines.push('');

  // Rules — the top error-preventers.
  lines.push('RULES:');
  lines.push('- No <div> — components do all layout/spacing. Full page → AppShell; sidebar nav → SideNav.');
  lines.push('- Frame first: pick the shell (AppShell / Layout+LayoutPanel) and budget regions in px BEFORE writing content (`pharos docs layout`).');
  ...
  if (stylingSystem === 'stylex') {
    lines.push('- Custom styling: component props first; else the xstyle prop / StyleX tokens (@pharos-ds/core/theme/tokens.stylex). No raw hex/px.');
  ...
  lines.push('- Tokens for every value (`pharos docs tokens`). Brand/accent via `pharos theme` — never override --color-* in :root.');
  ...
  lines.push(
    `- SELF-CHECK before you finish: re-read the file and ${selfCheckFix[stylingSystem] ?? selfCheckFix.css}. If unsure a component/prop exists, run \`pharos component <Name>\` / \`pharos search "<thing>"\`; don't hand-roll CSS.`,
  );
  lines.push('');
  ...
  lines.push('  upgrade --apply    run after any @pharos-ds/core bump');
...
/**
 * Get Pharos version from core package.
...
/**
 * Inject or update Pharos section in a file using Pharos markers.
 ...
 * @param {boolean} [options.onlyReplace] - Only write if Pharos markers already exist (skip files without markers)
...
export function injectXdsBlock(filePath, compressedIndex, {createIfMissing = false, header = '', onlyReplace = false} = {}) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (startIdx !== -1 && endIdx !== -1) {
      ...
      // File exists but has no Pharos markers — skip it
      return false;
    } else {
...
/**
 * Inject or update Pharos section in AGENTS.md.
...
/**
 * Inject or update Pharos section in CLAUDE.md.
...
/**
 * Remove Pharos section from all known agent doc files.
 * @param {string} targetDir
 */
 ...
 * @param {boolean} [options.onlyReplace] - Only update files that already have Pharos markers (for upgrades)
