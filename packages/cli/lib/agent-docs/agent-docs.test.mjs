...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-agent-docs-test-'));
});
...
describe('generateCompressedIndex', () => {
  it('includes the version number', () => {
    ...
    expect(result).toContain('Pharos v1.2.3');
    expect(result).toContain('<!-- PHAROS:START -->');
    expect(result).toContain('<!-- PHAROS:END -->');
  });

  it('includes theme nudge rule', () => {
    const result = generateCompressedIndex('1.0.0');
    expect(result).toMatch(/pharos theme/);
  ...
  it('includes upgrade command and migration rule', () => {
    ...
    expect(result).toMatch(/after any @pharos-ds\/core bump/);
  });

  it('states the invocation once in the CLI header (yarn)', () => {
    const result = generateCompressedIndex('1.0.0', {invocation: 'yarn pharos'});
    expect(result).toContain('yarn pharos <cmd>');
    expect(result).not.toContain('npx pharos');
  });

  it('uses the pnpm exec invocation', () => {
    const result = generateCompressedIndex('1.0.0', {invocation: 'pnpm exec pharos'});
    expect(result).toContain('pnpm exec pharos <cmd>');
    expect(result).not.toContain('npx pharos');
  });

  it('uses the scoped package for one-off (uninstalled) runs so agents never hit the bare name', () => {
    const result = generateCompressedIndex('1.0.0', {invocation: 'npx @pharos-ds/cli'});
    expect(result).toContain('npx @pharos-ds/cli <cmd>');
    // The header defines the mapping; the bare "run every command as `npx pharos`" footgun must be absent.
    expect(result).not.toContain('npx pharos <cmd>');
  });
});
...
describe('injectXdsBlock', () => {
  it('injects into an existing file without markers', () => {
    ...
    const result = injectXdsBlock(filePath, '<!-- PHAROS:START -->\nnew\n<!-- PHAROS:END -->');

    expect(result).toBe(true);
    const content = fs.readFileSync(filePath, 'utf-8');
    expect(content).toContain('# Existing content');
    expect(content).toContain('<!-- PHAROS:START -->');
  });
  ...
  it('replaces existing markers', () => {
    ...
    injectXdsBlock(filePath, '<!-- PHAROS:START -->\nnew\n<!-- PHAROS:END -->');
  ...
  it('skips files without markers when onlyReplace is true', () => {
    ...
    const result = injectXdsBlock(filePath, '<!-- PHAROS:START -->\nnew\n<!-- PHAROS:END -->', {onlyReplace: true});

    expect(result).toBe(false);
    const content = fs.readFileSync(filePath, 'utf-8');
    expect(content).not.toContain('<!-- PHAROS:START -->');
...
describe('injectAgentsMd', () => {
  it('creates new AGENTS.md when none exists', () => {
    ...
    expect(content).toContain('<!-- PHAROS:START -->');
    expect(content).toContain('Pharos v1.0.0');
    expect(content).toContain('<!-- PHAROS:END -->');
  });
...
`;
    ...
    const content = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    expect(content).toContain('Pharos v2.0.0');
...
describe('installAgentDocs', () => {
  function setupCorePackage(dir, version = '1.0.0') {
    // Create a minimal @pharos-ds/core so getXdsVersion works
    const coreDir = path.join(dir, 'node_modules', '@pharos-ds', 'core');
    fs.mkdirSync(coreDir, {recursive: true});
    fs.writeFileSync(
      path.join(coreDir, 'package.json'),
  ...
  it('defaults to AGENTS.md but writes .claude/CLAUDE.md only when --agent claude is explicit', () => {
    ...
    const claudeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-agent-docs-claude-'));
  ...
  it('injects into CLAUDE.md at root when it exists', () => {
    ...
    expect(fs.existsSync(path.join(tmpDir, 'AGENTS.md'))).toBe(false);
    const claudeContent = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claudeContent).toContain('<!-- PHAROS:START -->');
  ...
  it('injects into all existing agent doc files', () => {
    ...
    const claudeContent = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(agentsContent).toContain('<!-- PHAROS:START -->');
    expect(claudeContent).toContain('<!-- PHAROS:START -->');
  });
...
describe('parseBlockVersion', () => {
  ...
  it('reads prerelease versions', () => {
    expect(parseBlockVersion('Pharos v1.2.3-beta.4 · 10 components')).toBe(
      '1.2.3-beta.4',
    );
  });
...
describe('inspectAgentDocs', () => {
  ...
  it('reports missing when agent files exist but carry no Pharos marker', () => {
    fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), '# Agents\n\nHand-written notes.\n');
  ...
  it('defaults the installed version to the core package when omitted', () => {
    const coreDir = path.join(tmpDir, 'node_modules', '@pharos-ds', 'core');
    fs.mkdirSync(coreDir, {recursive: true});
