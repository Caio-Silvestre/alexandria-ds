...
/**
 * @file Component discovery — find, list, and resolve Pharos components
 */
...
export const CORE_PACKAGE = '@pharos-ds/core';
...
/**
 * Auto-discover components by scanning for Pharos*.tsx files in core/src/.
 *
...
export function discoverComponents(coreDir) {
  ...
  for (const entry of topEntries) {
    ...
    // Check both {Name}.doc.mjs and Pharos{Name}.doc.mjs naming conventions
...
export function findComponentReadme(coreDir, name) {
  ...
  // Direct match: src/{name}/{Name}.doc.mjs or src/{name}/Pharos{Name}.doc.mjs
  ...
  // Nested match: src/*/{name}/{Name}.doc.mjs or src/*/{name}/Pharos{Name}.doc.mjs
  ...
  // Per-component doc in a parent directory: src/*/{Name}.doc.mjs or src/*/Pharos{Name}.doc.mjs
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
...
/**
 * Find the main source file for a component (Pharos*.tsx, excluding tests).
 ...
 * bare `@pharos-ds/core` root instead of its tree-shakeable subpath.
 * @param {string} coreDir
 * @param {string} name
 * @returns {string | null}
 */
export function findComponentSource(coreDir, name) {
  ...
  // on-disk convention, then the bare forms (`Button.tsx`) that the Pharos-prefix
...
export function resolveImportPath(coreDir, componentName) {
  ...
  if (exactMatch) {
    return `@pharos-ds/core/${exactMatch.slice(2)}`;
  }

  const sourcePath = findComponentSource(coreDir, componentName);
  if (!sourcePath) return '@pharos-ds/core';
  ...
  if (topMatch) {
    return `@pharos-ds/core/${topMatch.slice(2)}`;
  }

  return '@pharos-ds/core';
}
 ...
 * @param {string} name bare component name (no `XDS`/`Pharos` prefix)
 * @returns {string|null}
 */
 ...
 *
 * Core records carry `package: '@pharos-ds/core'`, `issuesUrl: undefined`
