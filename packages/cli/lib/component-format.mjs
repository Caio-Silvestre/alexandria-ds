 ...
 *
 * Override keys are the component's stable class name with the `pharos-`
 * namespace prefix stripped — `generateThemeRules` re-adds the prefix when it
 * builds the `.pharos-*` selector. So `pharos-base-table` → key `base-table`
 * (→ `.pharos-base-table`), and `pharos-button` → key `button`.
 *
 * Keep the `pharos-` literal in sync with packages/core/src/naming.ts
...
function targetKey(target) {
  return target.className.replace(/^pharos-/, '');
}
 ...
 * @param {string|null} [options.importHint] - Import path hint (e.g. '@pharos-ds/core/Button')
 */
...
export function formatCompact(docs, componentName, importHint) {
  ...
  // without the Pharos prefix. componentName is already bare post-P4.
