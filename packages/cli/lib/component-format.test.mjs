...
describe('formatFull sub-component rendering', () => {
  ...
  it('does not print "undefined" for a bare {name} sub-component', () => {
    ...
    expect(out).toContain('pharos component XDSRadioListItem');
  });
  ...
  it('renders a full props table for a sub-component that has inline props', () => {
    ...
    expect(out).not.toContain('pharos component XDSButtonGroup');
  });
});

describe('formatFull theming override keys', () => {
  // Regression guard: the `defineTheme` component-override key is the stable
  // class name with the `pharos-` prefix stripped — `generateThemeRules`
  // re-adds the prefix to build the `.pharos-*` selector. targetKey() used to
  // strip a dead `xds-` prefix (from before the pharos rename), so the printed
  // example advertised `pharos-*` keys. Those double-prefix to
  // `.pharos-pharos-*` at runtime and silently match nothing. See issue #3458.
  it('prints override keys without the pharos- prefix (single-word component)', () => {
    const docs = {
      name: 'Button',
      description: 'A button.',
      theming: {targets: [{className: 'pharos-button', visualProps: ['variant']}]},
    };
    const out = formatFull(docs);

    expect(out).toContain("'button': {");
    // The full DOM class must not appear as an override key.
    expect(out).not.toContain("'pharos-button': {");
  });

  it('prints override keys without the pharos- prefix (compound Table classes)', () => {
    const docs = {
      name: 'Table',
      description: 'A table.',
      theming: {
        targets: [
          {className: 'pharos-base-table'},
          {className: 'pharos-table-cell'},
        ],
      },
    };
    const out = formatFull(docs);

    // Correct keys: class name minus the pharos- prefix.
    ...
    expect(out).not.toContain("'pharos-base-table': {");
    expect(out).not.toContain("'pharos-table-cell': {");
  });
});
