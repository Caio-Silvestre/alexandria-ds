...
/**
 * @file Integration manifest loading for Pharos config.
 *
 * Integrations are PACKAGE NAMES listed in pharos.config.{ts,mjs,js}. Each
 * package declares a single conventional root manifest sibling to its
 * package.json — pharos.integration.{ts,mjs,js} — which contributes
...
import {PharosIntegrationSchema} from './config-schema.mjs';
...
export const MANIFEST_BASENAMES = [
  'pharos.integration.ts',
  'pharos.integration.mjs',
  'pharos.integration.js',
];
 ...
 * @returns {Promise<import('../types/integration').PharosIntegration>}
 */
export async function loadManifestObject(file, label = 'integration manifest') {
  return loadModuleWithSchema(file, PharosIntegrationSchema, {label});
}
