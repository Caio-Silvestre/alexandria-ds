 ...
 * try/catch in `bin/pharos.mjs` never sees them). The result: a --json
 ...
 *      envelope path (so `pharos bogus-cmd --json` gets exit 1 + envelope
 ...
 * subcommand (e.g. `pharos component --help --json`).
 *
 * @param {import('commander').Command} cmd
 * @returns {{apiVersion: number, type: 'help', data: object}}
 */
export function buildHelpEnvelope(cmd) {
  // Reconstruct the fully-qualified command name (e.g. "pharos theme build").
  const nameParts = [];
...
/**
 * Map a Commander parse-error code (and its message) to a stable Pharos error
 ...
 *   commander.unknownCommand      — `pharos bogus-cmd`
 *   commander.unknownOption       — `--bogus-flag`
 *   commander.missingArgument     — `pharos theme build` (no <file>)
