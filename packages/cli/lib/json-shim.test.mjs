...
describe('--json shim: --help renders JSON envelope', () => {
  it('pharos --help --json emits a help envelope (not raw text), exit 0', async () => {
    ...
    expect(parsed.data.command).toBe('pharos');
  ...
  it('pharos component --help --json emits a subcommand help envelope', async () => {
    const {status, stdout} = await runCli(['component', '--help', '--json']);
    expect(status).toBe(0);
    const parsed = parseJson(stdout);
    expect(parsed.apiVersion).toBe(1);
    expect(parsed.type).toBe('help');
    expect(parsed.data.command).toBe('pharos component');
    // Should expose its options
    const flags = parsed.data.options.map((o) => o.flags);
    expect(flags).toContain('--list');
  });

  it('pharos theme build --help --json emits a nested subcommand help envelope', async () => {
    const {status, stdout} = await runCli(['theme', 'build', '--help', '--json']);
    expect(status).toBe(0);
    const parsed = parseJson(stdout);
    expect(parsed.apiVersion).toBe(1);
    expect(parsed.type).toBe('help');
    expect(parsed.data.command).toBe('pharos theme build');
    expect(parsed.data.usage).toMatch(/<file>/);
  });
});

describe('--json shim: parse errors emit JSON error envelope', () => {
  it('pharos theme build --json (missing required arg) emits error envelope, exit 1', async () => {
  ...
  it('pharos --bogus-flag --json (unknown option) emits error envelope, exit 1', async () => {
  ...
  it('pharos component --json --bogus-flag (unknown option on subcmd) emits error envelope, exit 1', async () => {
...
describe('--json shim: unknown subcommand emits error envelope', () => {
  it('pharos bogus-cmd --json emits error envelope, exit 1 (not exit 0 + help)', async () => {
...
describe('--json shim: invalid --detail choice', () => {
  it('pharos --detail bogus --json emits a single error envelope, exit 1', async () => {
...
describe('--json shim: invalid --lang choice', () => {
  it('pharos docs color --lang fr --json emits a single error envelope, exit 1', async () => {
  ...
  it('pharos docs color --lang fr (no --json) writes to stderr and exits 1', async () => {
  ...
  it('pharos docs color --lang zh is accepted (exit 0)', async () => {
    const {status} = await runCli(['docs', 'color', '--lang', 'zh']);
    expect(status).toBe(0);
  });

  it('pharos docs color --lang en is accepted (exit 0)', async () => {
...
describe('--json shim: non-JSON behavior is preserved', () => {
  it('pharos theme build (no --json, missing arg) writes to stderr and exits 1', async () => {
  ...
  it('pharos --bogus-flag (no --json) writes to stderr and exits 1', async () => {
  ...
  it('pharos bogus-cmd (no --json) writes to stderr and exits 1', async () => {
  ...
  it('pharos --detail bogus (no --json) writes to stderr and exits 1', async () => {
  ...
  it('pharos --help (no --json) writes raw help text to stdout, exit 0', async () => {
    const {status, stdout, stderr} = await runCli(['--help']);
    expect(status).toBe(0);
    expect(stdout).toMatch(/^Usage: pharos/);
    expect(stderr).toBe('');
  });
});
