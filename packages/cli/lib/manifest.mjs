 ...
 *
 * `pharos --json` (and `pharos manifest --json`) emit a structured manifest that
 ...
 * intended shape, and the bare `pharos --json` / `pharos manifest --json` handlers
 ...
 * @position consumed by the bare `pharos --json` action and the `manifest` command
 */
 ...
 */
const EXAMPLES = {
  component: ['pharos component', 'pharos component XDSButton', 'pharos component XDSButton --props --json'],
  docs: ['pharos docs', 'pharos docs spacing --json'],
  discover: ['pharos discover --json'],
  search: ['pharos search modal --json', 'pharos search button --type component --json'],
  build: ['pharos build', 'pharos build "analytics dashboard" --json'],
  swizzle: ['pharos swizzle XDSButton'],
  template: ['pharos template --json', 'pharos template dashboard ./src/app'],
  hook: ['pharos hook', 'pharos hook useToggle --json'],
  'theme build': ['pharos theme build ./src/themes/ocean.ts --out ./dist/ocean.css'],
  'theme list': ['pharos theme list --json'],
  'theme add': ['pharos theme add matcha', 'pharos theme add matcha ./src/themes/matcha'],
  upgrade: ['pharos upgrade --json'],
  manifest: ['pharos manifest --json', 'pharos --json'],
  doctor: ['pharos doctor', 'pharos doctor --json'],
  'validate-integration': [
    'pharos validate-integration',
    'pharos validate-integration @acme/widgets --json',
  ],
  init: ['pharos init'],
  'layout expand': [`pharos layout expand 'V[g6] > C{card-callout}*4' ./src/Page.tsx`],
  'layout check': [`pharos layout check 'A[cp6] > L > LC > S[p6]' --json`],
  'layout grammar': ['pharos layout grammar'],
};
