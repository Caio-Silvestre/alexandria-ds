...
/**
 * A discovered documentation package (its package.json declares `pharos.docs`).
 ...
 * @property {string} dir
 * @property {Record<string, any>} pharos
...
export function scanDirectory(scanDir) {
  ...
  for (const entry of entries) {
    ...
    if (!pkg.pharos || !pkg.pharos.docs) continue;
    const pkgDir = path.join(scanDir, entry.name);
    const docsDir = path.resolve(pkgDir, pkg.pharos.docs);
    ...
    packages.push({
      ...
      dir: pkgDir,
      pharos: pkg.pharos,
      category: pkg.pharos.category || pkg.name || entry.name,
      docsDir,
      components,
    });
  }
