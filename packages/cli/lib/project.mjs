 ...
 *     PharosIntegrationIssue encountered is collected into a private set and
...
import {PharosConfigSchema} from './config-schema.mjs';
 ...
 * {@link import('../types/integration').PharosIntegrationIssue} fields plus the
 * `package` that produced it, which Project tracks for routing/dedup.
 * @typedef {import('../types/integration').PharosIntegrationIssue & {package: string}} ProjectIntegrationIssue
 */
...
/**
 * Find pharos.config.{ts,mjs,js} as a sibling of the nearest package.json.
...
export function findConfigPath(startDir = process.cwd()) {
  ...
  if (present.length > 1) {
    throw new Error(
      `Multiple Pharos config files found in ${root} (${present
...
export class Project {
  ...
  /** @type {string|null} */
  #configPath;
  /** @type {import('../types/config').PharosConfig} */
  #config;
  /** @type {string[]} */
  #integrations;
   ...
   * @param {import('../types/config').PharosConfig} init.config validated PharosConfig surface
  ...
  static async load(cwd = process.cwd(), {cache} = {}) {
    ...
    if (configPath) {
      config = await loadModuleWithSchema(configPath, PharosConfigSchema, {
        label: 'pharos.config',
      });
   ...
   * @returns {import('../types/config').PharosConfig}
   */
  get config() {
    return this.#config;
  }
   ...
   * @param {string} pkg
   * @param {import('../types/integration').PharosIntegrationIssue} issue
   */
  #pushIssue(pkg, issue) {
   ...
   *
   * @returns {Promise<import('../types/integration').PharosIntegrationIssue[]>}
   */
  async issues() {
    for (const integration of this.#loadedIntegrations) {
      await this.#collectIssues(integration);
