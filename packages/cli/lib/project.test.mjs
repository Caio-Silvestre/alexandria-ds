...
/**
 * Scaffold a consumer project (package.json + pharos.config.mjs) under a
...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.pharos-project-test-'));
...
describe('findConfigPath', () => {
  it('rejects multiple config files at the same root', () => {
    ...
    expect(() => findConfigPath(tmpDir)).toThrow(/Multiple Pharos config files/);
  });
});
...
describe('Project.issuesUrl routing', () => {
  it('routes core to config.issuesUrl, falling back to the default', async () => {
    ...
    expect(project.issuesUrl({package: '@pharos-ds/core'})).toBe(
