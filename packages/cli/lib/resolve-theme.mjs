 ...
 * 1. PHAROS_THEME environment variable
 ...
 * - Otherwise → try `@pharos-ds/theme-{name}`, then try as bare package name
 *
...
/**
 * Resolve the active Pharos theme from config and environment.
 *
...
export function resolveTheme(cwd = process.cwd()) {
  ...
  let specifier = process.env.PHAROS_THEME || null;
  ...
  if (!specifier) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        specifier = pkg.pharos?.theme || null;
      } catch {
        // Ignore parse errors
      }
    }
  ...
  if (specifier.startsWith('.') || specifier.startsWith('/')) {
    ...
    // Convention: try @pharos-ds/theme-{name} first, then bare package
    mod = tryLoadModule(`@pharos-ds/theme-${specifier}`, cwd);
    if (!mod) {
      mod = tryLoadModule(specifier, cwd);
    }
    if (!mod) {
      console.warn(`⚠ theme: could not resolve "${specifier}" (tried @pharos-ds/theme-${specifier} and ${specifier})`);
      return null;
    }
  }
