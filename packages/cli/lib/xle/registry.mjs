...
/**
 * @file XLE component registry — built from @pharos-ds/core .doc.mjs metadata.
 *
...
export async function buildRegistry({cwd = process.cwd()} = {}) {
  ...
  if (!coreDir) {
    throw new Error('Could not find @pharos-ds/core package — run from an XDS workspace');
  }
  ...
  const upgradeFromDoc = (/** @type {string} */ rawName, /** @type {import('./xle-ast').DocProp[]} */ props, /** @type {string} */ fallbackDir) => {
    ...
    const name = normalizeName(rawName);
    // Prefer the component's own export subpath (Heading → @pharos-ds/core/Heading);
