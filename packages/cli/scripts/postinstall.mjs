...
/**
 * @file @pharos-ds/cli postinstall — nudge to run `pharos init`.
 *
 * Enforcement layer 2 of making `pharos init` foolproof: when the CLI is
 ...
 *
 * Reuses the ONE setup check (isPharosInitialized from agent-docs.mjs — a
...
async function projectIsSetUp(root) {
  try {
    const {isPharosInitialized} = await import('../lib/agent-docs/agent-docs.mjs');
    return isPharosInitialized(root);
  } catch {
...
async function main() {
  ...
  if (nudge) {
    // Scoped package form (`@pharos-ds/cli`) — always resolves to us. Bare
    // `npx pharos` would fetch an unrelated look-alike package (see PR #4151).
