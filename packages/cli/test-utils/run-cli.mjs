 ...
 *
 * Drives the REAL Pharos program — all commands, the preAction --json gate, the
 * hooks, and the json-shim — via createProgram() + parseAsync, instead of
 * spawning `node bin/pharos.mjs` per assertion. That makes CLI suites fast and
...
/**
 * Run the Pharos CLI in-process and capture its observable surface.
 *
 * @param {string[]} args argv after `pharos` (e.g. `['component', '--list']`)
...
export async function runCli(args, cwdOrOpts) {
  ...
  try {
    ...
    if (e instanceof ExitSignal) {
      ...
      // Mirror bin/pharos.mjs's error boundary: route it through the JSON shim
