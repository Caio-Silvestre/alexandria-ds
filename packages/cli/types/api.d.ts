...
/**
 * Programmatic API types for @pharos-ds/cli/api.
 *
 * Every function returns the same { type, data } envelope as `xds --json`.
 * Errors throw PharosError.
 */
...
export declare class PharosError extends Error {
...
export interface SwizzleOptions {
  cwd?: string;
  /** Output directory (must resolve inside cwd). Defaults to ./components/pharos. */
  output?: string;
 ...
 * throw PharosError (ERR_UNKNOWN_COMPONENT, ERR_AMBIGUOUS_COMPONENT, …).
 */
...
export declare function summarizeIssues(issues: PharosIntegrationIssue[]): {
  errors: number;
  warnings: number;
};
 ...
 * Performs the effect in `apply` mode and returns a receipt; throws PharosError
 * on failure.
 */
...
/**
 * Read the Pharos blog over the canonical RSS feed. No args lists posts; a slug
 * reads that post's plaintext. Throws PharosError on a fetch failure or an
 * unknown slug.
 */
 ...
 * PharosError on an unknown feature or template name.
 */
 ...
 * PharosError on failure. `out` overrides the output CSS path.
 */
