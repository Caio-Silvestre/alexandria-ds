 ...
 * `@pharos-ds/cli/codemod`.
 */

/** A single source file presented to a codemod's transform. */
export interface PharosCodemodFile {
...
export interface PharosCodemodApi {
 ...
 */
export type PharosCodemodTransform = (
  file: PharosCodemodFile,
  api: PharosCodemodApi,
) => string | null | undefined;

/** Definition accepted by {@link createCodemod}. */
export interface PharosCodemodDef {
  ...
  transform: PharosCodemodTransform;
}

/** Result of {@link createCodemod}. */
export interface PharosCodemod extends PharosCodemodDef {
  isOptional: boolean;
  type: 'code';
}

/** Definition accepted by {@link createConfigCodemod}. */
export interface PharosConfigCodemodDef {
  /** Short, human-readable title shown in upgrade output. */
  title: string;
  /** Optional longer description. */
  description?: string;
  /** When true, the codemod runs only when explicitly requested. */
  isOptional?: boolean;
  /** The transform function applied to the pharos.config.* file. */
  transform: PharosCodemodTransform;
}

/** Result of {@link createConfigCodemod}. */
export interface PharosConfigCodemod extends PharosConfigCodemodDef {
  isOptional: boolean;
  type: 'config';
}

/** Define a file-transforming codemod. */
export declare function createCodemod<T extends PharosCodemodDef>(
  def: T,
): PharosCodemod;

/** Define a codemod that targets the consumer's pharos.config.* file. */
export declare function createConfigCodemod<T extends PharosConfigCodemodDef>(
  def: T,
): PharosConfigCodemod;
 ...
 * {@link PharosCodemodApi} except `jscodeshift` is typed as the callable
