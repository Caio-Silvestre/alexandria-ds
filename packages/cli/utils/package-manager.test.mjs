...
function makeTmpDir() {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-pm-test-'));
  return tmpDir;
}
...
describe('isCliOneOff', () => {
  it('detects an npm npx cache entry', () => {
    process.argv[1] = '/home/u/.npm/_npx/a1b2/node_modules/.bin/pharos';
    expect(isCliOneOff()).toBe(true);
  });

  it('detects a pnpm dlx cache entry', () => {
    process.argv[1] = '/home/u/.cache/pnpm/dlx/9f/node_modules/@pharos-ds/cli/bin/pharos.mjs';
    expect(isCliOneOff()).toBe(true);
  });

  it('detects a bunx cache entry', () => {
    process.argv[1] = '/home/u/.bun/install/cache/@pharos-ds/cli/bin/pharos.mjs';
    expect(isCliOneOff()).toBe(true);
  });

  it('is false for an installed node_modules entry', () => {
    process.argv[1] = '/proj/node_modules/@pharos-ds/cli/bin/pharos.mjs';
    expect(isCliOneOff()).toBe(false);
  });

  it('is false for a source checkout (dev) entry', () => {
    process.argv[1] = '/repo/packages/cli/bin/pharos.mjs';
    expect(isCliOneOff()).toBe(false);
  });
});
...
describe('getCliInvocation', () => {
  it('uses the run-prefix + bare bin when installed (not one-off)', () => {
    ...
    const dir = makeTmpDir();
    fs.writeFileSync(path.join(dir, 'pnpm-lock.yaml'), '');
    expect(getCliInvocation(dir)).toBe('pnpm exec pharos');
  });
  ...
  it('uses the dlx runner + scoped package when run one-off', () => {
    ...
    const dir = makeTmpDir();
    delete process.env.npm_config_user_agent;
    expect(getCliInvocation(dir)).toBe('npx @pharos-ds/cli');
  });
  ...
  it('pairs the dlx runner with the scoped package for pnpm one-off', () => {
    ...
    const dir = makeTmpDir();
    fs.writeFileSync(path.join(dir, 'pnpm-lock.yaml'), '');
    expect(getCliInvocation(dir)).toBe('pnpm dlx @pharos-ds/cli');
  });
});

describe('formatCliCommand', () => {
  it('strips a leading "pharos" token and prepends the invocation stem', () => {
    process.argv[1] = '/proj/node_modules/@pharos-ds/cli/bin/pharos.mjs';
    const dir = makeTmpDir();
    fs.writeFileSync(path.join(dir, 'pnpm-lock.yaml'), '');
    expect(formatCliCommand('pharos component Button', dir)).toBe('pnpm exec pharos component Button');
  });

  it('accepts a bare subcommand (no leading pharos)', () => {
    process.argv[1] = '/proj/node_modules/@pharos-ds/cli/bin/pharos.mjs';
    const dir = makeTmpDir();
    fs.writeFileSync(path.join(dir, 'package-lock.json'), '{}');
    expect(formatCliCommand('docs tokens', dir)).toBe('npx pharos docs tokens');
  });
  ...
  it('rewrites to the scoped package for one-off invocations', () => {
    ...
    const dir = makeTmpDir();
    fs.writeFileSync(path.join(dir, 'package-lock.json'), '{}');
    expect(formatCliCommand('pharos component Button', dir)).toBe('npx @pharos-ds/cli component Button');
  });
});
