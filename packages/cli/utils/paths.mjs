...
/**
 * @file Path resolution utilities for Pharos CLI
 *
...
/** Root of the @pharos-ds/cli package */
export const CLI_ROOT = path.resolve(__dirname, '..');

/**
 * Find packages/core directory by walking up from startDir.
 * Also checks node_modules/@pharos-ds/core for installed usage.
 */
export function findCoreDir(startDir = process.cwd()) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const nodeModules = path.join(dir, 'node_modules', '@pharos-ds', 'core');
...
/**
 * Discover external Pharos-compatible packages from node_modules.
 * Scans for packages with an "pharos" field in their package.json:
 *
 *   { "pharos": { "docs": "./src", "category": "Common", "blocks": "./blocks/components" } }
 *
  ...
  const scanDir = (searchDir) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (pkg.name === '@pharos-ds/core') continue;
        if (pkg.pharos && pkg.pharos.docs) {
          externals.push({
            name: pkg.name,
            category: pkg.pharos.category || pkg.name,
            docsDir: path.resolve(fullPath, pkg.pharos.docs),
            blocksDir: pkg.pharos.blocks
              ? path.resolve(fullPath, pkg.pharos.blocks)
              : null,
          });
        }
      } catch {
 ...
 * Returns directory names that contain Pharos*.tsx files.
