...
/**
 * @file Check for newer @pharos-ds/core versions via local signals.
 *
 * Resolution order:
 * 1. $PHAROS_LATEST_VERSION env var
 ...
 * Also sets $PHAROS_LATEST_VERSION for subsequent commands in the same shell.
 */
 ...
 * No network calls — purely an env-var check ($PHAROS_LATEST_VERSION),
...
export function getLatestVersion() {
  ...
  const envVersion = process.env.PHAROS_LATEST_VERSION;
...
/**
 * Get the currently installed @pharos-ds/core version from the consumer's package.json.
 *
...
export function getInstalledVersion(cwd = process.cwd()) {
  try {
    ...
    const version = deps['@pharos-ds/core'];
 ...
 * Also sets $PHAROS_LATEST_VERSION env var for subsequent commands.
 *
 * @param {string} [cwd] - Project directory
...
export function checkForUpdate(cwd = process.cwd()) {
  ...
  process.env.PHAROS_LATEST_VERSION = latest;
  ...
  if (semverGt(latest, installed)) {
    return `FYI: A newer version of @pharos-ds/core (${latest}) is available. Install the new package version, then run: ${getCliInvocation()} upgrade --from <old-version> --apply`;
  }
