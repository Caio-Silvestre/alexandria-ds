...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-update-check-'));
  // Save and clear env
  originalEnv.PHAROS_LATEST_VERSION = process.env.PHAROS_LATEST_VERSION;
  delete process.env.PHAROS_LATEST_VERSION;
});

afterEach(() => {
  fs.rmSync(tmpDir, {recursive: true, force: true});
  // Restore env
  if (originalEnv.PHAROS_LATEST_VERSION !== undefined) {
    process.env.PHAROS_LATEST_VERSION = originalEnv.PHAROS_LATEST_VERSION;
  } else {
    delete process.env.PHAROS_LATEST_VERSION;
  }
});
...
describe('getLatestVersion', () => {
  it('reads from PHAROS_LATEST_VERSION env var', () => {
    process.env.PHAROS_LATEST_VERSION = '0.0.8';
    expect(getLatestVersion()).toBe('0.0.8');
  });

  it('ignores invalid env var values', () => {
    process.env.PHAROS_LATEST_VERSION = 'not-a-version';
...
describe('getInstalledVersion', () => {
  it('reads @pharos-ds/core version from dependencies', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@pharos-ds/core': '^0.0.7'}}),
    );
  ...
  it('strips semver range prefix', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@pharos-ds/core': '~0.0.5'}}),
    );
    expect(getInstalledVersion(tmpDir)).toBe('0.0.5');
  });

  it('returns null when @pharos-ds/core is not a dependency', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
...
describe('checkForUpdate', () => {
  it('returns hint when newer version is available', () => {
    ...
    expect(hint).toContain('pharos upgrade --from <old-version> --apply');
    expect(hint).toContain('FYI');
  });

  it('returns null when up to date', () => {
    process.env.PHAROS_LATEST_VERSION = '0.0.7';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@pharos-ds/core': '^0.0.7'}}),
    );
  ...
  it('uses semver (not lexicographic) comparison for double-digit patches', () => {
    ...
    // 0.0.20 is available.
    process.env.PHAROS_LATEST_VERSION = '0.0.20';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@pharos-ds/core': '^0.0.5'}}),
    );
  ...
  it('does not suggest an upgrade when latest < installed (semver)', () => {
    ...
    process.env.PHAROS_LATEST_VERSION = '0.0.9';
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({dependencies: {'@pharos-ds/core': '^0.0.10'}}),
    );
