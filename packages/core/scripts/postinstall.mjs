...
/**
 * @file @pharos-ds/core postinstall — nudge to run `npx @pharos-ds/cli init`.
 *
 * Enforcement layer 1 of making `pharos init` foolproof: when the design system
...
const MARKERS = ['<!-- PHAROS:START -->', '<!-- XDS:START -->'];

/**
 * True when the project already has the Pharos agent prompt installed (an Pharos
 * marker is present in any agent-doc file) — the same check the CLI performs.
 * @param {string} root @returns {boolean}
 */
export function isPharosInitialized(root) {
...
function main() {
  ...
  if (
    shouldNudge({
      ...
      isSetUp: isPharosInitialized(root),
    })
  ) {
    // Scoped package form (`@pharos-ds/cli`) — always resolves to us, even
    // before the CLI is installed. Bare `npx pharos` would fetch an unrelated
    // look-alike package (see PR #4151). After that lands, switch this to its
    // getCliInvocation() single source of truth.
    process.stdout.write(
      '\nNext step: run `npx @pharos-ds/cli init` to finish setup and install the Pharos agent prompt.\n\n',
    );
  }
}
