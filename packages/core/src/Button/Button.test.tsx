...
describe('Button', () => {
  ...
  it('renders the loading spinner with the inherit shade for every variant (#2717)', () => {
    ...
    for (const variant of [
      ...
      const spinner = container.querySelector('.pharos-spinner');
  ...
  it('renders pharos-* classes and data attributes for theme targeting', () => {
    render(<Button label="Test" variant="secondary" size="sm" />);
    const button = screen.getByRole('button');
    expect(button.className).toContain('pharos-button');
