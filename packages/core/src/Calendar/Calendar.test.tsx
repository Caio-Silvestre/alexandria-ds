...
describe('Calendar', () => {
  ...
  describe('day-cell marker theme state', () => {
    ...
    it('exposes the marker states as themeable defineTheme targets', () => {
      ...
      expect(css).toContain('.pharos-calendar-day.today-only');
      expect(css).toContain('.pharos-calendar-day.today-in-range');
  ...
  describe('theming targets', () => {
    it('renders the pharos-calendar-nav target on both month-nav buttons', () => {
      render(<Calendar focusDate="2026-01-01" />);
      ...
      // global pharos-button handle that hits every Button in the app.
      expect(prev).toHaveClass('pharos-calendar-nav');
      expect(next).toHaveClass('pharos-calendar-nav');
    ...
    it('keeps the default nav rendering unchanged (still a ghost icon button)', () => {
      ...
      const prev = getButton('Previous month');
      expect(prev).toHaveClass('pharos-button');
      expect(prev).toHaveClass('ghost');
      expect(prev.tagName).toBe('BUTTON');
    });

    it('renders the pharos-calendar-day target with its reflected states', () => {
      render(
        <Calendar mode="single" value="2026-01-15" focusDate="2026-01-01" />,
      );

      const selected = getDayButton(15);
      expect(selected).toHaveClass('pharos-calendar-day');
      ...
      expect(plain).toHaveClass('pharos-calendar-day');
    ...
    it('exposes calendar-nav as a themeable defineTheme target', () => {
      ...
      const css = generateThemeCSSFlat(theme);
      expect(css).toContain('.pharos-calendar-nav {');
      expect(css).toContain('color: var(--color-accent)');
      expect(css).toContain('.pharos-calendar-nav.next');
