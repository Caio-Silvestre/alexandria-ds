...
function serialize(node: Node): string {
  ...
  for (const child of Array.from(node.childNodes)) {
    if (child.nodeType === Node.TEXT_NODE) {
      ...
      if (child.hasAttribute('data-pharos-token')) {
        result += child.getAttribute('data-pharos-token-value') ?? '';
...
export function ChatComposerInput(props: ChatComposerInputProps) {
  ...
  const label = labelFromProps ?? t('@pharos.chat.composerInput.label');
  const placeholder =
    placeholderFromProps ??
    composerCtx?.placeholder ??
    t('@pharos.chat.composer.placeholder');
  ...
  const emitChange = useCallback(() => {
    ...
    const hasTokens =
      editableRef.current.querySelector(
        '[data-pharos-token], [data-pharos-dictation-interim]',
      ) != null;
...
export function ChatComposerTokenElement({token}: {token: ChatComposerToken}) {
  return (
    <span
      data-pharos-token=""
      data-pharos-token-value={token.value}
