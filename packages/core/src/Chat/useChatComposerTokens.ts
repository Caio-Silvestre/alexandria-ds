 ...
 * - Serialization awareness (data-pharos-token attributes)
 *
...
function isInsideToken(node: Node): HTMLElement | null {
  ...
  while (current) {
    if (
      ...
      current.hasAttribute('data-pharos-token')
    ) {
      return current;
    }
...
export function useChatComposerTokens({
  ...
  const insertToken = useCallback(
    (token: ChatComposerToken): string | undefined => {
      ...
      span.setAttribute('data-pharos-token', '');
      span.setAttribute('data-pharos-token-value', token.value);
      span.setAttribute('data-pharos-token-id', id);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent): boolean => {
      ...
      if (
        ...
        startContainer.previousSibling.hasAttribute('data-pharos-token')
      ) {
        return false;
      }
  ...
  const expandToken = useCallback(
    (id: string) => {
      ...
      const {span} = portal;
      const value = span.getAttribute('data-pharos-token-value') ?? '';
