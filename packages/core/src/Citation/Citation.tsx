...
export interface CitationSource {
   ...
   * - a React node / Pharos `<Icon>` / SVG element — rendered as-is, or
   ...
   * favicon behavior. Pass an Pharos `<Icon>` (or other node) for a real icon.
   */
  icon?: ReactNode;
}
...
export function Citation({
  ...
  // Resolve the source icon. A non-string `icon` node renders as-is (an Pharos
  ...
  if (variant === 'number') {
    return (
      <Tag
        ...
        ref={elementRef}
        role={noteRole}
        aria-label={t('@pharos.citation.label', {number, title})}
        data-testid={testId}
        {...linkProps}
        {...mergeProps(
