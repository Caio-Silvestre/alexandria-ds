...
export interface CodeBlockProps extends BaseProps<HTMLPreElement> {
   ...
   * `@pharos-ds/core/theme/syntax` or a theme created with
  ...
  for (const token of tokens) {
    ...
    parts.push(
      <span
        ...
        className={`pharos-token-${token.type} xds-token-${token.type}`}>
...
export function CodeBlock({
  ...
  const copyButtonEl = hasCopyButton ? (
    <button
      ...
      aria-label={
        copied ? t('@pharos.codeBlock.copied') : t('@pharos.codeBlock.copyCode')
      }
      {...stylex.props(
        styles.copyButton,
  ...
  const codeBody = (
    <div
      ...
      tabIndex={0}
      role="group"
      aria-label={languageLabel ?? t('@pharos.codeBlock.code')}
