...
function ensureDynamicHighlightType(tokenType: string): void {
  ...
  if (!dynamicStyleSheet) {
    ...
    style.setAttribute('data-pharos-highlight-dynamic', '');
  ...
  const name = `pharos-${tokenType}`;
  const colorVar = `var(--color-syntax-${tokenType}, currentColor)`;
  dynamicStyleSheet.insertRule(
    `.pharos-codeblock code::highlight(${name}), .pharos-codeeditor code::highlight(${name}) { color: ${colorVar}; }`,
  );
}
