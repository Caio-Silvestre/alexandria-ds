 ...
 * For static lists, use `createStaticSource` from `@pharos-ds/core/Typeahead`.
 *
...
export function CommandPalette<T extends SearchableItem = SearchableItem>({
  ...
  const label = labelFromProps ?? t('@pharos.commandPalette.label');
  const emptySearchText =
    emptySearchTextFromProps ?? t('@pharos.commandPalette.emptySearch');
  const emptyBootstrapText =
    emptyBootstrapTextFromProps ?? t('@pharos.commandPalette.emptyBootstrap');
  const listId = useId();
  ...
  const runSearch = useCallback(
    (query: string) => {
      ...
      startTransition(async () => {
        ...
        if (!isBootstrap) {
          announce(t('@pharos.commandPalette.loading'));
        }
        ...
        if (searchVersionRef.current === version) {
          ...
          if (isBootstrap) {
            ...
            announce(t('@pharos.commandPalette.noResultsFor', {query}));
          } else {
            announce(
              t('@pharos.commandPalette.resultCount', {count: items.length}),
            );
          }
