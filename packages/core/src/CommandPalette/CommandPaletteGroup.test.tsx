...
describe('CommandPaletteGroup', () => {
  ...
  describe('heading theme target', () => {
    it('renders the pharos-command-palette-group-heading target on the heading', () => {
      render(
      ...
      // The root carries `pharos-command-palette-group`; this is the stable
      // handle on the heading itself, so a theme can style just the heading
      // without a fragile structural selector.
      expect(heading).toHaveClass('pharos-command-palette-group-heading');
    });
    ...
    it('keeps the heading target distinct from the group root target', () => {
      ...
      expect(root).toHaveClass('pharos-command-palette-group');
      expect(root).not.toHaveClass('pharos-command-palette-group-heading');
      expect(heading).not.toHaveClass('pharos-command-palette-group');
    });
    ...
    it('exposes command-palette-group-heading as a themeable defineTheme target', () => {
      ...
      expect(css).toContain('.pharos-command-palette-group-heading {');
