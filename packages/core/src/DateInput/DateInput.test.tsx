...
describe('DateInput', () => {
  ...
  describe('InputGroup', () => {
    it('uses group ARIA and skips standalone Field chrome when grouped', () => {
      ...
      expect(document.querySelectorAll('.pharos-field')).toHaveLength(1);
...
describe('DateInput clear icon theme target', () => {
  ...
  it('renders the pharos-date-input-clear-icon target on the clear glyph', () => {
    render(
      <DateInput
        label="Date"
        value="2026-01-15"
    ...
    expect(icon).toHaveClass('pharos-date-input-clear-icon');
  ...
  it('renders the default icon (secondary color, sm size) byte-identically', () => {
    ...
    const styleClasses = (el: HTMLElement) =>
      el.className
        .split(' ')
        .filter(c => c !== 'pharos-date-input-clear-icon')
        .sort();
  ...
  it('exposes date-input-clear-icon so a theme reaches the icon color, size, and hover', () => {
    ...
    expect(css).toContain('.pharos-date-input-clear-icon {');
    expect(css).toContain('width: 12px');
    expect(css).toContain('height: 12px');
    expect(css).toContain('.pharos-date-input-clear-icon:hover {');
...
describe('DateInput calendar-toggle icon theme target', () => {
  ...
  it('renders the pharos-date-input-toggle-icon target on the toggle glyph', () => {
    render(<DateInput label="Date" onChange={() => {}} />);
    ...
    expect(icon).toHaveClass('pharos-date-input-toggle-icon');
    expect(icon).toHaveClass('pharos-icon');
  ...
  it('exposes date-input-toggle-icon so a theme reaches the icon size and per-state color', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-date-input-toggle-icon {');
    expect(css).toContain('width: 14px');
    expect(css).toContain('height: 14px');
    expect(css).toContain('.pharos-date-input-toggle-icon.expanded');
    expect(css).toContain('color: var(--color-icon-primary)');
  });
});
