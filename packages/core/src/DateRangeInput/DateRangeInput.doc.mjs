...
export const docs = {
  ...
  props: [
    ...
    },
    {
      ...
      type: 'DateRange | null',
      description:
        'Selected date range or null. Import the `DateRange` type from `@pharos-ds/core/DateRangeInput`; it is `{start: ISODateString, end: ISODateString}`. Do NOT redeclare your own DateRange type; use the exported one so TypeScript structurally matches.',
      required: true,
    },
    {
      name: 'onChange',
    ...
    },
    {
      ...
      type: 'ISODateString',
      description:
        'Minimum selectable date. `ISODateString` is a template literal type (`\\`${number}${number}${number}${number}-${number}${number}-${number}${number}\\``). Pass a string literal like `"2026-01-28"`, not a runtime string variable. Import it from `@pharos-ds/core/Calendar` or use `as ISODateString` if computing the value dynamically.',
    },
    {
      name: 'max',
      type: 'ISODateString',
  ...
  theming: {
    targets: [
      {className: 'pharos-date-range-input', visualProps: ['size', 'status']},
      {className: 'pharos-date-range-input-toggle-icon', states: ['state']},
      {className: 'pharos-date-range-input-clear-icon'},
    ],
  },
  usage: {
    description:
...
export const docsDense = {
  ...
  propDescriptions: {
    ...
    value: 'selected range {start, end} or null; import DateRange type from @pharos-ds/core/DateRangeInput (do not redeclare)',
