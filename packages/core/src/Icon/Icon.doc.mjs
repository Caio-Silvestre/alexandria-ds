...
export const docs = {
  ...
  props: [
    ...
    },
    {
      ...
      description: 'Color variant mapped to Pharos icon color tokens.',
      default: "'inherit'",
    },
    {
      name: 'size',
  ...
  theming: {
    targets: [
      {className: 'pharos-icon', visualProps: ['color', 'size']},
    ],
  },
  usage: {
...
export const docsZh = {
  ...
  props: [
    ...
    },
    {
      name: 'color',
      type: "'primary' | 'secondary' | 'tertiary' | 'disabled' | 'accent' | 'success' | 'error' | 'warning' | 'inherit'",
      description: '映射到 Pharos 图标颜色令牌的颜色变体。',
      default: "'inherit'",
    },
    {
      name: 'size',
...
export const docsDense = {
  description:
    'Renders icons w/ Pharos design system colors + sizes. Supports direct SVG icon components + semantic icon names that adapt to active theme.',
  usage: {
    description: 'Icons are small visual symbols that represent actions, objects, or concepts. They improve scannability and reinforce meaning alongside text. Supports both direct SVG components and semantic icon names that adapt to the active theme.',
    bestPractices: [
      { guidance: true, description: 'Use semantic icon names when available; they adapt to theme changes automatically.' },
  ...
  propDescriptions: {
    ...
    color: 'Color variant mapped to Pharos icon color tokens.',
    size: 'Icon size.',
