 ...
 * the `--pharos-*` tokens that the component reads:
 *
 *   --pharos-card-padding            (shorthand — all sides; emitted)
 *   --pharos-card-padding-inline     (horizontal)
 *   --pharos-card-padding-block-start
 *   --pharos-card-padding-block-end
 *
 * Read order per level: `var(--pharos-…, <next level>)`, terminating at
 * `--spacing-4`. Same pattern for section and dialog.
 *
 * ```ts
 * components: {
 *   card: { base: { padding: '20px' } },          // → --pharos-card-padding: 20px
 ...
 * properties that themes can set. The pipeline emits the `--pharos-*` names,
 * which the component reads via `var(--pharos-…, …)`:
 *
 *   --pharos-card-padding          (shorthand — all sides)
 *   --pharos-card-padding-inline
 *   --pharos-card-padding-inline-start
 *   --pharos-card-padding-inline-end
 *   --pharos-card-padding-block-start
 *   --pharos-card-padding-block-end
 *
 ...
 * to override from @layer pharos-theme. By reading from a higher-level token,
...
// Card padding chains: --pharos-* then the next specificity level, terminating
...
const cardShorthand = `var(--pharos-card-padding, ${SP4})`;
const cardInline = `var(--pharos-card-padding-inline, ${cardShorthand})`;
const cardInlineStart = `var(--pharos-card-padding-inline-start, ${cardInline})`;
const cardInlineEnd = `var(--pharos-card-padding-inline-end, ${cardInline})`;
const cardBlockStart = `var(--pharos-card-padding-block-start, ${cardShorthand})`;
const cardBlockEnd = `var(--pharos-card-padding-block-end, ${cardShorthand})`;

// Section padding chains: --pharos-* then the next specificity level, terminating
// at --spacing-4. Built as chained const strings (no function calls) so StyleX
// can statically analyze them; see naming.ts for the prefix policy.
const sectionShorthand = `var(--pharos-section-padding, ${SP4})`;
const sectionInline = `var(--pharos-section-padding-inline, ${sectionShorthand})`;
const sectionInlineStart = `var(--pharos-section-padding-inline-start, ${sectionInline})`;
const sectionInlineEnd = `var(--pharos-section-padding-inline-end, ${sectionInline})`;
const sectionBlockStart = `var(--pharos-section-padding-block-start, ${sectionShorthand})`;
const sectionBlockEnd = `var(--pharos-section-padding-block-end, ${sectionShorthand})`;

// Dialog padding chains: --pharos-* then the next specificity level, terminating
// at --spacing-4. Built as chained const strings (no function calls) so StyleX
// can statically analyze them; see naming.ts for the prefix policy.
const dialogShorthand = `var(--pharos-dialog-padding, ${SP4})`;
const dialogInline = `var(--pharos-dialog-padding-inline, ${dialogShorthand})`;
const dialogInlineStart = `var(--pharos-dialog-padding-inline-start, ${dialogInline})`;
const dialogInlineEnd = `var(--pharos-dialog-padding-inline-end, ${dialogInline})`;
const dialogBlockStart = `var(--pharos-dialog-padding-block-start, ${dialogShorthand})`;
const dialogBlockEnd = `var(--pharos-dialog-padding-block-end, ${dialogShorthand})`;
 ...
 * @example
 * ```
 * import { container } from '@pharos-ds/core/Layout';
 * import * as stylex from '@stylexjs/stylex';
 *
