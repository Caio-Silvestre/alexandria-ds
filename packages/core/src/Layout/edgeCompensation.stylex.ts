 ...
 *
 * 1. **Components** mark themselves with `data-pharos-edge-comp` (a passive
 *    data attribute — no styles attached).
 * 2. **Containers** use `:has(> [data-pharos-edge-comp]:first-child)` and
 *    `:has(> [data-pharos-edge-comp]:last-child)` to detect edge-adjacent
 ...
 */
export const EDGE_COMP_ATTR = 'data-pharos-edge-comp';
