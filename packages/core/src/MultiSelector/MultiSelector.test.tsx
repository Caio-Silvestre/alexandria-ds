...
describe('MultiSelector clear icon theme target', () => {
  ...
  it('renders the pharos-multi-selector-clear-icon target on the clear glyph', () => {
    render(
      <MultiSelector
        label="Fruit"
    ...
    expect(icon).toHaveClass('pharos-multi-selector-clear-icon');
  ...
  it('renders the default icon (secondary color, sm size) byte-identically', () => {
    ...
    const styleClasses = (el: HTMLElement) =>
      el.className
        .split(' ')
        .filter(c => c !== 'pharos-multi-selector-clear-icon')
        .sort();
  ...
  it('exposes multi-selector-clear-icon so a theme reaches the icon color, size, and hover', () => {
    ...
    expect(css).toContain('.pharos-multi-selector-clear-icon {');
    expect(css).toContain('width: 12px');
    expect(css).toContain('height: 12px');
    expect(css).toContain('.pharos-multi-selector-clear-icon:hover {');
...
describe('MultiSelector indicator (chevron) icon theme target', () => {
  ...
  const getIndicatorIcon = (container: HTMLElement): HTMLElement => {
    ...
    const icon = container.querySelector(
      '.pharos-multi-selector-indicator-icon',
    );
    if (icon == null) {
  ...
  it('renders the pharos-multi-selector-indicator-icon target on the chevron glyph', () => {
    const {container} = render(
      <MultiSelector
        label="Fruit"
        options={ICON_OPTIONS}
    ...
    expect(icon).toHaveClass('pharos-multi-selector-indicator-icon');
    expect(icon).toHaveClass('pharos-icon');
  ...
  it('exposes multi-selector-indicator-icon so a theme reaches the icon size and per-state color', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-multi-selector-indicator-icon {');
    expect(css).toContain('width: 14px');
    expect(css).toContain('height: 14px');
    expect(css).toContain('.pharos-multi-selector-indicator-icon.expanded');
    expect(css).toContain('color: var(--color-icon-primary)');
  });
});
