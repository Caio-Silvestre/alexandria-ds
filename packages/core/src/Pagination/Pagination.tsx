 ...
 * @example
 * ```
 * declare module '@pharos-ds/core/Pagination' {
...
export function Pagination({
  ...
  const label = labelFromProps ?? t('@pharos.pagination.label');
  const previousLabel = t('@pharos.pagination.previous');
  const nextLabel = t('@pharos.pagination.next');
  ...
  const pageIndicatorsLabel = t('@pharos.pagination.pageIndicators');
  const itemsPerPageLabel = t('@pharos.pagination.itemsPerPage');
  ...
  const handlePageChange = (newPage: number) => {
    ...
    announce(
      computedTotalPages != null
        ? t('@pharos.pagination.pageOfTotal', {
            current: newPage,
            total: computedTotalPages,
          })
        : t('@pharos.pagination.pageAnnounce', {current: newPage}),
    );
          ...
          <>
            {pageRange.map((item, index) => {
              ...
              return (
                <Button
                  key={item}
                  label={t('@pharos.pagination.goToPage', {page: item})}
                  aria-label={t('@pharos.pagination.goToPage', {
                    page: item,
                  })}
                  variant="ghost"
                  size={buttonSize}
