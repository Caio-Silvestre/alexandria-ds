...
export function PowerSearch({
  ...
  const label = labelFromProps ?? t('@pharos.powersearch.label');
  const placeholder =
    placeholderFromProps ?? t('@pharos.powersearch.placeholder');
  const popoverSaveButtonLabel =
    popoverSaveButtonLabelFromProps ?? t('@pharos.powersearch.editor.apply');
  ...
  const resultCountText = useMemo((): string | null => {
    ...
    if (typeof resultCount === 'number') {
      return t('@pharos.powersearch.resultCount', {count: resultCount});
    }
    return resultCount;
  }, [resultCount, t]);
