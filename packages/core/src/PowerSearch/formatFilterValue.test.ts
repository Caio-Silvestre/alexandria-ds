...
const t: TranslatorFn = (key, values) => {
  ...
  switch (key) {
    case '@pharos.powersearch.valueEditor.itemsCount':
      return `${count} ${count === 1 ? 'item' : 'items'}`;
    case '@pharos.powersearch.valueEditor.entitiesCount':
      return `${count} ${count === 1 ? 'entity' : 'entities'}`;
    case '@pharos.powersearch.valueEditor.filtersCount':
      return `${count} ${count === 1 ? 'filter' : 'filters'}`;
    case '@pharos.powersearch.valueEditor.dateRange':
      return 'date range';
    default:
      return key;
  }
