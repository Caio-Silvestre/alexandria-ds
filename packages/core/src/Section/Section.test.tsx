...
describe('Section', () => {
  ...
  it('renders with variant="section" (default)', () => {
    ...
    expect(inner.className).toContain('pharos-section');
  ...
  it('renders pharos-* class names for theme targeting', () => {
    const {container} = render(<Section>Content</Section>);
    const inner = container.firstElementChild!.firstElementChild!;
    expect(inner.className).toContain('pharos-section');
  });

  it('renders variant in pharos class names', () => {
    const {container} = render(<Section variant="muted">Content</Section>);
    const inner = container.firstElementChild!.firstElementChild!;
    expect(inner.className).toContain('pharos-section');
    expect(inner.className).toContain('muted');
  ...
  it('propagates explicit padding to nested sections via --pharos-section-padding', () => {
    const {container} = render(
      <Section padding={6}>
        <Section data-testid="inner">Inner</Section>
      </Section>,
    );
    // Outer section's inner div should set --pharos-section-padding
