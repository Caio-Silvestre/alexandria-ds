...
export const docs = {
  ...
  theming: {
    targets: [
      ...
      {className: 'pharos-table'},
      {className: 'pharos-table-scroll-wrapper'},
      {className: 'pharos-table-header'},
      {className: 'pharos-table-body'},
      {className: 'pharos-table-footer'},
      {className: 'pharos-table-row'},
      {className: 'pharos-table-cell'},
      {className: 'pharos-table-header-cell'},
    ],
  },
  ...
  props: [
    ...
    },
    {
      name: 'columns',
      type: 'TableColumn<T>[]',
      description: 'Column definitions: each column has {key, header, width?, align?, renderCell?}. The `header` field sets the column heading text. If omitted, columns are auto-generated from data object keys. The `width` field is typed as `ColumnWidth` (not a number); use `proportional(n)` or `pixel(n)` helpers imported from `@pharos-ds/core/Table`. Example: `width: pixel(120)` for 120px fixed, `width: proportional(1)` for flex distribution.',
    },
    {
      name: 'idKey',
  ...
  usage: {
    ...
    bestPractices: [
      ...
      { guidance: true, description: 'Compose rich cell content with Pharos components like Badge, StatusDot, and Avatar via renderCell.' },
