...
describe('BaseTable', () => {
  ...
  describe('root element styling props (#3679)', () => {
    ...
    it('keeps the pharos theme classes alongside a consumer className', () => {
      render(<Table data={users} columns={columns} className="custom-table" />);
      const table = screen.getByRole('table');
      expect(table.className).toContain('pharos-base-table');
      expect(table.className).toContain('pharos-table');
...
describe('Table', () => {
  ...
  it('wraps table in a scroll container', () => {
    ...
    expect(wrapper!.className).toContain('pharos-table-scroll-wrapper');
  });
