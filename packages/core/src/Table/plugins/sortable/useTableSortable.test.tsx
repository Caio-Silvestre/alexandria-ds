...
describe('useTableSortable — i18n', () => {
  it('localizes the unsorted sort-by aria-label via @pharos.table.sort.sortBy', () => {
    render(
      <InternationalizationProvider
        locale="fr"
        overrides={{
          fr: {'@pharos.table.sort.sortBy': 'Trier par {label}'},
        }}>
        <SortableTable />
      </InternationalizationProvider>,
    );
  ...
  it('localizes the multi-sort priority aria-label with ICU number args', () => {
    render(
      <InternationalizationProvider
        ...
        overrides={{
          fr: {
            '@pharos.table.sort.sortedByWithPriority':
              'Trier par {label}, tri {direction}, priorité {rank, number} sur {total, number}',
            '@pharos.table.sort.direction.ascending': 'croissant',
            '@pharos.table.sort.direction.descending': 'décroissant',
          },
        }}>
        <SortableTable
          isMultiSortEnabled
  ...
  it('localizes the sorted aria-label and direction word via their own keys', () => {
    render(
      <InternationalizationProvider
        ...
        overrides={{
          fr: {
            '@pharos.table.sort.sortedBy': 'Trié par {label}, {direction}',
            '@pharos.table.sort.direction.ascending': 'croissant',
          },
        }}>
        <SortableTable
