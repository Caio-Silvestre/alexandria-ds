 ...
 *
 * Follows Pharos headless plugin conventions: the consumer owns all state
...
const sortStyles = stylex.create({
  ...
  rank: {
    fontSize: 10,
    // eslint-disable-next-line @pharos-ds/no-hardcoded-styles -- no token for lineHeight:1 (tight badge)
    lineHeight: '1',
...
function buildAriaLabel<T extends Record<string, unknown>>(
  ...
  if (direction == null) {
    return t('@pharos.table.sort.sortBy', {label});
  }
  const directionLabel =
    direction === 'ascending'
      ? t('@pharos.table.sort.direction.ascending')
      : t('@pharos.table.sort.direction.descending');
  if (rank != null && total > 1) {
    return t('@pharos.table.sort.sortedByWithPriority', {
      label,
  ...
  return t('@pharos.table.sort.sortedBy', {label, direction: directionLabel});
}
        ...
        const getSortActions = (): TableContextAction[] => {
          ...
          const actions: TableContextAction[] = [
            {
              id: 'sort-asc',
              group: 'sort',
              label: t('@pharos.table.sort.ascending'),
            ...
            },
            {
              id: 'sort-desc',
              group: 'sort',
              label: t('@pharos.table.sort.descending'),
          ...
          if (dir != null) {
            actions.push({
              id: 'sort-clear',
              group: 'sort-clear',
              label: t('@pharos.table.sort.clear'),
