...
function newestMediaAttr(scope: ParentNode): string | null {
  const nodes = scope.querySelectorAll('[data-pharos-media]');
  const last = nodes[nodes.length - 1];
  return last ? last.getAttribute('data-pharos-media') : null;
}
...
async function dismissAllFallbackToasts(): Promise<void> {
  const fallback = document.querySelector('[data-pharos-toast-fallback]');
...
describe('useToast fallback viewport theme mode', () => {
  ...
  it('resolves the app mode (light) instead of OS preference (dark) with no LayerProvider', async () => {
    ...
    await waitFor(() => {
      expect(
        document.querySelector(
          '[data-pharos-toast-fallback] [data-pharos-media]',
        ),
      ).not.toBeNull();
    });
    ...
    expect(
      newestMediaAttr(document.querySelector('[data-pharos-toast-fallback]')!),
    ).toBe('dark');
  });
  ...
  it('mirrors <html data-theme> and data-pharos-theme onto the fallback container', async () => {
    mockMatchMedia(false);
    ...
    await waitFor(() => {
      const fallback = document.querySelector(
        '[data-pharos-toast-fallback]',
      ) as HTMLElement;
      expect(fallback.getAttribute('data-theme')).toBe('dark');
      expect(fallback.getAttribute('data-pharos-theme')).toBe('test');
