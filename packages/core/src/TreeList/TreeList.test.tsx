...
describe('TreeList', () => {
  ...
  describe('variant', () => {
    it('renders guide lines by default', () => {
      ...
      expect(container.querySelector('.pharos-tree-list-guide')).not.toBeNull();
    });
    ...
    it("variant='noGuides' renders NO guide lines", () => {
      ...
      expect(container.querySelector('.pharos-tree-list-guide')).toBeNull();
    });
  ...
  describe('guide theme target', () => {
    it('renders the pharos-tree-list-guide target on the connector lines', () => {
      const {container} = render(<TreeList items={nestedItemsExpanded} />);
      const guide = container.querySelector('.pharos-tree-list-guide');
    ...
    it('exposes tree-list-guide as a themeable defineTheme target', () => {
      ...
      expect(css).toContain('.pharos-tree-list-guide {');
    ...
    it('lets a theme hide the guides via display: none on the target', () => {
      ...
      // theme rule lands in @layer pharos-theme, above StyleX's base layer.
      const theme = defineTheme({
  ...
  describe('indent lever', () => {
    it('lets a theme retune the indent step via the tree-list target', () => {
      ...
      const css = generateThemeCSSFlat(theme);
      expect(css).toContain('.pharos-tree-list {');
  ...
  it('applies pharos-tree-list class name', () => {
    render(<TreeList items={simpleItems} data-testid="tree" />);
    const root = screen.getByTestId('tree');
    expect(root.className).toContain('pharos-tree-list');
  });
  ...
  describe('chevron theme target', () => {
    it('renders the pharos-tree-list-chevron target on the toggle button', () => {
      render(<TreeList items={nestedItems} />);
      const toggle = screen
        .getByText('Parent')
        .closest('li')!
      ...
      expect(toggle).toHaveClass('pharos-tree-list-chevron');
    ...
    it('exposes tree-list-chevron as a themeable defineTheme target', () => {
      ...
      const css = generateThemeCSSFlat(theme);
      expect(css).toContain('.pharos-tree-list-chevron {');
      expect(css).toContain('color: var(--color-accent)');
      expect(css).toContain('.pharos-tree-list-chevron.expanded');
  ...
  describe('item label theme target', () => {
    it('renders the pharos-tree-list-item-label target on the label span', () => {
      render(<TreeList items={simpleItems} />);
      ...
      // structural selector.
      expect(label).toHaveClass('pharos-tree-list-item-label');
    ...
    it('exposes tree-list-item-label as a themeable defineTheme target', () => {
      ...
      const css = generateThemeCSSFlat(theme);
      expect(css).toContain('.pharos-tree-list-item-label {');
      expect(css).toContain('color: var(--color-text-primary)');
      expect(css).toContain('.pharos-tree-list-item-label.selected');
