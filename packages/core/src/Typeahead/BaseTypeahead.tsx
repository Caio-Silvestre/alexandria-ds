...
export const BaseTypeahead = function BaseTypeahead<T extends SearchableItem>({
  ...
  const placeholder =
    placeholderFromProps ?? t('@pharos.typeahead.searchPlaceholder');
  const emptySearchResultsText =
    emptySearchResultsTextFromProps ??
    t('@pharos.typeahead.emptySearchResults');
  ...
  return (
    <>
      ...
      {isLoading && (
        <span
          role="status"
          aria-label={t('@pharos.typeahead.loading')}
      ...
      {popover.render(
        <div
          id={listboxId}
          role="listbox"
          aria-label={t('@pharos.typeahead.searchResults')}
          {...mergeProps(
