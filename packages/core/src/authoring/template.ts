...
export interface PharosTemplatePreview {
...
export interface PharosTemplateInput {
  ...
  preview?: PharosTemplatePreview;
}

/** Input accepted by {@link createPageTemplate} (no `type` field). */
export type PharosPageTemplateInput = PharosTemplateInput;
/** Input accepted by {@link createBlockTemplate} (no `type` field). */
export type PharosBlockTemplateInput = PharosTemplateInput;

/** A validated page template doc. */
export type PharosPageTemplate = PharosTemplateInput & {type: 'page'};
/** A validated block template doc. */
export type PharosBlockTemplate = PharosTemplateInput & {type: 'block'};

/** A validated template doc (page or block). */
export type PharosTemplate = PharosPageTemplate | PharosBlockTemplate;

/**
 * Author an Pharos page template doc. Stamp-only: returns the def with
 * `type: 'page'` injected. Validation happens at the CLI load boundary.
 */
export function createPageTemplate<T extends PharosPageTemplateInput>(
  def: T,
): T & {type: 'page'} {
...
/**
 * Author an Pharos block template doc. Stamp-only: returns the def with
 * `type: 'block'` injected. Validation happens at the CLI load boundary.
 */
export function createBlockTemplate<T extends PharosBlockTemplateInput>(
  def: T,
): T & {type: 'block'} {
