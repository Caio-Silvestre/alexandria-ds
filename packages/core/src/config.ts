...
/**
 * @file config.ts
 * @input An app's Pharos config object.
 * @output A type-preserving `createConfig` helper plus the `PharosConfig`
 *   surface authors write in `pharos.config.{ts,mjs,js}`.
 * @position Consumer-facing authoring surface. Lives in `@pharos-ds/core`
...
/** User config exported from pharos.config.{ts,mjs,js}. */
...
/**
 * Type-preserving helper for the Pharos config file. Returns its argument
 ...
 */
export function createConfig<T extends PharosConfig>(config: T): T {
  return config;
}
