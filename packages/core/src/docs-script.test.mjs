 ...
 * redirects to the Pharos CLI (`npx @pharos-ds/cli init`) so agents and
...
describe('docs.mjs redirects to the Pharos CLI', () => {
  it('redirects to the CLI instead of printing a component catalog for --list', () => {
    ...
    expect(r.stdout).toMatch(/docs are served by the Pharos CLI/);
    expect(r.stdout).toMatch(/npx @pharos-ds\/cli init/);
    expect(r.stdout).toMatch(/npx @pharos-ds\/cli component --list/);
  });
  ...
  it('redirects to the CLI instead of rendering docs for a single component', () => {
    ...
    expect(r.stdout).toMatch(/npx @pharos-ds\/cli component <Name>/);
  });
});
