...
export interface PropDoc {
  ...
  required?: boolean;
  /** For ReactNode props: the Pharos components this slot typically accepts.
 ...
 * @example
 * ```
 * {className: 'pharos-button', visualProps: ['variant', 'size']}
 * {className: 'pharos-avatar-status-dot', visualProps: ['variant']}
 * {className: 'pharos-card'}
 * ```
 */
export interface ThemingTarget {
  /** The stable CSS class name rendered by the component.
   *  Always starts with `pharos-`.
   *  e.g. `"pharos-button"`, `"pharos-avatar-status-dot"`, `"pharos-card"` */
  className: string;
...
export interface ComponentVar {
   ...
   * `pharos theme build` errors if a theme sets a private var directly.
   */
  private?: boolean;
}
...
export interface ComponentEntry {
  /** Full export name including Pharos prefix. e.g. `"TableRow"`,
   ...
   *  (e.g. `"TableRow"` → `"Pharos Table Row"`). See `BaseDoc.displayName`. */
  displayName: string;
...
export interface PlaygroundConfig {
   ...
   */
  wrapper?: {
    /** Parent component name as exported from `@pharos-ds/core`, e.g. `'TabList'`. */
    component: string;
 ...
 */
interface BaseDoc {
  /** Directory name without the Pharos prefix, PascalCase.
   ...
   *  Lowercase only. Used by `pharos component <term>` for fuzzy matching.
   ...
   *  directory's doc covers a group but some Pharos*.tsx files shouldn't
...
export interface ComponentRef {
  /** Full export name including Pharos prefix, e.g. `"ChatComposer"`. Must
 ...
 * and can be individually retrieved via `pharos docs <topic> <section>`.
 */
 ...
 * in the docs/ directory and it shows up in `pharos docs`.
 *
 ...
 *
 *   /** @type {import('@pharos-ds/core').TemplateDoc} *\/
 *   export const doc = { ... };
 *
...
interface BaseTemplateDoc {
   ...
   *  `pharos template <name>` — it's only hidden from the browsable gallery.
 ...
 * they cluster related components but have no corresponding Pharos*.tsx file.
...
export interface HookDoc {
   ...
   *  Enables cross-referencing: \`pharos component Toast\` can mention useToast,
   *  and \`pharos hook useToast\` can link back to Toast. */
  relatedComponents?: string[];
  /** Other hook names this hook is commonly used with. */
  relatedHooks?: string[];
  /** Import path, e.g. '@pharos-ds/core/hooks' or '@pharos-ds/core/Toast'. */
  importPath?: string;
