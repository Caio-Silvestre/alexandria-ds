...
export const docs = {
  ...
  usage: {
    description:
      'Wraps your app to set the active locale and (optionally) merge additional translation catalogs + per-locale overrides. Pharos components inside the subtree resolve their strings against this context. If no provider is present, components fall back to the shipped English defaults.',
    bestPractices: [
      {
        guidance: true,
        description:
          'Use shipped Pharos locale catalogs from `@pharos-ds/core/locales/*` when one exists for your target locale.',
      },
      {
        guidance: true,
        description:
          'Use a same-shape local catalog only when Pharos has not shipped that locale yet or you are testing in-progress translations.',
      },
      {
        guidance: true,
        description:
  ...
  props: [
    ...
    },
    {
      ...
      required: false,
      description:
        'Optional map of BCP 47 tag to translation catalog. Import shipped catalogs from `@pharos-ds/core/locales/*`; the shipped "en" catalog is always available and does not need to be listed here.',
    },
    {
      name: 'overrides',
      type: 'Overrides',
  ...
  examples: [
    {
      label: 'Load a shipped Pharos locale catalog',
...
export function AppI18n({children}: {children: ReactNode}) {
  return (
    ...
    {
      label: 'Override one Pharos string',
      code: `import type {ReactNode} from 'react';
import {InternationalizationProvider} from '@pharos-ds/core/i18n';
...
export function AppI18n({children}: {children: ReactNode}) {
  return (
    <InternationalizationProvider
      locale="en"
      overrides={{
        en: {'@pharos.selector.placeholder': 'Choose...'},
      }}
    >
      {children}
    </InternationalizationProvider>
...
export const docsDense = {
  description:
    'Wraps app to set active locale and (optionally) merge translation catalogs + per-locale overrides. Pharos components resolve strings against this context; missing keys fall back to shipped English.',
  usage: {
    description:
      'Wraps app to set active locale and (optionally) merge translation catalogs + per-locale overrides. Pharos components resolve strings against this context; missing keys fall back to shipped English.',
    bestPractices: [
      {
        guidance: true,
        description:
          'Use shipped Pharos locale catalogs from `@pharos-ds/core/locales/*` when one exists for the target locale.',
      },
      {
        guidance: true,
        description:
          'Use a same-shape local catalog only when Pharos has not shipped that locale yet or when testing in-progress translations.',
      },
      {
        guidance: true,
        description:
