 ...
 * @position Provider component for pharos i18n locale + messages
 *
 * Wraps a subtree with a locale and (optional) additional message catalogs +
 * overrides. Pharos components inside the subtree resolve their strings via
 * this context. If a consumer never renders a provider, pharos components
...
export interface InternationalizationProviderProps {
   ...
   * `en` is bundled with pharos and never needs to be listed here.
   *
   * @example
   * ```
   ...
   *   locale="fr"
   *   overrides={{fr: {'@pharos.pagination.next': 'Suivant'}}}>
   * ```
   */
  overrides?: Overrides;
  /**
...
/**
 * Provides locale + additional messages + overrides to all pharos
