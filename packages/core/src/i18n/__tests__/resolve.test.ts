...
describe('resolve — basic lookup', () => {
  ...
  test('returns the key itself and warns for unknown key', () => {
    ...
    const out = resolve(
      '@pharos.does.not.exist',
      undefined,
      'en',
      {},
      undefined,
    );
    expect(out).toBe('@pharos.does.not.exist');
...
describe('resolve — provider messages', () => {
  const catalog: Catalog = {
    '@pharos.pagination.next': {defaultMessage: 'Suivant'},
  };
...
describe('resolve — locale-chain fallback (regional → language)', () => {
  const messages: MessagesByLocale = {
    'pt-BR': {
      '@pharos.pagination.next': {defaultMessage: 'Próxima (BR)'},
    },
    pt: {
      '@pharos.pagination.next': {defaultMessage: 'Próxima'},
      '@pharos.pagination.previous': {defaultMessage: 'Anterior'},
    },
  };
...
describe('resolve — overrides', () => {
  test('per-locale override wins over shipped catalog', () => {
    const messages: MessagesByLocale = {
      fr: {'@pharos.pagination.next': {defaultMessage: 'Suivant'}},
    };
    const overrides: Overrides = {
      fr: {'@pharos.pagination.next': 'Suiv.'},
    };
    const out = resolve(
      '@pharos.pagination.next',
      undefined,
  ...
  test('overrides respect the locale chain (pt-BR → pt)', () => {
    ...
    const overrides: Overrides = {
      pt: {'@pharos.pagination.next': 'Próxima (pt override)'},
    };
    const out = resolve(
      '@pharos.pagination.next',
      undefined,
  ...
  test('override for a different locale does not apply', () => {
    const overrides: Overrides = {
      es: {'@pharos.pagination.next': 'Siguiente'},
    };
    const out = resolve(
      '@pharos.pagination.next',
      undefined,
...
describe('resolve — warn-once behavior', () => {
  test('duplicate missing-key warnings are suppressed', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    resolve('@pharos.does.not.exist', undefined, 'en', {}, undefined);
    resolve('@pharos.does.not.exist', undefined, 'en', {}, undefined);
    resolve('@pharos.does.not.exist', undefined, 'en', {}, undefined);
