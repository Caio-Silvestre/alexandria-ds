 ...
 * - Token overrides set as CSS custom properties on [data-pharos-theme]
 * - Component overrides scoped via @scope'd CSS selectors on the stable Pharos
 ...
 * - `data-pharos-theme` — enables @scope'd theme CSS to reach elements rendered
 ...
 * Built themes (from `pharos theme build`) skip injection — their CSS
...
function useThemeStyleInjection(theme: DefinedTheme): void {
  ...
  useInsertionEffect(() => {
    ...
    const themeKey = `pharos-theme-${theme.name}`;
    ...
    warnOnce(
      ...
      `"${theme.name}" is using runtime style injection. ` +
        ...
        `  import {${theme.name}Theme} from '@pharos-ds/theme-${theme.name}/built';\n` +
        `  import '@pharos-ds/theme-${theme.name}/theme.css';\n\n` +
        `For custom themes, run \`npx @pharos-ds/cli theme build <file>\` to generate ` +
        `the built artifacts.`,
    );
    ...
    // Component overrides go into @layer pharos-theme — above StyleX layers
    ...
    if (component) {
      ...
      compStyle.textContent = `@layer pharos-theme {\n${component}\n}`;
    ...
    return () => {
      ...
      // pharos id marker, which is always written above).
 ...
 * - `data-pharos-theme` (theme name) — enables @scope'd theme CSS to reach elements
 ...
 * Theme provider component
 *
 * Sets data-pharos-theme attribute so @scope'd CSS takes effect.
 ...
 * it syncs `data-theme` and `data-pharos-theme` to `<html>` so browser
