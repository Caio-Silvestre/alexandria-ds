...
describe('generateThemeCSS', () => {
  ...
  it('splits prose into reset layer and components into pharos-theme', () => {
    const theme = defineTheme({
      name: 'split-test',
      components: {
    ...
    expect(prose).not.toContain('.pharos-button');
    // Component block should contain overrides
    expect(component).toContain('.pharos-button');
...
describe('generateThemeCSS with components', () => {
  it('generates scoped CSS for base component overrides', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-card {');
    expect(css).toContain('border-width: 2px');
    expect(css).toContain('border-color: var(--color-accent)');
    expect(css).toContain('.pharos-button {');
  ...
  it('generates variant selectors from prop:value keys', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-button.secondary');
  ...
  it('generates compound selectors from prop:value+prop:value keys', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-button.destructive.sm');
...
describe('custom status via components', () => {
  ...
  it('generates correct CSS selectors for banner status extensions', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    // parseStyleKey('status:neutral') → '.neutral', so CSS should have .pharos-banner.neutral
    expect(css).toContain('.pharos-banner.neutral');
  ...
  it('generates correct CSS for custom button variant', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('.pharos-button.primary-muted');
...
describe('pseudo-class overrides in components', () => {
  it('generates pseudo-class rules from nested objects', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    // Base rule
    expect(css).toContain('.pharos-radio {');
    expect(css).toContain('border-color: #8F9296');
    // Pseudo rule — separate selector
    expect(css).toContain('.pharos-radio:hover {');
  ...
  it('generates pseudo-class rules on variant selectors', () => {
    ...
    expect(css).toContain('.pharos-button.primary-muted {');
    expect(css).toContain('background-color: #ECF5FF');
    expect(css).toContain('.pharos-button.primary-muted:hover {');
    expect(css).toContain('background-color: #D6EBFF');
    expect(css).toContain('.pharos-button.primary-muted:focus-visible {');
  ...
  it('handles pseudo-only overrides (no base properties)', () => {
    ...
    expect(css).not.toMatch(/\.pharos-switch\s*\{\s*\}/);
    // Should emit the pseudo rule
    expect(css).toContain('.pharos-switch:hover {');
  });
  ...
  it('keeps non-pseudo string values as regular properties', () => {
    ...
    expect(css).toContain('border-color: var(--color-accent)');
    // No pseudo rules
    expect(css).not.toContain('.pharos-card:');
  });
});
...
describe('container padding mapping', () => {
  it('maps uniform padding to shorthand token on card', () => {
    ...
    expect(css).toContain('--pharos-card-padding: 20px');
    // Should NOT emit directional tokens (shorthand covers all sides)
    expect(css).not.toContain('--pharos-card-padding-inline');
    expect(css).not.toContain('--pharos-card-padding-block-start');
  });
  ...
  it('maps asymmetric padding to directional tokens', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-inline: 20px');
    expect(css).toContain('--pharos-card-padding-block-start: 16px');
    expect(css).toContain('--pharos-card-padding-block-end: 16px');
    // Should NOT emit shorthand (sides differ)
    expect(css).not.toMatch(/--pharos-card-padding:[^-]/);
  });
  ...
  it('maps paddingBlock and paddingInline separately', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-inline: 16px');
    expect(css).toContain('--pharos-card-padding-block-start: 24px');
    expect(css).toContain('--pharos-card-padding-block-end: 24px');
  });
  ...
  it('works for section and dialog too', () => {
    ...
    expect(css).toContain('--pharos-section-padding: 12px');
    // Dialog — asymmetric → directional
    expect(css).toContain('--pharos-dialog-padding-inline: 32px');
    expect(css).toContain('--pharos-dialog-padding-block-start: 24px');
  });
  ...
  it('does NOT map padding on non-container components', () => {
    ...
    expect(css).not.toContain('--pharos-button-padding');
  });
  ...
  it('maps 3-value padding shorthand', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-block-start: 16px');
    expect(css).toContain('--pharos-card-padding-block-end: 12px');
    expect(css).toContain('--pharos-card-padding-inline: 20px');
  });
  ...
  it('maps paddingInline shorthand with two values', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-inline-start: 12px');
    expect(css).toContain('--pharos-card-padding-inline-end: 20px');
  });
  ...
  it('maps paddingBlockStart alone', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-block-start: 32px');
    expect(css).not.toContain('--pharos-card-padding-block-end');
    expect(css).not.toContain('--pharos-card-padding-inline');
  });
  ...
  it('maps paddingInlineStart and paddingInlineEnd separately', () => {
    ...
    const css = generateThemeCSSFlat(theme);
    expect(css).toContain('--pharos-card-padding-inline-start: 8px');
    expect(css).toContain('--pharos-card-padding-inline-end: 24px');
  });
});
