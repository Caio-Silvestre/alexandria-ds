...
/**
 * defineTheme — Create an Pharos theme from a flat token map.
 *
 * Two distribution modes:
 * - Unbuilt: Theme generates CSS and injects a <style> tag at runtime
 * - Built: `pharos theme build` pre-compiles to a CSS file; Theme just
 *   sets the data-pharos-theme attribute
 *
 * Token values can be:
...
/** All valid Pharos core token names */
...
/** All valid Pharos token names — core + domain tokens */
...
export interface DefineThemeInput {
  /** Theme name — used for data-pharos-theme attribute and identification */
  name: string;
   ...
   * the theme's data-pharos-theme attribute via @scope.
   *
   * Use `prop:value` keys to target specific visual props. New values
   * not in the base type are automatically detected by `pharos theme build`
...
/** All Pharos token defaults as a flat map. Useful for resolving full token sets. */
