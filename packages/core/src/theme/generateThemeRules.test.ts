...
describe('generateThemeRules', () => {
  ...
  it('includes .pharos-heading.level-* rules for all 6 levels', () => {
    for (let level = 1; level <= 6; level++) {
      const rule = rules.find(r =>
        r.includes(`.pharos-heading.level-${level}`),
      );
  ...
  it('includes .pharos-text.* rules for all 5 types', () => {
    for (const type of ['body', 'large', 'label', 'code', 'supporting']) {
      const rule = rules.find(r => r.includes(`.pharos-text.${type}`));
      expect(rule).toBeDefined();
  ...
  it('includes explicit component overrides', () => {
    const buttonRule = rules.find(r => r.includes('.pharos-button.secondary'));
  ...
  it('applies pseudo-class suffixes to both compat selector prefixes', () => {
    ...
    expect(
      pseudoRules.some(rule => rule.includes('.pharos-button:hover')),
    ).toBe(true);
  });
  ...
  it('includes prose heading rules with computed values', () => {
    ...
    // via StyleX (@layer pharos-base). Emitting margins here would re-introduce
  ...
  it('includes color prop overrides for text and heading', () => {
    expect(rules.some(r => r.includes('.pharos-text.primary'))).toBe(true);
    expect(rules.some(r => r.includes('.pharos-text.secondary'))).toBe(true);
    expect(rules.some(r => r.includes('.pharos-heading.primary'))).toBe(true);
    expect(rules.some(r => r.includes('.pharos-heading.disabled'))).toBe(true);
    expect(rules.some(r => r.includes('.pharos-text.active'))).toBe(false);
    expect(rules.some(r => r.includes('.pharos-text.accent'))).toBe(true);
  });
  ...
  it('emits Text size-prop font-size overrides in the same layer as type rules', () => {
    ...
    const sizeRule = rules.find(
      r => r.includes('.pharos-text.size-2xs') && r.includes('font-size'),
    );
    ...
    const xsmRule = rules.find(r => r.includes('.pharos-text.xsm'));
  ...
  it('emits a size override for every TextSize value', () => {
    ...
    for (const cls of sizes) {
      expect(
        rules.some(
          r => r.includes(`.pharos-text.${cls}`) && r.includes('font-size'),
        ),
      ).toBe(true);
    }
  });
  ...
  it('orders size overrides after the themed type font-size rules', () => {
    ...
    // override must come after the `.pharos-text.<type>` type rule.
    const typeIdx = rules.findIndex(
      r => r.includes('.pharos-text.supporting') && r.includes('font-size'),
    );
  ...
  it('generateThemeCSS returns prose and component blocks with @scope', () => {
    ...
    expect(combined).toContain('@scope ([data-pharos-theme="default"])');
    expect(combined).toContain('to ([data-pharos-theme])');
  ...
  it('routes Text size overrides into the component (pharos-theme) block', () => {
    // The size override only beats a themed type if it lands in the same
    // layer as the type rules (pharos-theme / component block), not the
    // reset-tier prose block.
    const {prose, component} = generateThemeCSS(theme);
    expect(component).toContain('.pharos-text.size-2xs');
    expect(component).toContain('.pharos-text.xsm');
    expect(prose).not.toContain('.pharos-text.size-2xs');
  });
});
...
describe('derived var expansion', () => {
  it('emits borderRadius AND internal var for card', () => {
    ...
    const rules = generateThemeRules(theme);
    const cardRule = rules.find(r => r.includes('.pharos-card'));
  ...
  it('emits borderRadius AND internal var for dropdown-menu', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-dropdown-menu'));
    expect(rule).toBeDefined();
  ...
  it('emits internal vars for chat composer', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-chat'));
    expect(rule).toBeDefined();
  ...
  it('emits internal var for button borderRadius', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-button'));
    expect(rule).toBeDefined();
  ...
  it('emits internal vars for avatar fallback typography (base)', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-avatar'));
    expect(rule).toBeDefined();
  ...
  it('emits a per-size fallback font-size for avatar (size:sm)', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-avatar.sm'));
    expect(rule).toBeDefined();
  ...
  it('does not emit derived vars for components without registry entries', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-badge'));
    expect(rule).toBeDefined();
  ...
  it('container expansion still works for card padding', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-card'));
    expect(rule).toBeDefined();
    // Container expansion emits --pharos-card-padding token
    expect(rule).toContain('--pharos-card-padding: 20px');
  });
  ...
  it('handles variant-specific derived vars', () => {
    ...
    const rules = generateThemeRules(theme);
    const rule = rules.find(r => r.includes('.pharos-card.muted'));
    expect(rule).toBeDefined();
    expect(rule).toContain('border-radius: 16px');
...
describe('brutalist-style derived expansion', () => {
  ...
  it('card padding emits container tokens via derived expansion', () => {
    ...
    const rule = rules.find(r => r.includes('.pharos-card'));
    expect(rule).toBeDefined();
    expect(rule).toContain('--pharos-card-padding: 24px');
  });
