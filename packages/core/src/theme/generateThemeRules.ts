 ...
 * path (`pharos theme build` pre-compiles to CSS files).
 *
...
/** Media-surface selector, e.g. for [data-pharos-media="dark"]. */
function mediaSelector(surface: string): string {
  return `[data-${dataAttrNamespace}-media="${surface}"]`;
}

/** Component base-class selector, e.g. '.pharos-button'. */
...
export interface ThemeRulesSplit {
  /** Token overrides + component .pharos-* overrides + prop-level color rules */
  component: string[];
...
export interface ThemeCSSOutput {
  ...
  prose: string;
  /**
   * Token overrides + component .pharos-* overrides scoped to the theme.
   * Should be injected into @layer pharos-theme — above StyleX layers so
 ...
 *
 * Emits the rebranded --pharos-<component>-padding tokens (shorthand +
 * directional overrides), e.g.:
 *   --pharos-card-padding: 20px
 *   --pharos-card-padding-inline: 20px
 *   --pharos-card-padding-block-start: 20px
 *   --pharos-card-padding-block-end: 20px
 *
 * The component reads these with an inverted fallback chain
 * (var(--pharos-*, default)). The container.stylex.ts default styles read
 ...
 * the runtime path (useInsertionEffect) and the build path (pharos theme build).
 */
export function generateThemeRules(theme: DefinedTheme): string[] {
  ...
  // 2. Component overrides (.pharos-* class rules; components also emit
  ...
  //    themed type's font-size across the pharos-base → pharos-theme layers)
...
/**
 * Generate component override rules using the .pharos-* class selector
 ...
 * any class-based style (StyleX, .pharos-* overrides) should beat.
 * The caller places these in the reset layer (not pharos-theme) so they
 ...
 * size implied by `type`. Its StyleX class lives in `@layer pharos-base`, but a
 * theme's per-type font-size rule (`.pharos-text.<type>`) lives in the higher
 * `@layer pharos-theme`, so the layer cascade let the theme silently shadow
 * `size` for any `type` the theme styled. Re-emitting the size classes here —
 * same layer as the type rules, same `.pharos-text.<x>` specificity, later in
 ...
 *
 * Component rules (tokens, .pharos-* overrides) are intentional theme overrides
 * that need to beat StyleX — they stay in pharos-theme (above StyleX layers).
 */
 ...
 * [data-pharos-media] elements. Parent theme component overrides flow through
 ...
 * - `component`: @scope'd token + .pharos-* overrides → inject into @layer pharos-theme
 *
...
export function generateThemeCSS(theme: DefinedTheme): ThemeCSSOutput {
  ...
  // on-media rules in unbounded scope (can reach [data-pharos-media] elements)
  let componentCss = '';
