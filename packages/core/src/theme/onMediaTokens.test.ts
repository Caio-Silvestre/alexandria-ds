...
describe('generateOnMediaCSS', () => {
  it('emits @scope with [data-pharos-media] token rules', () => {
    const theme = defineTheme({name: 'test'});
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      '@scope ([data-pharos-theme="test"])',
    );
    // Same scope boundary as main theme
    expect(css).toContain('to ([data-pharos-theme])');
    expect(css).toContain('[data-pharos-media="dark"]');
  ...
  it('emits light media rules', () => {
    const theme = defineTheme({name: 'test'});
    const css = generateOnMediaCSS(theme);
    expect(css).toContain('[data-pharos-media="light"]');
  ...
  it('emits component override rules', () => {
    ...
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      ':is([data-pharos-media="dark"]) :is(.pharos-button.secondary)',
    );
    expect(css).toContain(
  ...
  it('emits pseudo-class rules for on-media components', () => {
    ...
    const css = generateOnMediaCSS(theme);
    expect(css).toContain(
      ':is([data-pharos-media="dark"]) :is(.pharos-button):hover',
    );
...
describe('reset.css baseline media rules', () => {
   ...
   * MediaTheme: a color-scheme flip on [data-pharos-media].
   *
  ...
  it('flips color-scheme on [data-pharos-media="dark"]', () => {
    const darkMatch = resetCSS.match(
      /:where\(\[data-pharos-media="dark"\]\)\s*\{([^}]+)\}/,
    );
    expect(darkMatch).not.toBeNull();
    expect(darkMatch![1]).toContain('color-scheme: dark');
  });

  it('flips color-scheme on [data-pharos-media="light"]', () => {
    const lightMatch = resetCSS.match(
      /:where\(\[data-pharos-media="light"\]\)\s*\{([^}]+)\}/,
    );
