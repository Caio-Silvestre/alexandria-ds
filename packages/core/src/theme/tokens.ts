 ...
 *
 * Use these helpers when code outside React hooks needs Pharos token values:
...
/**
 * Return a CSS custom property reference for an Pharos token name.
 *
 ...
 * stay connected to the active Pharos theme through the CSS cascade.
 *
 * @example
 * ```ts
 * const theme = {
...
/** Flat map of every known Pharos token name to its `var(--token-name)` reference. */
...
/**
 * Resolve all Pharos token values for a theme and effective color mode.
 *
...
/** Resolve one Pharos token value for a theme and effective color mode. */
