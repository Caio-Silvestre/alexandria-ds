...
/**
 * Pharos Type Definitions
 *
 * Shared types used across Pharos components.
 */
 ...
 * or an @import in CSS). Pharos does not handle font loading — it only
 ...
 * treatment from theme CSS (`.pharos-text.<custom-type> { ... }`).
 *
 * To add type-safe custom types, use module augmentation:
 * ```ts
 * declare module '@pharos-ds/core/theme' {
 ...
 * ```
 *
 * `pharos theme build` generates these augmentations automatically when it
