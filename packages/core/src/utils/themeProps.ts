...
/**
 * Build the pharos-* class name string for a component.
 *
 * Every component renders a stable base class (`pharos-button`, `pharos-card`,
 ...
 *
 * The `pharos-` prefix comes from the centralized naming module
 ...
 * @returns Class name string (e.g. "pharos-button secondary sm")
 *
 * @example
 * ```ts
 * buildClassName('button', { variant: 'secondary', size: 'sm' })
 * // → "pharos-button secondary sm"
 *
...
/**
 * Reflect Pharos visual props as `data-*` attributes.
 *
 ...
 * receives the stable Pharos class name.
 *
 * This emits the stable pharos class plus the data-attribute reflection
 * surface. For example:
 *
 * ```ts
 * themeProps('button', { variant: 'primary', size: 'sm' })
 * // → { className: 'pharos-button primary sm', data-variant: 'primary', data-size: 'sm' }
 * ```
 */
