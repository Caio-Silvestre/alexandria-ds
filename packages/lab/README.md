...
Experimental Pharos components. This package is a staging area for components being developed before graduating to `@pharos-ds/core`. It ships to npm **only under the `@canary` dist-tag** — there is never a stable (`latest`) release.
...
- Compose with `@pharos-ds/core` components (use the theme, follow naming conventions)
- Graduate to `@pharos-ds/core` after thorough engineering review

See the **[Component Lifecycle](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Lifecycle)** wiki for how a component moves from lab → core (and the promotion gates), and the **[Component Hardening Protocol](https://github.com/Caio-Silvestre/alexandria-ds/wiki/Component-Hardening-Protocol)** for the bar a component must clear to graduate.
...
```tsx
import {CodeEditor} from '@pharos-ds/lab';
```
...
```bash
npm install @pharos-ds/lab@canary @pharos-ds/core@canary
```

```tsx
import {CodeEditor} from '@pharos-ds/lab';
import '@pharos-ds/core/pharos.css';
import '@pharos-ds/lab/lab.css';
```
