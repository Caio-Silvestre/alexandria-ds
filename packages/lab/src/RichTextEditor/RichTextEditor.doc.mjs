...
export const docs = {
  ...
  theming: {
    targets: [{className: 'pharos-rich-text-editor', visualProps: []}],
  },
  usage: {
    description:
      'A WYSIWYG rich-text editor built on Lexical, styled with Pharos design tokens. Experimental component in @pharos-ds/lab (canary). lexical and @lexical/* are optional peer dependencies. The editor is deliberately minimal and extensible: pass nodes and plugins to layer richer behaviour (toolbars, mentions, hover cards) on top without forking. Use RichTextView to render serialized content read-only.',
    bestPractices: [
      {
        guidance: true,
        description:
          'Install lexical and @lexical/react (optional peers) before importing from @pharos-ds/lab.',
      },
      {
        guidance: true,
        description:
