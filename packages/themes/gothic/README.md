...
**Dark-only** atmospheric gothic theme for Pharos — deep blue-gray surfaces, distressed display heading, and pastel categorical accents that glow against the dark page like illuminated panels. Uses Manufacturing Consent for headings, Fustat for body text, and [Lucide](https://lucide.dev) icons.
...
```bash
npm install @pharos-ds/theme-gothic
```
...
import {gothicTheme} from '@pharos-ds/theme-gothic/built';
...
| Path                          | Use case                                               |
| ----------------------------- | ------------------------------------------------------ |
| `@pharos-ds/theme-gothic`           | Source build (StyleX compilation via `@pharos-ds/build`)     |
| `@pharos-ds/theme-gothic/built`     | Pre-built dist (Tailwind, plain CSS, or no build step) |
| `@pharos-ds/theme-gothic/theme.css` | Pre-built CSS file (import in your stylesheet)         |
...
```css
@import '@pharos-ds/theme-gothic/theme.css';
```
...
```tsx
import {gothicPalettes} from '@pharos-ds/theme-gothic';
