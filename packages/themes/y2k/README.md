...
Playful Y2K pop theme for Pharos: a soft periwinkle body, charcoal accent, holographic categorical colors derived from HCT tonal palettes, Poppins for body/headings, and a Crimson Text serif for display sizes.

## Install

```bash
npm install @pharos-ds/theme-y2k
```
...
import {y2kTheme} from '@pharos-ds/theme-y2k/built';
...
| Path                       | Use case                                               |
| -------------------------- | ------------------------------------------------------ |
| `@pharos-ds/theme-y2k`           | Source build (StyleX compilation via `@pharos-ds/build`)     |
| `@pharos-ds/theme-y2k/built`     | Pre-built dist (Tailwind, plain CSS, or no build step) |
| `@pharos-ds/theme-y2k/theme.css` | Pre-built CSS file (import in your stylesheet)         |
...
```css
@import '@pharos-ds/theme-y2k/theme.css';
```
