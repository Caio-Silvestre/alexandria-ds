 ...
 * @input Pharos theme tokens via useTheme
 * @output Predefined Vega-Lite Config object for Pharos-themed charts
 * @position Utility module; consumed by VegaChart and exported standalone
 *
 * Ported from the internal Pharos data-viz config — structural config only.
 * Colors are wired through Pharos data tokens (see domainTokens/dataTokens.ts).
 *
...
/**
 * Build a Vega-Lite `Config` object themed with Pharos tokens.
 *
 ...
 * scales (which are set via `range` using the Pharos data-viz tokens).
 */
