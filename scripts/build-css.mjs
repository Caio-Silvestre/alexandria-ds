 ...
 * and outputs a combined stylesheet wrapped in @layer pharos-base.
 *
 * Usage:
 *   node scripts/build-css.mjs                 # core  → packages/core/dist/pharos.css
 ...
 * 3. Outputs a combined stylesheet with all rules in @layer pharos-base
 *
...
// cross-package theme-token imports (lab consumes @pharos-ds/core tokens).
const TARGETS = {
  core: {
    src: path.resolve(ROOT, 'packages/core/src'),
    dist: path.resolve(ROOT, 'packages/core/dist'),
    outFile: 'pharos.css',
    banner: 'Pharos Pre-compiled StyleX CSS — all components',
    aliases: {},
  },
  lab: {
    ...
    outFile: 'lab.css',
    banner: 'Pharos Lab Pre-compiled StyleX CSS — experimental components',
    // lab imports @pharos-ds/core/theme/tokens.stylex; point the resolver at
    // core's source so the cross-package token reference resolves.
    aliases: {
      '@pharos-ds/core/*': [path.join(ROOT, 'packages/core/src/*')],
      '@pharos-ds/core': [path.join(ROOT, 'packages/core/src')],
    },
  },
  charts: {
    ...
    outFile: 'charts.css',
    banner:
      'Pharos Charts Pre-compiled StyleX CSS — data visualization components',
    // charts imports @pharos-ds/core/theme/tokens.stylex; point the resolver
