 ...
 *
 * Validates that pharos.css contains expected @media wrappers and
...
describe('build-css pharos.css', () => {
  let pharosCss;

  beforeAll(async () => {
    // Core — including its `build:css` step that emits pharos.css — is built
    ...
    ensureCoreBuilt();
    pharosCss = await fs.readFile(path.join(CORE_DIST, 'pharos.css'), 'utf8');
  }, 180_000);

  it('contains @media rules', () => {
    const mediaRules = extractMediaRules(pharosCss);
    expect(mediaRules.length).toBeGreaterThan(0);
    console.log(`pharos.css has ${mediaRules.length} @media rules`);
  });
  ...
  it('contains prefers-reduced-motion rules', () => {
    ...
    console.log(`pharos.css has ${motionRules.length} prefers-reduced-motion rules`);
  });
  ...
  it('no transition-duration:0s rules appear outside @media blocks', () => {
    ...
    const matches = [...pharosCss.matchAll(zeroTransitionRegex)];

    for (const match of matches) {
      const position = match.index;
      const before = pharosCss.slice(0, position);
      const mediaStarts = [...before.matchAll(/@media[^{]*\{/g)];
      const isWrapped = mediaStarts.some(m => {
        const mediaStart = m.index;
        const afterMedia = pharosCss.slice(mediaStart);
        let depth = 0;
