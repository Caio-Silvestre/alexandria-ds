...
function measure(names, comps) {
  ...
  const pharosCss = existsSync(join(DIST, 'pharos.css')) ? gzSize(join(DIST, 'pharos.css')) : {raw: 0, gzip: 0};
  return {
    count: found.length, found,
    js: {gzip: tG, raw: tR, files: allDeps.size},
    pharosCss,
    total: {gzip: tG + pharosCss.gzip, raw: tR + pharosCss.raw},
  };
}
...
function printSet(label, r) {
  ...
  console.log(`  pharos.css:       ${pad(fmt(r.pharosCss.gzip), 10, 'right')}`);
...
async function printCompare(mappings, comps) {
  ...
  if (hasMap) {
    ...
    console.log('-'.repeat(65));
    console.log(`${pad(`XDS @pharos-ds/core (${an.length} groups)`, 42)} ${pad(fmt(xr.total.raw), 10, 'right')} ${pad(fmt(xr.total.gzip), 10, 'right')}`);
    for (const r of results)
