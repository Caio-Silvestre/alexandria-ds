...
const ctx = {
  pre1: true,
  pubNames: new Set(['@pharos-ds/core', '@pharos-ds/cli']),
  allNames: new Set([
    '@pharos-ds/core',
    '@pharos-ds/cli',
    '@pharos-ds/storybook',
...
describe('validateChangeset — 0.x semver coupling', () => {
  ...
  it('accepts a non-breaking category with a patch bump', () => {
    for (const category of [
      ...
      const problems = validateChangeset(
        'a.md',
        cs("'@pharos-ds/core': patch", `[${category}] Something (#1)\n@who`),
        ctx,
      );
  ...
  it('rejects [breaking] declared as patch', () => {
    const problems = validateChangeset(
      'a.md',
      cs("'@pharos-ds/core': patch", '[breaking] Remove onHide (#1)\n@who'),
      ctx,
    );
  ...
  it('rejects a major bump while 0.x', () => {
    const problems = validateChangeset(
      'a.md',
      cs("'@pharos-ds/core': major", '[breaking] Total rewrite (#1)\n@who'),
      ctx,
    );
    expect(problems).toHaveLength(1);
  ...
  it('does not enforce the coupling once past 1.0', () => {
    const problems = validateChangeset(
      'a.md',
      cs("'@pharos-ds/core': minor", '[feat] Add prop (#1)\n@who'),
      {...ctx, pre1: false},
    );
    expect(problems).toEqual([]);
  });
  ...
  it('still flags a missing category and missing contributor', () => {
    const problems = validateChangeset(
      'a.md',
      cs("'@pharos-ds/core': patch", 'no category or handle here'),
      ctx,
    );
  ...
  it('flags a private/ignored package', () => {
    const problems = validateChangeset(
      'a.md',
      cs("'@pharos-ds/storybook': patch", '[fix] x (#1)\n@who'),
      ctx,
    );
