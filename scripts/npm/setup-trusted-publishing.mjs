 ...
 * active npm session) that prepares every publishable @pharos-ds/* package
 ...
 *
 * It ports facebook/lexical's scripts/npm/setup-trusted-publishing.mjs to pharos
 * conventions (node:util parseArgs, node:fs, node:child_process, global fetch —
 * no minimist / fs-extra, which pharos does not declare). It does three
 ...
 *      "pending publisher"). None of the @pharos-ds/* names exist yet, so this
 ...
 *   `job_workflow_ref`). pharos's publish runs directly in the non-reusable
  ...
  for (const dir of expandWorkspaceDirs(ROOT)) {
    ...
    if (p.name) {
      pkgs.push({
        ...
        name: p.name,
        private: !!p.private,
        canaryOnly: !!p.pharos?.canaryOnly,
        version: p.version,
      });
    }
  }
...
// The publishable @pharos-ds/* packages that need a trusted-publisher config.
...
// ---------------------------------------------------------------------------
// CLI flags (node:util parseArgs — pharos avoids minimist)
// ---------------------------------------------------------------------------
...
function parseCliArgs(argv) {
  const {values} = parseArgs({
    ...
    options: {
      ...
      // The CALLING workflow filename npm should trust. For pharos this is the
      ...
      repo: {type: 'string', default: 'facebook/pharos'},
    },
...
async function publishStub(pkg, {registry, stubVersion, repo, dryRun}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pharos-stub-'));
  try {
...
async function main() {
  ...
  if (!repoOwner || !repoName) {
    console.error(
      `Invalid --repo value "${repo}". Expected owner/name, e.g. facebook/pharos.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Found ${pkgs.length} publishable @pharos-ds/* package(s); ` +
